...
- [ ] **バッテリー電圧 ADC → `/<ns>/battery`** — `doc02:17`（7.4V リポ）+ `doc03:79`（`sensor_msgs/BatteryState`・実機は micro-ROS firmware が供給 Phase 1+）。`publishBattery()` の `percentage` は config `safety.battery_percentage_scale` と**同一スケール**で出す（State Cache / Emergency Guardian の split-brain 回避、`doc03:82`）。
...
- [ ] **D1. heartbeat / watchdog → motor 停止（通信断 stop）を L0(MCU) に実装** — box-map/eval-gate 群は既に **firmware(L0) 所有**として割当済（自作境界 `heartbeat / watchdog` [productization/06:225](../docs/productization/06-oss-reuse-and-box-small-designs.md)、H-G6「heartbeat lost → motor stop・上位通信非依存・owner=firmware」[productization/08:105](../docs/productization/08-navigation-hardware-eval-gates.md)、reason_code `heartbeat_lost` [08:113](../docs/productization/08-navigation-hardware-eval-gates.md) / [06:228](../docs/productization/06-oss-reuse-and-box-small-designs.md)）。だが **doc12 の Layer 0（[12:76-78](../docs/architecture/12-infrastructure-common.md)＝速度クランプ＋近接停止のみで heartbeat 行 79 を含まない範囲）には #466 起票時点で heartbeat/watchdog が未記載**だった（`src/main.cpp:147-158` は「MCU に置く hard-cut は Phase 1 で判断」と**明示先送り**、本チェックリストにも項目が無かった）。**その後 #467 で doc12 §Layer 0（[12:79](../docs/architecture/12-infrastructure-common.md)）に heartbeat/watchdog deadman が反映済**＝下記 CMD_TIMEOUT サブ箇条と整合する（現状は「反映済」が正）。
  ...
  - **`CMD_TIMEOUT` 導出ポリシー（値は発明せず Phase 1 実測。ここでは導出則のみ確定）**:
    - **上限** = `0.3 m/s × CMD_TIMEOUT ≤ 許容 residual 走行距離`。ミニチュア（車体~15cm・通路~200mm＝R-42 [doc07:252](../docs/shared/07-research-notes.md)）ゆえ **Policy Gate の 0.6m 包絡（`0.3 m/s × 2.0s`）より締め、robot-length オーダー（車体 150mm ＝上記 R-42 ・≤0.2m 目安）** に置く。前例＝[`policy_gate.py:43-48`](../ws/src/warehouse_mcp_server/warehouse_mcp_server/policy_gate.py) の residual 包絡（`0.3 × window` が unobserved-travel を bound）。
    - **下限** = 健全時 `/cmd_vel` 到着間隔 × N（false-trip 回避）。実値は上下限 band 内で **Phase 1 実機実測**（`/cmd_vel` レート測定）。
    - **実装時**: [`config.h:10`](include/config.h) の `MAX_ANGULAR_VELOCITY`（`2.0 rad/s`・plain constexpr placeholder）と **同型の plain constexpr Phase-1 placeholder**（`// TODO: Phase 1 実測`）として `config.h` に宣言する。**build-flag 化・凍結ハードキャップ化しない**（build-flag 由来＝速度上限だけ＝[`config.h:9`](include/config.h) の `MAX_LINEAR_VELOCITY = MAX_LINEAR_VELOCITY_MMPS/1000`＝0.3 m/s）。純判定 [`command_watchdog.h`](include/command_watchdog.h)（`command_is_stale`・`timeout`=引数・unsigned rollover-safe・`>` 境界＝#468）は **不変**。
    - **規律**: **tighten-only**（config は shrink のみ・residual 上限を超える loosen は fail-closed 拒否＝`warehouse_interfaces/config.py` の lower-never-raise と同型・[ADR-0004](../docs/adr/0004-l2-restrict-only-policy-profile.md)）。**doc12 §Layer 0 heartbeat/watchdog 反映は #467 で済**（[doc12:79](../docs/architecture/12-infrastructure-common.md)・safety-state 所有）ゆえ本 PR は doc12 を触らない。
- [x] **D2. R-43 LaserScan MTU → 既定＝Reliable + MTU-first（採用決定）** — 360°/0.4°≈3.6KB/scan が UDP MTU 512B 超過（2台常時送出。[doc07:253](../docs/shared/07-research-notes.md)）。**既定＝(b) Reliable QoS + MTU/fragmentation 設定を第一手**とし **0.4° フル分解能を保持**する。ESP32 RAM 予算（T4 [doc06:131](../docs/architecture/06-implementation-phases.md) / [doc07:78](../docs/shared/07-research-notes.md)）・再送レイテンシ／単一フラグメント欠落リスクが実機で保てない場合の **fallback = (a) ダウンサンプル**（2段ゲート導出: (i) 単一 UDP datagram に収まる最粗 → (ii) AMCL 収束＋200mm 隘路（R-41/R-42 [doc07:251-252](../docs/shared/07-research-notes.md)）を通る最粗）。**最終手段 = (c) USB 有線**。**値（MTU・分解能）は発明せず Phase 1 実機（T3 併せ）で導出**（R-37 host spike は MTU 未検証）。トピック契約＝[doc03:78 `/bot{n}/scan`](../docs/architecture/03-software-architecture.md)（producer=firmware が分解能/QoS 設定）。→ 用語 [R-43](../docs/GLOSSARY.md)。
