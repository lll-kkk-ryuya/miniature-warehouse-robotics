...
- **実装戦略・未決判断**: L0 責務の docs 横断突合・4層安全マップ・A〜D 依存順・cross-doc 整合ドリフト（heartbeat/watchdog stop の層割当・R-43 MTU）を [`phase1-l0-strategy.html`](phase1-l0-strategy.html) に図解。着手前に確定すべき decision-needed（D1 heartbeat/watchdog・D2 R-43）は [`PHASE1_CHECKLIST.md`](PHASE1_CHECKLIST.md) 末尾「未決の設計判断・cross-doc 整合」。**doc12 §Layer 0 に heartbeat/watchdog deadman を反映済（#467・責務・実 enforcement は Phase 1）。純判定 `command_watchdog.h`（`command_is_stale`・unsigned rollover-safe・host-test R-26・本 PR #468）＝L1 の blocked-timeout watchdog(doc12:82) とは別**。
...
- **XRCE client_key host unit（R-26・R-37 enabler）**: distinct/deterministic な per-board `client_key` 導出（`include/client_key.h`・Arduino/rmw 非依存）を host 検証する（ESP32 不要）。`pio test -e native`（allowlist に `test_client_key` 追加済）／ `pio` 不在なら `bash test/run_client_key_test.sh`（g++/clang + Unity shim・同一テスト源）。固定する契約: PRIMARY distinct-by-BOT_ID（`(key>>24)==bot_id`・同一MACでも bot1≠bot2＝R-37 load-bearing）＋ distinct-by-MAC（同一 BOT_ID 誤焼き縮退の defense-in-depth）＋ deterministic/exact 手計算リテラル（`mac={AA..FF}`→bot1 `0x01775533`/bot2 `0x02775533`）＋ 非ゼロ（`bot_id>=1 ⇒ key!=0`・CLIENTKEY_INVALID 回避）。期待値は**独立オラクル（手計算）**でありビット式の再導出ではない。安全 R-26 クランプ gate（`run_host_test.sh`）とは別ゲートで不変。
...
- **produce（純ロジック・host-unit-tested・R-26）**: `include/command_watchdog.h`（`command_is_stale(last,now,timeout)`＝comms-loss heartbeat deadman の純判定・**unsigned rollover-safe**）。**L0（MCU内・通信非依存）**。Arduino 非依存・`test/test_watchdog` で固定（mutation-red・CI `firmware-safety`）。timeout は引数＝Phase 1 実測（発明しない）。`main.cpp:loop` の Phase-1 watchdog wiring が消費予定。L1 の blocked-timeout watchdog(doc12:82) とは別。
- **produce（純ロジック・host-unit-tested・R-26・R-37 enabler）**: `include/client_key.h`（`derive_xrce_client_key(bot_id, mac[6])`＝2台が単一 Agent で衝突しないための **distinct/deterministic な XRCE `client_key`** 導出、helper `fold_mac24`）。**L0 enabler**（弱RNG `srand(uxr_nanos());rand()` 由来の session 衝突を board identity 決定的導出で排除。spike/RESULT.md:54）。Arduino/rmw 非依存・`test/test_client_key` で固定（独立オラクル手計算リテラル・mutation-red・CI `firmware-safety`）。`bot_id`(BOT_ID build flag)・`mac`(実行時 MAC) は引数＝発明しない。`main.cpp:setup` の Phase-1 `rmw_uros_options_set_client_key()`（main.cpp:132）配線が消費予定。正本 doc07:242 / spike/RESULT.md:63。**新トピック/型/契約は産まない**（firmware は ROS 契約外）。
