...
- **キネマティクス host unit**: skid-steer mix（`mixSkidSteer` 差動逆運動学）+ dead-reckon odom（`integrateOdom` 順運動学）の純ロジック（`include/kinematics.h`・Arduino 非依存）を `test/test_kinematics/` で host 検証。`bash test/run_kinematics_test.sh`（同梱 Unity shim・**hardware 値は引数＝発明しない**・mutation 非空）。**安全 R-26 クランプ gate（`run_host_test.sh`）とは別ゲート**＝クランプ gate は不変に保つ。
