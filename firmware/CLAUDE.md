...
- **produce（内部 stub IF・Phase 1 で実装）**: `main.cpp` に `setMotorVelocity(v,w)`（クランプ後の Layer-0 sink・doc02:14 左右2ch ミックスはコメント契約）/ `publishOdom|Scan|Battery`（doc03:77-79 の**凍結型**を載せる publisher stub）/ `initMS200`（UART init）。**`publishImu` は placeholder で publish 未配線**（`/bot{n}/imu` は doc03:81 で契約外＝新規配線は contract-PR・判断は epic #3）。実 rclc publish・UART parse・PWM は Phase 1（[`PHASE1_CHECKLIST.md`](PHASE1_CHECKLIST.md)）。**新トピック/型/契約は産まない**（doc03 のまま）。
