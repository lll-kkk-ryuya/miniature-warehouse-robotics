...
- `compat.py` — **py3.10 互換 shim（#563・ADR-0008 追記 2026-08-30）**: 単一共有 `StrEnum`（py3.11+ = stdlib re-export＝挙動不変 / py3.10 = `str,Enum` + `__str__ = str.__str__`）。全 pkg はここから import する（`from enum import StrEnum` 直 import・`auto()` は `tests/unit/test_py310_compat.py` の source-scan で恒久禁止＝Jetson/Humble ボードで ImportError / isinstance 破壊を防ぐ）。
