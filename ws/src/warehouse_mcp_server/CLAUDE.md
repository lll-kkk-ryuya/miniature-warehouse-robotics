...
- **`get_fleet_status` は emergency 観測 2 系統を additive に返す**（#593 follow-up・doc12:389-409 のフロー実装）: `emergency{active,history}`＝State Cache event ring の素通し（append-only・clear protocol なし）／`l2_emergency_holds`＝`PolicyGate.emergency_holds()`（L2 が**今** enforce している Guardian estop mirror の level 状態・sorted list）。**両系統は独立・別キーで混ぜない**（doc12:631 裁定）。tool 返り値は契約外 JSON＝凍結契約不変。R-26 unit=`tests/unit/test_fleet_status_emergency.py`。
