...
- `emergency_sync.py` — **Guardian estop→L2 emergency ミラー（#592・doc12 【2026-09-07 追補】）**。`EmergencyLevelMirror`（純・rclpy 非依存）: `/bot{n}/cmd_vel/emergency` の level 信号を `PolicyGate.set_emergency(bot, True)` へ写像し、無信号 > `policy_gate.emergency_clear_after_s`（既定 1.0s・fail-closed・tighten-only **floor**＝ADR-0004。鮮度窓の ceiling と向きが逆）で `False` に落とす（厳密 `>`・`clear_after_from_config` が `freshness_from_config` と同規律で検証）。購読 marshal は L4 `llm_bridge`（layer ≠ process・productization/01:192）。`WarehouseTools.policy_gate` read-only property が配線 seam。R-26 unit=`tests/unit/test_emergency_gate_sync.py`（mutation 4 種赤化確認済）。**hold/clear の遷移エッジのみ 1 行 log**（held=受信・clear=沈黙>設定窓・各行に現在 held 集合。20Hz 再アサートは無音＝エッジ検出で抑制）。**両エッジとも `WARNING`**（既定 WARNING 構成で clear だけ落ちると「いつ dispatch が再開したか」が消えるため。level は unit で pin）。
...
- **`get_fleet_status` は emergency 観測 2 系統を additive に返す**（#593 follow-up・doc12:389-409 のフロー実装）: `emergency{active,history}`＝State Cache event ring（**history のみ LLM 表示面で直近 `EMERGENCY_HISTORY_LLM_MAX`(10) 件に切詰め**＝doc08 【2026-09-09 追補】・active は素通し・state.json 不変）／`l2_emergency_holds`＝`PolicyGate.emergency_holds()`（L2 が**今** enforce している Guardian estop mirror の level 状態・sorted list）。**両系統は独立・別キーで混ぜない**（doc12:631 裁定）。`EMERGENCY_HISTORY_LLM_MAX` は本 pkg 単一定義で situation 表示面（`warehouse_llm_bridge/situation.py`）と共有。tool 返り値は契約外 JSON＝凍結契約不変。R-26 unit=`tests/unit/test_fleet_status_emergency.py`。
...
- **availability / emergency は契約に無い**: availability は `StateSnapshot.timestamp`（snapshot 全体の鮮度）から局所導出（>0.5s stale / >2s unavailable）。emergency は in-memory set＝**#592 で Guardian estop level ミラー（`emergency_sync.py`・llm_bridge 配線）が feed 済**（seed kwarg は tests/recovery 用に存続。stdio `server.py` 別プロセス経路は ROS 文脈なしで未 feed＝現用外・doc12 追補「残」）。availability の明示 field 化は引き続き契約拡張課題 → rules §4 で調整。# TODO(coordinate availability field)
