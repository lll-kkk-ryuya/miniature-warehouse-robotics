...
- `nav2_bridge.py` — `BasicNavigatorBackend`（実 `BasicNavigator` ×bot, **lock 直列化**）+ rclpy ノード（200ms timer→ goal_result publish）+ `main()`（rclpy thread + uvicorn main、doc12a:200-219）。**runtime のみ**（unit 非対象＝AST スキャンでのみ検査）。**座標ゴールは backend `Pose=(x,y)` をそのまま通る（yaw 非対応のまま・`orientation.w=1.0`）**。**pip dep（`uvicorn`/`fastapi`）は lazy**＝`main()` 冒頭の `require_runtime_deps()` が rclpy.init() より先に走る（#283）。／ `preflight.py` — **純 / no rclpy / no fastapi / no uvicorn**（＝この行の下に別 bullet を足すと `CLAUDE.md:19-21` を参照する doc11c:260・rmf_adapter/CLAUDE.md:23 が行ズレするため同一 bullet に同居。#165）：pip runtime dep の起動時 preflight。`probe_runtime_modules(importer=…)`（importer 注入可）/ `missing_runtime_modules` / `format_missing_hint`（**環境非依存の純関数**＝`.env` 値を起動ログに出さない、rules/safety.md）/ `require_runtime_deps`→ 不足時 `SystemExit(hint)`（idiom は `warehouse_mcp_server/server.py:136-139`）。バージョン pin は **setup.py `install_requires` 単一ソース**（hint は pin を複製せず setup.py を指す）。
...
- pip: `fastapi` + `uvicorn`（lazy・setup.py:17 宣言。CI pytest は純 core のみ＝非依存）。**colcon/rosdep は pip を入れない**ので dev/sim image 側の provisioning が正本＝`deploy/dev/Dockerfile:41-48`（不在時は `preflight.py` が hint 付きで exit。#283）。ROS: `nav2_simple_commander` / `geometry_msgs` / `std_msgs`（package.xml exec_depend、runtime のみ）。
...
- `tests/unit/test_nav2_bridge_preflight.py`（#283）：pip dep 不在シミュレート（importer 注入）→ hint に module 名＋実行可能な install コマンド・present dep を巻き込まない・宣言順で全件列挙・`ImportError` 以外（壊れた install）は握り潰さない・hint は環境非依存（secret 不出）・**独立オラクル＝`setup.py` `install_requires` と被覆集合の一致**・`nav2_bridge.py` の **AST** で「module-level に uvicorn/fastapi を import しない」「`main()` は `rclpy.init()` と `import uvicorn` より前に preflight する」を pin。mutation 6/6 RED（preflight 呼び出し削除 / module-level import 復活 / 被覆集合縮小 / `except Exception` 化 / install コマンド削除 / preflight を rclpy.init 後ろへ移動）。
...
- **#283 残（preflight の射程外）**: ① **ROS dep は preflight で拾えない**（`rclpy` は `Nav2BridgeNode(Node)` の class 定義時に必要＝module-level import が `main()` より先に走る／`nav2_simple_commander` は rosdep 提供）。ヒットするのは「ROS あり・pip 未 provision」＝#283 が記録した dev/sim Docker の実障害パターン。ROS 不在ホストで entry point を起動した場合は従来どおり素の ImportError。② **同型の preflight を他パッケージへ共通化していない**（`warehouse_web_bridge` も fastapi/uvicorn を使うが**別パッケージ所有**＝`warehouse_interfaces` に置かない限り共有できず、契約拡張は contract-PR 案件。現状は各パッケージが自前で持つ意図的トレードオフ）。③ **hint の実挙動確認（実際に uvicorn を抜いた container 起動）は user-docker-gated**（host unit は文言・順序・被覆のみ検証）。
