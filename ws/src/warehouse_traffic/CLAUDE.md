...
- **Phase**: 0.5→3
- **ビルド**: ament_python
- **ノード**: `traffic_manager`（薄い診断ラッパ）, `virtual_scan`（相手ロボ注入, bot 毎に1）
...
## 提供 (produce)
- **lib**: `traffic_logic.TrafficManager`(ABC) + `NoTrafficManager`(Mode A) + `SimpleTrafficManager`(Mode B) + `make_traffic_manager(mode, nav2_bridge=None, route_planner=None)`（11a:14-145, 47-54）。LLM Bridge(#4) が `MANAGERS` レジストリで consume する想定。
...
- **node param**: `traffic_manager.traffic_mode`（none|simple|open-rmf）, `virtual_scan.{own_robot,other_robot}`。
