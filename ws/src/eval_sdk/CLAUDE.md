...
- **Tier1 additive（Phase 1.5b）= #432 で「audit 由来のみ」実装済**: `rate`/`jain_fairness_index`/`makespan`/`throughput` を stats に追加し、倉庫 `kpi.py` が介入率・command rejection 率・fairness・throughput/makespan を合成。**odom 由来 Tier-1（detour factor / jerk 合成 / 速度予算 / idle 率）は未実装**＝`DistanceAccumulator` が `_totals` と直前1点しか持たず pose/velocity 列が残らないため **新 producer が要る**（doc21:189「新 producer ゼロ」は audit 系にのみ成立＝doc21 §6 の producer 行 :186-187 は現コードとドリフト。doc 修正は別 PR）。**残**: live 結線(SR/SPL Nav2 goal-reached + planner `lᵢ`)は Phase 3a。
