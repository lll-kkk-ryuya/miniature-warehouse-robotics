...
- **テスト**: `tests/unit/test_eval_sdk_{seed,tracer,sink,stats,cost}.py`。langfuse 注入 fake / `NoopTracer` / `create_fn` 注入で **langfuse・ROS 非依存**に検証（host `python3.12`、host py3.7 不可）。Ruff(py312/line100/double-quote) + pytest 緑を維持。
