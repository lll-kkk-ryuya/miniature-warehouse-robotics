...
**Not live-verified end-to-end — see doc08:508,510.** The exact ``usage_details`` key shape and
...
(doc08:510), so a cost derived from it is not "verified". Every test injects its own table so
...
# literal model strings Hermes forwards (doc08:508) before trusting any derived cost. xAI
...
GROK_PRICE_TABLE_2026_06_04: dict[str, GrokPrice] = {
    # grok-4 family. PLACEHOLDER prices pending live confirmation (doc08:508).
    "grok-4": GrokPrice(
        input_usd_per_token=3.0 / _USD_PER_MILLION,
        output_usd_per_token=15.0 / _USD_PER_MILLION,
    ),
    # grok-3 family. PLACEHOLDER prices pending live confirmation (doc08:508).
    "grok-3": GrokPrice(
        input_usd_per_token=3.0 / _USD_PER_MILLION,
        output_usd_per_token=15.0 / _USD_PER_MILLION,
    ),
...
# price-field form remain unconfirmed (doc08:510), and ``grok-* cost_details.total > 0`` must be
# asserted on a real Langfuse 4.9.x trace in Phase 3 (doc08:508 / doc13:520②). Do NOT treat a cost
...
# price (doc08:510 "don't fix by guessing"). The 2026_06_04 table retains those families if a caller
...
def resolve_grok_price(
    ...
    *,
    ...
    """Resolve a ``model`` string to its price row, else ``default`` (doc08:504 prefix match).
