...
- ``LangfuseTranscriptTracer`` — **SKELETON ONLY (雛形)**. It defines the seam where the audio /
  ...
  yet (design only). Because the audio leg routes DIRECT to ER, by design bypassing Hermes (Hermes
  /v1/chat/completions rejects ``input_audio`` content parts with HTTP 400 ``unsupported_content_type``
  — PROBE-2, measured 2026-06-27; audio goes direct to ER; docs/mode-x-er/06-unfrozen-contract-resolutions.md §5),
  the Hermes built-in Langfuse plugin cannot observe it — this Bridge-side tracer is the uniform owner
  for that leg when implemented (doc06 §5). Live wiring is gated behind the HLF spike
  (productization/02:177-199, doc08 §Langfuse).
"""
