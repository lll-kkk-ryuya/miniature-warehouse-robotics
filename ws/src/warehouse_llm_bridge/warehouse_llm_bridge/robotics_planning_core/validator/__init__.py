...
- :class:`PlanningContext` / :class:`RuntimeSafetyState` / :class:`RuntimeStateSource` /
  :class:`InMemoryRuntimeStateSource` — per-cycle policy + runtime state (brief step 7). The
  source IF is ``RuntimeStateSource`` (NOT ``StateStore``) to avoid shadowing the frozen
