...
- #126 progress_checker 委譲（明文化）: nav 実行中の進捗喪失（`FAILED_TO_MAKE_PROGRESS`）は Nav2 `progress_checker`（`nav2_params.yaml:94-97`, 所有=nav-traffic doc16:187）の責務。Guardian の `BlockTracker`（変位ベース blocked→low-harm recovery）は **nav_status 非依存の独立フォールバック**（doc12:254 別系統）として維持。idle 誤検出の解消は nav_status ゲート＝Phase-2（本スライスで `nav2_params.yaml` は無編集）。
...
- # ✅(#126) /emergency/event の edge-trigger 化（`gl.EdgeLatch`: 立ち上がり `(bot, reason)` のみ発行、解消→再発で再発火）。物理停止 cmd_vel/Nav2 cancel は毎 tick 維持＝level。State Cache 側 active/history 50件 ring は維持。残: 近接の collision_monitor 委譲（cmd_vel 挿入トポロジ docs 未定義＝先に docs PR, #126）／blocked の progress_checker 完全委譲（nav_status feed＝Phase-2）。**freshness（pose 途絶）と progress_checker 委譲の明文化は ✅#126 実装済（上記）**
