...
- config: `safety.emergency_min_distance`（既存・2台間距離。速度cap とは別概念）, `safety.blocked_timeout`（← **このトラックで `config/warehouse.base.yaml` に追加: 10.0**）, `safety.battery_percentage_scale`（← **#44 で追加: `percent` 既定=fail-safe**）, `safety.pose_freshness_timeout`（← **#126 で追加: 1.0s 暫定**。amcl_pose 鮮度窓。`config/warehouse.base.yaml` 所有=bringup/skeleton へ PR で予告）, **変位ゲート4キー（← doc23 A-5③ で追加。同じく `config/warehouse.base.yaml` 所有=bringup/skeleton へ PR で予告）**: `safety.pose_freshness_motion_epsilon`（0.10 m ＝ 2 × `nav2_params.yaml:58` `update_min_d`）, `safety.pose_freshness_angular_epsilon`（0.4 rad ＝ 2 × `:57` `update_min_a`）, `safety.pose_freshness_speed_epsilon`（0.02 m/s）, `safety.odom_freshness_timeout`（0.5s ＝ これを超えた odom は不明扱い→ゲート fail-closed）。いずれも Phase-2 実測で確定する暫定値。
...
- doc23 A-5③ 変位ゲート（OQ-11 第1実装スライス）: `pose_stale` を **`pose_age > pose_freshness_timeout` ∧ ゲート開** に限定。ゲートは wheel odom（`/{bot}/odom`＝AMCL と独立）で「前回 pose 到着以降の累積変位 > 0.10m ／ 累積 `|Δyaw|` > 0.4rad ／ 速度 > 0.02m/s」のいずれかで開く（`gl.pose_gate_open` / `gl.PoseGateTracker` / `gl.yaw_from_quaternion`・すべて rclpy 非依存）。**AMCL は motion-gated ゆえ駐機中の沈黙が正常**で、CURRENT はこれを誤 estop していた（自己ラッチ）。**odom 不明/stale(`odom_freshness_timeout` 超)/非有限・epsilon 未設定は fail-closed でゲート開＝CURRENT へ縮退**（ゲートは estop を抑止しかできず、足せない）。変位は**経路長の累積**＝単調非減少でラッチ内蔵（速度のみのゲートが作る fail-open limit cycle を排除）。R-26: `tests/unit/test_guardian_displacement_gate.py`（非緩和性＝走行中の発火 tick が CURRENT と一致・駐機・クリープ・fail-closed・ラッチ・厳密 `>` 境界。独立オラクル = upstream `nav2_amcl` の publish 則）。本スライスで `WAREHOUSE__SAFETY__POSE_FRESHNESS_TIMEOUT=999` 回避策を 5ファイル6箇所すべて削除。**残: 既定 1.0s のまま sim full-stack 完走は human gate（未実施）**。正本: doc23【2026-08-10 追補】A-5③/A-10 ＋ doc12 末尾【2026-08-17 追補】。
