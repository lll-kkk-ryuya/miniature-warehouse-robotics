...
- #126 progress_checker 委譲（明文化）: nav 実行中の進捗喪失（`FAILED_TO_MAKE_PROGRESS`）は Nav2 `progress_checker`（`nav2_params.yaml:94-97`, 所有=nav-traffic doc16:189）の責務。Guardian の `BlockTracker`（変位ベース blocked→low-harm recovery）は **nav_status 非依存の独立フォールバック**（doc12:254 別系統）として維持。idle 誤検出の解消は nav_status ゲート＝Phase-2（本スライスで `nav2_params.yaml` は無編集）。
