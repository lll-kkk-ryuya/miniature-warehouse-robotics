...
- `# TODO(docs)` **正本 doc が未 land**: mode-outdoor/07 は `docs/mode-outdoor-skeleton` ブランチにあり、本 worktree の `docs/` には無い。**行 pin（`file:line`）を張れるのは land 後**なので、本節は §番号（§9 案 A / §3 (d) / §1 / §10）で参照している。land 後に file:line へ差し替えること。
