...
- 契約（offline コアが実 import）: `warehouse_interfaces.locations.is_known_location` / `KNOWN_LOCATIONS`
  ...
  **座標 {x,y} は凍結ではない**＝`load_config` 経由で `config/warehouse.base.yaml:47-56` から解決し、同 `:34` の
...
- **GATE 後（R-26）**: EasyFullControl 登録・action client 実体化を含む安全機構（停止・速度・stale）はユニット必須。
  2 台 Open-RMF E2E / Jetson メモリ実測（cgroup 計測）は tiryoh/実機（docs/architecture/16-repository-and-conventions.md:219-223 のテスト戦略 §11）。
