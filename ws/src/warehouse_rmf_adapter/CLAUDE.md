...
- **GATE 後（R-26）**: EasyFullControl 登録・action client 実体化を含む安全機構（停止・速度・stale）はユニット必須。
  2 台 Open-RMF E2E / Jetson メモリ実測（cgroup 計測）は tiryoh/実機（docs/architecture/16-repository-and-conventions.md:216-220 のテスト戦略 §11）。
...
- **governance（本レーン編集境界外＝orchestrator 調整・tracked: #221）**:
  ...
  - **doc16 3 箇所 = 登録済**（#221 PR で §1/§2/§9 追加 + 行挿入に伴う inbound file:line 参照を同 PR で re-pin）:
    ① §1 ディレクトリツリー（docs/architecture/16-repository-and-conventions.md:24-53・`warehouse_orchestrator/` の隣）
