...
ruff + `ruff format --check` + pytest **45 pass / 1 skip**〔skip = SDK 導入時のみ走る実 `create_trace_id`
決定性〕）。SDK は導入済（`langfuse 4.7.1` / `openai 2.41.1`）・`config/dev/.env` の Langfuse 3 鍵
...
2. **GCP prod Hermes 経由**（要・明示 prod 承認）: `verify.py --allow-remote --base-url <gcp>` で 4-provider
   ...
   比較ランに prod を混ぜるバイアス注意（doc13:526）。
