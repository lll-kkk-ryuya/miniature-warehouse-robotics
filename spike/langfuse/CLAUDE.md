...
- `spike/langfuse/RESULT.md` — ①〜⑤ + Grok cost + **v4 score metadata group-by 可否**（doc08:518 / doc20 §8.4 item2）の結果欄。**未計測 scaffold＝human run 後に転記**。
...
- net（live のみ）: Hermes Gateway `POST /v1/chat/completions`（`model="hermes-agent"`・Bridge-owned `langfuse.openai` 経由 doc13:517）+ Langfuse trace read-back（fetch API のフィールド形は**未確定** doc08:508 → 防御パース）。
...
- **read-back フィールド形が未確定**（doc08:508 / doc20 §8.4 item2）: `fetch_trace` は候補キーを**防御パース**し raw を記録、`fetch_ok=False` 時は①②③④を UNVERIFIED と loud 表示（推測で pass させない）。最終確認は Langfuse UI ＋ human。
