...
- **productionization hardening（#356 DoD・`api_server.py` `_normalize_multimodal_content`
  ...
  - **format allowlist**: `_AUDIO_FORMAT_ALLOWLIST = {"wav","mp3","m4a","aac","ogg","flac"}`
    を新設し、未知 `format` を reject（従来は任意 `format` が `audio/<fmt>` に素通しされ provider
    4xx で初めて弾かれた）。**この allowlist は保守的な暫定値であり、Gemini audio の対応
    mime-type docs に対して確定していない（verified とは主張しない）**。過不足は
    remaining[]（confirm-value）扱い。
