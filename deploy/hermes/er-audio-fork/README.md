...
> **標準 / fallback の集約（既存 `deploy/dev/run-er-hermes.sh` との関係）**: **`run-er-gateway.sh`（port `8644`・fork 適用・text+image+`input_audio` の全 modality）が SHIPPED STANDARD ER gateway**。`run-er-hermes.sh`（port `8643`・非 fork・text/image のみ）は **deprecated / fork-free fallback**（audio 不要時のみ）に格下げ済み（同 launcher の EOF banner 参照）。両者は **同じ lean home `~/.hermes-mwr-er-lean`** を使い、`.env` の `API_SERVER_PORT` が port を最終決定する（`run-er-gateway.sh` 既定 `8644` / `run-er-hermes.sh` 既定 `8643`）。**両者を同時起動しない**（同一 home の bind 競合）。Bridge は標準では `8644` に繋ぐ。HOME override 変数も非対称（`run-er-hermes.sh` は別変数）なので、env を渡すときは各 launcher の USAGE ヘッダのポート/HOME 変数名を確認する。
...
- **transport の default 切替の着地状況（honest）**: audio=Hermes を shipped default にする productionize は
  依然 **未 ship**（上記「現状（honest status）」§未出荷）。adapter からの live send（frozen Command 形へ）を
  含む transport の productionization は **#344/#389 で着地予定（PR pending・main 未マージ）**であり、
  それまで **音声 transport の default は `direct`**（恒久 fallback）。
