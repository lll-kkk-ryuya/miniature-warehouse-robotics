...
> 2026-09-12 新設（docs 先行・**骨格のみ**）。オペレーター指示で、室内のジェスチャ召喚・standby HRI（[ADR-0009 Decision 2](adr/0009-m1-room-scale-operation.md:21)）を第一優先から外し、**屋外の歩道を GNSS 主体で A→B 自律走行する構成を第一優先**にする。ADR-0014 は[番号予約のみ](adr/README.md)（裁定待ち 6 点 = [00 §4](mode-outdoor/00-mission-and-scope.md)）。本節の位置が mode 群の並びから外れているのは末尾追記原則（#165 行ズレ回避）による。
...
| [00-mission-and-scope](mode-outdoor/00-mission-and-scope.md) | ミッション（歩道 A→B・GNSS 主体・完全自律）・スコープ IN/OUT・法的 2 段階（随伴通行 → 遠隔操作通行）・**オペレーター裁定待ち 6 点**・流用する既存資産の仕分け |
