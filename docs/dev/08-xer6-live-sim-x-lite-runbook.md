...
| leg | 何を証明するか | 状態 | gate |
|---|---|---|---|
| **A. plan → frozen `Command`（offline）** | `compile_raw_output` が accept 済 plan を frozen `warehouse_interfaces.schemas.Command` にする（`pipeline.py:89-150`） | **DONE・main(208eb76) マージ済（#381）**。unit `tests/unit/test_l3_pipeline.py:197-317` | autonomous（network 無し） |
...
- **XER6 の産物 = frozen `Command`**（`compile_raw_output`、`pipeline.py:97,102-103`）。この関数は「**NO actuation happens here — the ``Command`` is handed to the downstream Bridge -> MCP -> Policy Gate path**」と明記する（`pipeline.py:102-103`）。つまり XER6 は **actuation しない**。動かすのは既存 L2。
...
- **X-lite に固定**: `compile_raw_output` の `profile` 既定は `ExecutionProfile.X_LITE`（`pipeline.py:96`）。`x_rmf` は `WarehouseNavCompiler` が `NotImplementedError`（`pipeline.py:126-127`、`docs/mode-x-er/01-architecture-and-flow.md:199-206`）。RMF 経路（Open-RMF Task API）は本 runbook の対象外。
...
`compile_raw_output` に ER の raw envelope（direct=Gemini `generateContent` / hermes=OpenAI 互換、どちらでも同一 `Command`）を渡すと frozen `Command` が返る（`pipeline.py:89-150`）。canonical fixture は **赤箱/青箱**（`docs/mode-x-er/01-architecture-and-flow.md:134-151`）:
...
Visual Resolver が pixel を KNOWN_LOCATION に snap する（red_box→`shelf_1`、blue_box→`shelf_2`、`tests/unit/test_l3_chain.py:12-20,60-73`）。site 依存の homography / location 座標 / snap 閾値は **caller が注入**し、code 定数に埋めない（`pipeline.py:115-121`、`docs/mode-x-er/01-architecture-and-flow.md` の未凍結注記 `:5`）。
...
- accept 経路: **1台目だけが ready**（one-shot）。`compile_raw_output` は `bot1 → NAVIGATE → shelf_1` を出し、`t2`(bot2) は `after t1` ゲートで **この cycle には出さない**（`test_l3_pipeline.py:197-208,242-249`、`pipeline.py:109-111`）。
- `destination` は必ず `KNOWN_LOCATIONS` の要素（`test_l3_pipeline.py:208`、`schemas.py:157-162`）。座標 goal / velocity / model 昇格は **やらない**（`pipeline.py:22-24`）。
- **R-26 0-dispatch を end-to-end で固定**: 非 accept な `ValidationReport`（unknown robot / emergency / needs_clarification）は **空 `Command`** を返し、resolve/compile に一切進まない（`test_l3_pipeline.py:252-298`、`pipeline.py:141-146`）。forbidden envelope は Handoff で `ValueError`（`test_l3_pipeline.py:301-307`、`pipeline.py:134`）。

> **順序付き移動（赤→青）の実現**: `compile_raw_output` は「今 ready なタスクだけ」をコンパイルする one-shot（`pipeline.py:109-111`）。bot1 が `shelf_1` 到達＝`t1.completed` 後に **次 cycle で再度**呼び、`t2`(bot2→`shelf_2`) を出す。cycle 間の running/completed 進行は caller のループ（live 経路）の責務で、この offline entry は状態を持たない（`pipeline.py:110-111`）。デモでは「bot1 到達を確認 → 2回目の `Command` を投入」を人手で行う（§4 手順）。
...
3. **cycle 2（bot2 → 青箱=`shelf_2`）**: 同じ raw plan を再度 `compile_raw_output` に流す。`t1` 完了を caller ループが state に反映した上で呼べば `t2`(bot2→`shelf_2`) が ready になる（`pipeline.py:109-111`）。得た `Command` を同経路（action_map → … → `/api/v1/navigate`）で投入。RViz で bot2 が `shelf_2` へ向かうのを確認。

> **順序保証はどこにあるか**: task_graph の `after t1.completed`（`docs/mode-x-er/01-architecture-and-flow.md:147`）を **Task Graph Executor**（XER4）が保持し、ready task だけを compile する（`pipeline.py:11,109-111`）。「bot1 が着く前に bot2 が動く」ことは L3 が構造的に防ぐ——L2/Nav2 側で順序を作らない。
...
| 層 | 機構 | GO 条件 | 根拠 |
|---|---|---|---|
| **L3** | 0-dispatch（R-26） | 非 accept plan は空 `Command`＝dispatch しない | `pipeline.py:141-146`、`test_l3_pipeline.py:252-298` |
...
- L3 → frozen `Command`: `ws/src/warehouse_llm_bridge/warehouse_llm_bridge/robotics_planning_core/pipeline.py:89-150`（`compile_raw_output`・main #381）/ unit `tests/unit/test_l3_pipeline.py:197-317`（compile）/ `tests/unit/test_l3_chain.py:12-20`（red/blue fixture）
