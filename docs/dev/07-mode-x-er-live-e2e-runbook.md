...
4. **unforked Hermes は音声を運べない**。素の `/v1/chat/completions` は `input_audio` を **HTTP 400 `unsupported_content_type`** で弾く（2026-06-27 実測、`docs/mode-x-er/06-unfrozen-contract-resolutions.md:159`）。**fork gateway（8644）はこの 400 を解消済で、本 runbook の標準経路として既に存在する**（§Step A = 本書 `:44` / `:51`・[ADR-0002](../adr/0002-er-in-hermes-standard.md)）＝残っているのは **default 配備**だけ（`:189` の残件 (b)）。それが済むまで **CURRENT〔shipped〕の音声既定は direct ER**（`docs/mode-x-er/06-unfrozen-contract-resolutions.md:263-270`）。
