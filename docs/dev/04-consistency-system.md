...
> **1つの checker を 3箇所から呼ぶ**（DRY）。人間の commit 時（pre-commit）・**全 push / PR 時**（CI、**Claude 非依存の唯一の durable 層**）・Claude の編集直後（PostToolUse hook で自己修正）すべてで効く。

```
                   ┌─ pre-commit            （人間のローカル commit 時）
scripts/           ├─ CI ci.yml: consistency （全 push/PR・durable／最終保険）
