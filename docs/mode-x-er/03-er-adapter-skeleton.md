...
- **消費側との整合**: L3 Visual Resolver は `Detection.pixel` を **calibration artifact の pixel 空間**で消費する（[02 2026-09-16 追補](02-l3-planning-core.md)）。ER の正規化値 → artifact 空間（raw px）の変換は **L4→L3 handoff の別 slice**（#699 DoD 最終項・frame の W×H が要る）。着地まで live ER の pixel は unresolved → 0-dispatch が設計どおり（fail-closed）。
