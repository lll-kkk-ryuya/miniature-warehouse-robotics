...
- **audio は input 一覧に明記される**が、公式 docs の robotics 例は画像・動画中心で、**音声指示を直接食わせる robotics 用途の作例は提示されていない**。よって「ER に audio を直接渡してタスク理解させる」用法は **API capability としては可能だが、robotics command としての実証は未確認**として扱う。実装前に offline probe（`docs/dev/vla-access-and-runtime-spike.md` の Gemini direct probe と同方式）で確認する。
- 既存 spike で text / image probe は `HTTP 200` 実測済み（`docs/dev/vla-access-and-runtime-spike.md:23-28`）。audio probe は未実施。
...
- ER への audio 直入力の robotics 用途実証（offline probe。spike 方式で `HTTP 200` と応答品質を確認）。実証まで STT 併走を fallback として残す。
