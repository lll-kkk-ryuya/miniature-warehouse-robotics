...
- ~~ER への audio 直入力の **疎通＋parseable plan 生成**~~ **✅ 実証（2026-06-26・PROBE-1）**: HTTP 200・音声理解＋plan 返却を実測（`06` §5）。STT は fallback / out-of-band（provenance）として残すが、ER 経路の必須上流ではない。**未確認（別 eval）**: ER audio の **robotics-grade command 品質**（連結誤認識耐性・現場性能）は本 probe の対象外で、別途 eval が要る。
- Fusion の activation 条件（例: 参照 model 数 ≥ 2 で有効化、ER 単体では pass-through）を実装時に config 化するか、box を物理的に省くかの判断。
- 低 confidence のしきい値・operator clarification policy は現場依存で、既存 tool には含まれない自作領域（`docs/productization/05-decision-observability-and-tooling.md:176`）。
