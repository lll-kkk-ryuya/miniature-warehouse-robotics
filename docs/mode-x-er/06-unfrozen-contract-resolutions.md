...
- **live 送信 seam**（`gemini_er.propose_plan` の live path）は **#344 = probe CLOSED / live-send は #389 が pending（main 未マージ）**。運用手順 = operator runbook [`../dev/07-mode-x-er-live-e2e-runbook.md`](../dev/07-mode-x-er-live-e2e-runbook.md)。
...
- **overclaim しない（CURRENT 不変）**: wire（live-send #389・main 未マージ）と fork ship が着地するまで **CURRENT（shipped）= audio は direct のまま**（:269 と一致）。本補遺は「向かう標準方針」であって現稼働ではない。audio format は当面 **wav 優先**（fork の map 先＝`audio/wav`）で後拡張。fork メンテ負債は当面受容・upstream PR は後で。
