...
- U-9 **`gemini_er.py` の `_SCHEMA` に pixel 正規化（`[u,v]` 0–1000）の節が無い**（live helper `_er_live_client.py` には有る）。live `command_items=0`（[07:37](07-implementation-status.md:37)）の候補原因＝本追補の射程外・別 Issue。
