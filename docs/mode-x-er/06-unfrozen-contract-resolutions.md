...
| 7 | OperatorNotice schema と `/operator/notice` topic（型/QoS） | **DEFER(gate)**（方式は案A 確定済） | XER-OF1 offline fixture で payload 実証後 | `std_msgs/String`(JSON)・`operator_notice.v0`・RELIABLE/KEEP_LAST depth=20(暫定)/VOLATILE |

**全体結論**: いま `warehouse_interfaces` / config / topic に手を入れて「凍結」すべき項目は**実質ゼロ**。#4 の MVP 方針（known location 限定）と #7 の方式（案A 専用 topic）は docs 上**確定済み**だが、コード凍結は依然として別 `contract`/`docs` PR とフェーズ通過を要する。
...
## 7. OperatorNotice schema ＋ /operator/notice topic — DEFER(gate)（方式は案A 確定済）
...
**判定 / 根拠**: **DEFER(gate=XER-OF1 + offline fixture)**。`05`:5 が凍結条件を「offline fixture と契約 PR で確定」と明示し、現状は実装ゼロ proposal（XER-OF0 のみ）で `05` §8.8(:369-376)に topic 名最終確定・depth 値・MVP publisher・emergency 二重化・schema_version の5未決が残る。今 FREEZE すると未検証の `depth=20`（:341「暫定・実装時調整」）を凍結契約に焼く。**ただし方式（案A 専用 topic）・型（std_msgs/String JSON）・QoS 種別・reject 級 v0 は確定済**＝DEFER は方式の再議論でなく**未決値の充填と fixture 実証待ち**。
