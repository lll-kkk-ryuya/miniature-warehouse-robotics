...
> §5 の PROBE-2 結論行（「Hermes は音声を運べない＝音声 direct 固定」）は **UNFORKED Hermes の実測**として正しく、他 doc が参照するので**書き換えない**。本補遺はそれを additive に精緻化する（§5 末尾の行参照を動かさないため末尾に置く・#165）。
