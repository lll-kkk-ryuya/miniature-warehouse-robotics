...
| 3 | Mode X-ER config key（x_lite/x_rmf・calibration_id）＋ calibration artifact 配置/形式 | **RESOLVED（2026-07-07 凍結）** | ゲート到来（XER3-5 offline core land 済・初回 consumer=XER6 #342） | 凍結形＝[08-x-er-bridge-node-spec §3](08-x-er-bridge-node-spec.md)（`mode_x_er:` ブロック・traffic_mode と直交・`config/<env>/calibration/<id>.yaml`）。経緯は本書末尾「追補（2026-07-07）」 |
...
| 5 | Hermes 経由で ER の audio / image API を扱えるか | **NEEDS-PROBE → RESOLVED**（2026-06-26〜27・PROBE-1/2/3 実測済） | （解錠済）live probe 3本（PROBE-1〜3）実測完了 → §5 実測結果 | 音声=**direct ER 確定**（unforked Hermes は `input_audio` 400・透過不可／fork carve-out → §補遺）/ text+image=**lean Hermes gateway** or direct（image_url passthrough・観測一元化） |
...
| 7 | OperatorNotice schema と `/operator/notice` topic（型/QoS） | **RESOLVED（#446・依存トラック合意待ち）**（案A 採用・§8.8 の未決 5+1 点を doc05 §8.10 で確定） | ゲート到来（XER-OF1 #367 で payload 実証済 → #446 で凍結） | doc05 §8.10 の**確定値** = `std_msgs/String`(JSON)・`operator_notice.v0`・RELIABLE/KEEP_LAST depth=10/VOLATILE・emergency は `/emergency/event` 相乗り（二重 publish しない）・v0=reject 級のみ。詳細は §7 |

**全体結論**: いま `warehouse_interfaces` に手を入れて「凍結」すべき項目は**実質ゼロ**。#4 の MVP 方針（known location 限定）は docs 上**確定済み**、#7 は **#446 で確定**（topic 名・型・depth=10・schema_version・emergency 扱い＝doc05 §8.10。凍結成立は依存トラック合意後）。残る項目のコード凍結は依然として別 `contract`/`docs` PR とフェーズ通過を要する。
...
- **ValidationReport**（出所 `02`:60-66 ＋ `02`:95、語彙確定は `02`「ValidationReport 語彙確定」節）: `status` ∈ `accepted`(`02`:61) / `rejected` / `needs_clarification`(`02`:79,84) / `emergency_stop`（reject 系 = `rejected`/`emergency_stop`・decision 固定語彙 `productization/05`:69 へ接地・確定） / `errors: list[RuleResult]` / `warnings: list[RuleResult]` / `normalized_plan: dict`。RuleResult={`code, severity, field_path, message_for_operator, debug_detail, dispatch_effect`}(`02`:95、`severity`∈error/warning・`dispatch_effect`∈block/needs_clarification/emergency_stop/none)。stable `code`(`02`:96・全8) ＋ clarification origin `OPERATOR_CLARIFICATION_REQUESTED`(`03`:71)＝全9（clarification は code を増やさず `dispatch_effect=needs_clarification` で表す・`02`「ValidationReport 語彙確定」節）。
...
- **確定（`02`「ValidationReport 語彙確定」節・#338 前提）**: `status`∈`accepted`(`02`:61)/`rejected`/`needs_clarification`/`emergency_stop`（reject 系 3 値の同一 Mode X-ER 文脈 literal は `README`:67、`productization/05`:69 は別 box `decision` field の例示「…など」）、`severity`∈`error`/`warning`（`02`:62-63 の errors[]/warnings[] へ接地）、stable `code`＝docs-literal 8 種（`02`:96）＋ ER field 由来の派生 code `OPERATOR_CLARIFICATION_REQUESTED`（`03`:71 の bool field `operator_clarification_required` 由来・UPPER_SNAKE code literal は docs に無い）、`dispatch_effect`∈`block`/`needs_clarification`/`emergency_stop`/`none`（**値集合に docs literal 無し**の内部派生ラベル＝`needs_clarification`/`emergency_stop` は status 同綴り・`block`→`rejected`・`none`→非ブロッキング `warnings[]`）、Detection/TaskNode は独立 model。status/severity/8 code を既存確定語彙へ接地しつつ `dispatch_effect` 値集合・派生 code は内部派生ゆえ凍結契約を新規に発明しない（`warehouse_interfaces` 昇格自体は本 §冒頭どおり XER1→XER2 後に DEFER）。
...
## 3. Mode X-ER config key ＋ calibration artifact — DEFER(gate) → RESOLVED（2026-07-07・末尾追補参照）
...
**判定 / 根拠**: **DEFER → RESOLVED（2026-07-07・末尾追補で凍結・[08 §3](08-x-er-bridge-node-spec.md) 正本）**。以下は DEFER 当時の根拠（凍結前の記録）: profile/artifact は「方向性」確定だが「凍結形」が docs 未確定。① config 表現（traffic_mode と同列 top-level か `x_er` ネストか）が docs 未記載＝発明領域。② calibration の file 形式（YAML/JSON）・配置パス・version 命名が未確定。③ `calib-YYYYMMDD` は「例示（illustrative）」。実装が無い現状で base.yaml を先に汚すと、profile 選択配線（launch/bridge 両所読み）と calibration ローダ設計が固まる前に key を確定してしまう。XER0/XER1 で request shape が固まり XER3 で Visual Resolver が calibration を実消費する段で凍結するのが docs-first ゲート（`README`:85-92）。なお `load_config`（[`config.py`](../../ws/src/warehouse_interfaces/warehouse_interfaces/config.py):114-131）は未知 top-level key を deep-merge で素通し、schema 制約は `safety.*` のみ（config.py:69-111）＝追加は非破壊で、凍結を急ぐ必然性なし。
...
- 凍結方針（probe 後・additive）: transport を `direct`(audio/image 一次) + `hermes`(image・OpenAI 互換 image_url) として固める（audio の fork carve-out → §補遺）。content part 形（`input_audio`/`inline_data`）は外部 API 仕様で当方の発明ではない。
...
ユーザー承認のもと live 実測（`~/.hermes/.env` の `GEMINI_API_KEY` を値非表示で使用）。PROBE-1/3 は 2026-06-26、PROBE-2 は 2026-06-27 に実測（全 3 本が実測済）。harness = `tests/live/test_er_handoff_live.py`（#351 で着地済）。
...
## 7. OperatorNotice schema ＋ /operator/notice topic — RESOLVED（#446・案A/depth=10 確定・依存トラック合意待ち）
...
- (2) box の**入力**を運ぶ別ノード event チャネル＝専用 topic `/operator/notice`（**案A・`05`:194 で確定**）。`05` §8 に doc03 契約ドラフト一式（型 `std_msgs/String`(JSON)・payload `operator_notice.v0` `05`:312-334・QoS RELIABLE/KEEP_LAST **depth=10**（`05` §8.10 item2 で確定・当初暫定20）/VOLATILE `05`:336-345・pub/sub）。`05`:292「doc03 への行追加は別 contract PR（owner=skeleton/governance）」。
- [`doc03`](../architecture/03-software-architecture.md):116「doc03 は topic 名・型・一行責務のみ」。grep（凍結前）: `/operator/notice` は ws/src・config・doc03 に**未存在＝完全 net-new**（#446 で doc03:111 へ additive 追加済）。`decision` 固定語彙（[`productization/05`](../productization/05-decision-observability-and-tooling.md):69）は `accepted/rejected/warning/needs_clarification/emergency_stop` のみで **milestone(arrived/completed) を含まない**。

**判定 / 根拠（当初 DEFER → #446 で RESOLVED）**: `05`:5 は凍結条件を「offline fixture と契約 PR で確定」と規定。**XER-OF1 offline fixture（#367）＋ publish-only 配線（`operator_feedback/publisher.py`）が完了し、`05` §8.8(:369-376) の未決 5+1 点は `05` §8.10（確定値表）で確定**した: topic=`/operator/notice`（item1）・**depth=10**（item2・暫定20→repo 慣習・実証済）・MVP publisher=nav2_bridge/mcp_server（item3）・**emergency は既存 `/emergency/event` 相乗り＝`/operator/notice` へ二重 publish しない**（item4）・`schema_version="operator_notice.v0"`（item5）・v0=reject 級のみ（item6）。方式（案A 専用 topic）は `05`:194 で既決＝#446 は方式の再議論でなく**未決値の充填と fixture 実証を完了**したもの。**残ゲートは依存トラック（safety-state/nav-traffic/wo/web）合意のみ**（§8.9）。確定値の正本は `05` §8.10。
...
- **QoS**（`05`:336-345 / §8.10 item2）: RELIABLE（lossless）/ KEEP_LAST **depth=10**（`05` §8.10 で確定・当初暫定20→repo 慣習）/ VOLATILE（再起動跨ぎの stale 再生防止）/ AUTOMATIC。
...
- `(旧・要確定 → #446 で `05` §8.10 で確定)`: topic 名（`/operator/notice`・item1）/ depth（**10**・item2）/ MVP publisher（nav2_bridge+mcp_server・item3）/ emergency（既存 `/emergency/event` 相乗り＝二重 publish せず・item4）/ `schema_version`（`operator_notice.v0`・item5）を確定。**milestone(arrived/completed) は現 `decision` 固定語彙に無い**ため v0 では運べず、喋るなら別 event 語彙/別チャネルが要る（`05`:376・item6）＝v0 は reject 級に scope。
...
**contract PR 手順**: 2段の別 PR。**[本体＝本 PR #446]** doc03 トピック契約 PR（`contract` ラベル・track:skeleton/governance 所有・`05`:292）: doc03「Jetson 内部」表（doc03:90-114）に1行追加済（payload/QoS/pub-sub 正本は `05` §8 へリンク）→ **マージ前に依存トラック safety-state/nav-traffic/wo/web へ予告し合意**（publisher 候補=mcp_server/traffic/rmf_adapter/nav2_bridge/safety/firmware、subscriber=llm_bridge feedback sub-box。**合意は本 PR の残ゲート**）→ OperatorNotice(出力) と本 topic(入力) の別物性を本文明記。**XER-OF1 offline fixture（#367）で payload 実証済**。**[別 PR]** `OperatorNotice`(box 出力 schema) の `warehouse_interfaces` 昇格は `05`:279「まだ追加しない」＝別 PR・別タイミング。taxonomy 正本登録（`productization/01` の box 一覧）は box-map owner 調整の別 PR。
...
| 7 | XER-OF1 後 | `contract`（doc03 topic）→ **RESOLVED（#446・合意待ち）** | skeleton/governance・safety-state/nav-traffic/wo/web |
...
- 監査指摘の反映: ① `accepted` の正確位置は `02`:61（`02`:60 は開き `{`）に修正。② ValidationReport の reject 系 status は当初「要確定」だったが、`02`「ValidationReport 語彙確定」節で decision 固定語彙（`productization/05`:69）へ接地し `rejected`/`emergency_stop` に**確定**（§1 の line 50/53 も更新済）。
...
## 2026-06-27 補遺 — `input_audio` fork demonstration（§5 の精緻化・末尾追記＝行参照非破壊）

> §5 の PROBE-2 結論行（「Hermes は音声を運べない＝音声 direct 固定」）は **UNFORKED Hermes の実測**として正しく、他 doc が参照するので**書き換えない**。本補遺はそれを additive に精緻化する（§5 末尾の行参照を動かさないため末尾に置く・#165）。本補遺以下で使う `F1–F6` の記号定義は [`productization/02`](../productization/02-l4-robotics-bridge-box.md) 末尾補足「Findings 凡例」が正本＝[`jetson/01`](../jetson/01-fidelity-and-validation.md):52-57 の fidelity tier `F1–F6` とは別体系。

- **fork（2 ファイル・hermes-agent v0.15.1）**: `gateway/platforms/api_server.py`（`_normalize_multimodal_content` が `input_audio` 受理・新 `_AUDIO_PART_TYPES`・`_content_has_visible_payload` が audio を visible payload に算入）/ `agent/gemini_native_adapter.py`（`_extract_multimodal_parts` が `input_audio → Gemini inlineData{mimeType:"audio/wav"}`）。
- **LIVE RESULT（n=4・single short English audio + simple task のみ）**: `/v1/chat/completions` に `input_audio` POST → **HTTP 200**・**ER が native audio 理解**（音声にのみ存在する語の transcript を返却＝STT 経由でなく native）。latency 中央値 = lean Hermes **3.69s** vs direct **4.24s**（n=4・comparable・ER-thinking 交絡あり）・+~408 prompt tok/call。
- **判定の精緻化（overclaim しない）**: UNFORKED Hermes = 400（PROBE-2 のまま不変）／ fork ありなら audio は Hermes に乗る（= `hermes` を音声の **default target** にできる・F1 を音声へ延長）／ **CURRENT（shipped reality）= 音声は direct のまま**（fork productionize まで）・**direct は恒久 fallback**。**「音声は今 Hermes default」とは書かない**。
- **残コスト（productionize 前・新 gate は起こさず既存方針内で追跡）**: fork メンテ負債（upstream 追従）／ **provider routing（F5）**=単一 server-side active model ゆえ 4-provider 比較は per-provider gateway／ **Langfuse（F6・Opt C）**=当面 Bridge 所有 trace を主、Hermes plugin 所有は **HLF-G0〜G5 gate 後**（[`productization/02`](../productization/02-l4-robotics-bridge-box.md):177-199 / [`../architecture/13-hermes-setup.md`](../architecture/13-hermes-setup.md):551-570）、flip しても outcome score は eval_sdk 所有／ ER thinking-config 制御（latency confound）。
- **検証スコープの限界**: single short English audio + simple task の 1 ケースのみ。多言語・長音声・複雑 task・robotics-grade command 品質は未検証（`04`:84 の eval は別）。

## 2026-07-01 追記 — transport 判定の実装 pointer（#388・main / §2・§5 の realize）

> §2（transport enum = bridge-local・observation-only）と §5/補遺（audio=direct 固定・fork で Hermes-default）を **additive に realize** する実装 pointer。行参照非破壊のため末尾に置く（#165）。

- **#388（main 在）**: ER audio leg の wire 判定は bridge-local `resolve_audio_transport`（[`../../ws/src/warehouse_llm_bridge/warehouse_llm_bridge/robotics/transport.py`](../../ws/src/warehouse_llm_bridge/warehouse_llm_bridge/robotics/transport.py):29）が担う＝`base_url` 非空 **かつ** `audio_input_audio_supported is True` の時のみ `Transport.HERMES`、他は恒久 fail-safe `Transport.DIRECT`（`transport.py:56-58`）。§2 が予測した通り **bridge-local・observation-only audit tag**（`warehouse_interfaces` には入れない）で dispatch しない（`transport.py:17-21`）。
- **§2:68 の grep note は #388 land 後に部分的に stale**: `Transport`/`ProviderType` enum は bridge-local **`adapters/enums.py:23,32`（`StrEnum`）に着地済**＝「ws/src に `Transport`/`ProviderType` enum ゼロ」は audio leg 用に解消。ただし `warehouse_interfaces` 昇格は依然 DEFER＝§2 の判定（observation-only・bridge-local・contract ラベル不要）は不変。
- **live 送信 seam**: **#344 = probe CLOSED / #389 の live-send 機構（`build_provider_request`/`ErTransportSender`/`_live_send`＝`gemini_er.py:87/154/222`）は main 着地済（ad563de）**。CURRENT で audio が `direct` なのは config `er_gateway` 既定 OFF（`transport.py:56-58`→`DIRECT`）＋ 8644 fork 非配備ゆえ・残は稼働 Bridge cycle の wiring（XER6）と 8644 fork 配備。運用手順 = operator runbook [`../dev/07-mode-x-er-live-e2e-runbook.md`](../dev/07-mode-x-er-live-e2e-runbook.md)。

## 2026-07-03 補遺 — ER-in-Hermes 標準化（方針転換・TARGET）（末尾追記＝行参照非破壊）

> §5 と上の各補遺で確定した「fork ありなら audio も Hermes に乗る」を、**設計方針として『ER-in-Hermes を標準 transport にする』へ格上げ**する（[`adr/0002-er-in-hermes-standard.md`](../adr/0002-er-in-hermes-standard.md) が決定正本）。**PROBE-2 = 400（§5:159「Hermes は音声を運べない」）は UNFORKED Hermes の実測として引き続き真**であり書き換えない。本補遺は additive で、行参照非破壊のため末尾に置く（#165）。

- **標準（TARGET・DESIGN）**: **1 本の fork gateway（port 8644）が全 modality（text + image_url + input_audio）を担う**。fork は input_audio を追加するだけで text/image を保持するため、従来 two-path（text/image=8643・audio=direct）の lean 8643 gateway は **8644 に統合し retire** する方針。
- **`direct` は緊急 fail-safe へ格下げ**: 標準経路は Hermes（8644）。`direct` は ①send 時 HERMES→DIRECT fallback（[`../../ws/src/warehouse_llm_bridge/warehouse_llm_bridge/robotics/adapters/gemini_er.py`](../../ws/src/warehouse_llm_bridge/warehouse_llm_bridge/robotics/adapters/gemini_er.py):222-243）②config 既定の恒久 fallback（`er_gateway` 両キー OFF・:269 と不変）としてのみ残す。
- **Langfuse**: **Pattern A（Bridge 所有 trace）を現行標準**、**Pattern B（Hermes plugin 所有・wrapper 除去）は HLF-G0〜G5 gate PASS 後の follow-up**（:270 / [`../productization/02-l4-robotics-bridge-box.md`](../productization/02-l4-robotics-bridge-box.md):177-199 と整合）。
- **overclaim しない（CURRENT 不変）**: **#389 の live-send 機構は main 着地済（ad563de）**だが、config `er_gateway` 既定 OFF（`transport.py:56-58`→`DIRECT`）＋ 8644 fork 非配備ゆえ **CURRENT（shipped）= audio は direct のまま**（:269 と一致）。残 TARGET = 稼働 Bridge cycle の wiring（XER6）と 8644 fork 配備。本補遺は「向かう標準方針」であって現稼働ではない。audio format は当面 **wav 優先**（fork の map 先＝`audio/wav`）で後拡張。fork メンテ負債は当面受容・upstream PR は後で。
- **2026-07-03 live 実測（evidence・operator 承認・課金）— 決定を裏づけ**（同一 TTS 音声で unforked / forked / direct を比較）:
  - **unforked Hermes + `input_audio` → HTTP 400 `unsupported_content_type`**（0.003s・ER 到達前に reject＝無課金）＝**fork 必須**を live 再確認（PROBE-2 と一致・§5:159 不変）。
  - **forked gateway + `input_audio` → HTTP 200**: ER が音声指示を理解し正しい 2-task plan（「bot1→赤箱／bot1 退出後 bot2→青箱」・611 tok）を返却＝**audio-in-Hermes（fork 経由）は ER 呼び出しまで end-to-end で実働**。
  - **latency（n=1・同一音声）**: fork-Hermes **5.49s** vs direct **4.61s**＝comparable。ER 推論（~4-5s・thoughts-token 支配）が支配的で transport overhead は小。先行 n=4（fork 3.69s / direct 4.24s・:268）とは順序が逆＝**noise 内**（速さは確定しない）。
  - **含意**: audio-in-Hermes は **fork 必須 かつ 実測で稼働** → **fork を維持して標準に採用**（「fork をやめる／drop」ではない）。
  - **運用 finding（Lane C #401 で解消予定）**: fork / unforked launcher が現状 `HERMES_HOME` / `.env` / port を共有し衝突（同時起動不可）＝分離は #401。


---

## 追補（2026-07-07）: §3 Mode X-ER config key — DEFER 解除・凍結（#165 回避のため末尾追記）

§3 の DEFER ゲート「XER3 Visual Resolver 着手前」は、XER3/4/5 の offline core が `origin/main` に land した時点（PR #371/#376・#372・#380。`cdb34e0` で検証）で実質到来し、**初回 consumer＝XER6 `x_er_bridge`（#342）の実装前が凍結の最終期限**となった。よって §3 を **RESOLVED** に更新し、凍結形は [08-x-er-bridge-node-spec §3](08-x-er-bridge-node-spec.md) を正本とする。
...
- U-9 **ER 出力 `pixel` の座標契約が未定義**（doc03 共通 instruction text は pixel に沈黙・production `_SCHEMA` はそれを忠実に写して pixel 節なし・live helper は「`[u,v]` 0–1000」を独自に要求・L3 の dev-sim-v1 artifact は raw px で fit・Gemini native は `[y, x]` 0–1000）= **#699**。**2026-09-16 訂正**: 2026-07-02 live の `command_items=0` は live helper（pixel 節あり）＋注入 calibration で「解決しないことが設計どおり」（`test_xer_full_chain_live.py:60-63`）であり本件が原因ではない。slice 1（#699 PR）= doc03 に pixel 節を追補し `PIXEL_RULE` 定数で production / live を単一ソース化、slice 2（同日・#699 close）= artifact が `pixel_space` / `image_size` を宣言し Visual Resolver が変換（[02 追補 ②](02-l3-planning-core.md)）。
