...
- **確定（`02`「ValidationReport 語彙確定」節・#338 前提）**: reject 系 `status`=`rejected`/`emergency_stop`、clarification=`OPERATOR_CLARIFICATION_REQUESTED`＋`dispatch_effect`、`severity`∈`error`/`warning`、`dispatch_effect`∈`block`/`needs_clarification`/`emergency_stop`/`none`、Detection/TaskNode は独立 model。いずれも既存確定語彙（`productization/05`:69・`02`:96・`03`:71）へ接地し新規契約を発明しない（`warehouse_interfaces` 昇格自体は本 §冒頭どおり XER1→XER2 後に DEFER）。
