...
| 5 | Hermes 経由で ER の audio / image API を扱えるか | **NEEDS-PROBE** | live probe 3本（PROBE-1〜3）の実測後 | ER audio capability=公式 YES / Hermes audio 透過=400 公算（**direct 一次**）/ image=Hermes 対応 |
...
## 5. Hermes 経由で ER の audio / image API を扱えるか — NEEDS-PROBE
...
**判定 / 根拠**: **NEEDS-PROBE**。ER 側 audio capability は公式記載で YES（憶測でない）だが、(1) ER の robotics audio 応答品質は Google も作例なし＝実測必須、(2) Hermes 透過は web 上「audio=400 公算・image=対応」で**実測が要る**。よって image は Hermes 経由でも probe で 200 を狙えるが、**audio は当面 direct（Gemini REST `inline_data`）が現実解**。`02`:281 の凍結は probe 後。
...
- **PROBE-1（ER audio direct・最優先）**: Gemini REST `POST .../models/gemini-robotics-er-1.6-preview:generateContent` に `parts=[{"text":"<指示>"},{"inline_data":{"mime_type":"audio/wav","data":"<base64 ≤20MB>"}}]` → HTTP 200 と task 理解応答を確認（`04`:83 の robotics 実証）。
...
[現在 = XER0 docs]
   ...
   │
   ├─ #5 NEEDS-PROBE  ── PROBE-1/2/3（live・要 .env スコープ承認）→ docs/* で結果確定
   │
...
| 5 | probe 実測後 | `docs/*`（probe 追記）→ 昇格時 `contract` | track:docs → llm-bridge |
...
- **decision_disagreements / missing_additive: なし**。7項目の判定（DEFER×4 + FREEZE-NOW/DEFER 二分×1 + NEEDS-PROBE×1）は docs 方針と整合。
