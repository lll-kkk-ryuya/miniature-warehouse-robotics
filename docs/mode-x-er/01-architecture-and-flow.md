...
Mode X-ER は、L4 で Gemini Robotics-ER と audio / camera / state を薄く接続し、L3 で ER の提案を既存実行基盤が理解できる command 候補へ変換する。L2 以降は既存の MCP / Policy Gate / Nav2 / Open-RMF 経路を使う。
...
L4 は大きな判断を自作しない。audio / image / state を束ね、Gemini Robotics-ER に渡し、raw output を受け取る薄い adapter である。
