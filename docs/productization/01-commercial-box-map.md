...
| **L1** 自律走行・安全 | Nav2 の実走と物理停止（collision_monitor / twist_mux / Emergency Guardian） | Navigation: `ws/src/warehouse_nav2_bridge/`（REST→Nav2 action）・`ws/src/warehouse_bringup/config/`（`nav2_params.yaml`・`collision_monitor.yaml`・`twist_mux.yaml`）／ Safety: `ws/src/warehouse_safety/`（`emergency_guardian.py`・`guard_logic.py`）／ 知覚入力（**安全層外**・producer のみ・L1 の一部ではない）: `ws/src/warehouse_perception/warehouse_perception/terrain_core.py`（Mode Outdoor 04 の 01_Geometry / 07 coverage・cliff_scan 純ロジック = [mode-outdoor/04 追補 ⑤](../mode-outdoor/04-perception-sidewalk-and-signals.md)。**0 node・0 cmd_vel・GPU 非依存**〔R-26 AST pin〕。出力先は L1 CM / L1 Navigation の costmap〔`cliff_scan`〕と X2・06〔`coverage`〕で、判定・停止・通行可否は持たない〔三分離 = [04:214](../mode-outdoor/04-perception-sidewalk-and-signals.md:214)〕。**2026-09-17: 地面 fit は取付事前値 `Z = 0` からの逸脱上限を注入した constrained RANSAC へ**（`OQ-OD4Z-d` 裁定 = [04 追補 ⑧](../mode-outdoor/04-perception-sidewalk-and-signals.md:838)。平面の傾き / 原点高さ / 支持 / 事前値フォールバック / 棄却候補数は `ObservationQuality` v0.1 で**自己申告のみ**——判定は X2））。**2026-09-18: node 化**＝`terrain_node.py` / `terrain_node_core.py`（`terrain_publisher`・`OQ-OD4Y-i` 裁定 = [04 追補 ⑨](../mode-outdoor/04-perception-sidewalk-and-signals.md:926)）。**producer のみ・0 actuation**〔`cmd_vel` / `stop_request` / `stop_state` / `speed_limit` を publish しない = AST pin〕・既定 `enabled:=false` で **subscription も publisher も作らない**・publish は `cliff_scan` と `terrain/coverage` の 2 本だけ。**2026-09-18 配線**: launch `_terrain_group` / config `perception.terrain.*`（既定 OFF・値は実測後）= #723〔[04 追補 ⑫](../mode-outdoor/04-perception-sidewalk-and-signals.md)〕／costmap `cliff_layer`（別 ObstacleLayer instance・両 costmap）+ CM source `cliff_scan`（launch 時 arm）= #724〔[04 追補 ⑩](../mode-outdoor/04-perception-sidewalk-and-signals.md)〕／X2 側 = 純アダプタ `warehouse_safety/terrain_health.py`（**L1**・#722・[04 追補 ⑪](../mode-outdoor/04-perception-sidewalk-and-signals.md)）＋ **Emergency Guardian の estop 理由 `terrain_health`**（`OQ-OD95` = A 裁定・`perception.terrain.enabled` で gate・#728・[04 追補 ⑬](../mode-outdoor/04-perception-sidewalk-and-signals.md)）） |
...
| **横断** Eval / Observability（観測面） | 全 box（L4–L0）を横断して raw event / trace / KPI を集約する（motion 権限なし） | `ws/src/eval_sdk/`・`ws/src/warehouse_orchestrator/`・`ws/src/warehouse_web_bridge/`（observe-only consumer・[architecture/22](../architecture/22-web-observability.md)）・`ws/src/warehouse_perception/`（`model_manifest.py` / `evaluation_core.py` = mode-outdoor 04 の 09_Runtime / 10_Evaluation・offline 評価基盤・**単一 layer に帰属させず本行で扱う**。理由 = [mode-outdoor/04 追補 ⑦ §6](../mode-outdoor/04-perception-sidewalk-and-signals.md:813)） |
