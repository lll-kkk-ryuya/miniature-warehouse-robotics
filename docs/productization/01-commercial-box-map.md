...
| **L1** 自律走行・安全 | Nav2 の実走と物理停止（collision_monitor / twist_mux / Emergency Guardian） | Navigation: `ws/src/warehouse_nav2_bridge/`（REST→Nav2 action）・`ws/src/warehouse_bringup/config/`（`nav2_params.yaml`・`collision_monitor.yaml`・`twist_mux.yaml`）／ Safety: `ws/src/warehouse_safety/`（`emergency_guardian.py`・`guard_logic.py`）／ 知覚入力（**安全層外**・producer のみ・L1 の一部ではない）: `ws/src/warehouse_perception/warehouse_perception/terrain_core.py`（Mode Outdoor 04 の 01_Geometry / 07 coverage・cliff_scan 純ロジック = [mode-outdoor/04 追補 ⑤](../mode-outdoor/04-perception-sidewalk-and-signals.md)。**0 node・0 cmd_vel・GPU 非依存**〔R-26 AST pin〕。出力先は L1 CM / L1 Navigation の costmap〔`cliff_scan`〕と X2・06〔`coverage`〕で、判定・停止・通行可否は持たない〔三分離 = [04:214](../mode-outdoor/04-perception-sidewalk-and-signals.md:214)〕。**2026-09-17: 地面 fit は取付事前値 `Z = 0` からの逸脱上限を注入した constrained RANSAC へ**（`OQ-OD4Z-d` 裁定 = [04 追補 ⑧](../mode-outdoor/04-perception-sidewalk-and-signals.md:838)。平面の傾き / 原点高さ / 支持 / 事前値フォールバック / 棄却候補数は `ObservationQuality` v0.1 で**自己申告のみ**——判定は X2）） |
