...
Hermes Agent は、Robotics Bridge Super-Box の transport / provider / generic tool integration を支える。2026-06-22 時点で公式 docs から確認できる範囲では、以下を Hermes-managed area に置ける。

| Sub-Box | Hermes-managed にできる内容 | Bridge-owned に残す内容 |
|---|---|---|
| Model Transport | provider 切替、custom endpoint、OpenAI 互換 endpoint、provider fallback | robotics cycle、request id、timeout 後の 0 dispatch、final output の採用判定 |
...
| 経路 | 使う条件 | 守るべきこと |
|---|---|---|
| Hermes 経由 | Hermes が対象 model、audio、image、structured output、timeout を扱える | server-side tool execution を使わず、Bridge が final output を受けて L3 へ渡す |
