...
- **鮮度窓の tighten-only（裁定済み・旧 REVIEW POINT）**: 2026-07-09 裁定で **① tighten-only を採用**（PR #427 で実装）。レーン `feat/policy-gate-freshness-config` は Policy Gate 鮮度窓を config 駆動にするが、overlay は凍結既定 0.5/2.0 を上限として**縮める方向のみ**許可し、より緩い値は起動拒否（restrict-only [../adr/0004-l2-restrict-only-policy-profile.md](../adr/0004-l2-restrict-only-policy-profile.md) と一致・旧 `MAX_FRESHNESS_S=10.0` ceiling は撤去）。詳細は §2026-07-09 補足 ②注記。
...
| robot 鮮度窓（stale/unavailable） | 窓を縮める（tighten）方向のみ | **不可**（緩める＝起動拒否・凍結既定 0.5/2.0 が上限。PR #427 で実装） |
| rate-limit 間隔 | 間隔を伸ばす（頻度を厳しく） | 間隔を縮める |

**注記（鮮度窓の裁定・旧 REVIEW POINT）**: **2026-07-09 裁定: ① tighten-only を採用**（PR #427 で実装）。レーン `feat/policy-gate-freshness-config` は Policy Gate の robot 鮮度窓 `policy_gate.stale_after_s`（既定 0.5）/`unavailable_after_s`（既定 2.0）を config 駆動・additive・既定不変・fail-closed にする。**overlay は凍結既定 0.5/2.0 を上限として窓を縮める（tighten）方向のみ許可**し、より緩い値（`>0.5` / `>2.0`）・非有限・<=0・stale>unavailable・非 mapping は起動拒否＝restrict-only と**一致**（旧案の `MAX_FRESHNESS_S=10.0` ceiling は 0.3 m/s×10s=3.0m>1.8m ジオラマで物理包絡を縛れないため撤去）。同レーンは restrict-only を config 駆動 fail-closed で具体化する **最初の実装例**として引く。
