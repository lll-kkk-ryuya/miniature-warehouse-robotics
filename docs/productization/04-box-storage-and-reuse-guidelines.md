...
顧客や現場責任者が自然言語で説明する rule は、直接 runtime config にしない。
LLM 支援の rule authoring では、まず確認質問で曖昧さを潰し、次に
site profile、plugin profile、fixture、simulation run manifest の draft へ分ける。
たとえば「人作業エリアに近づかない」は、距離、時間、例外 robot、違反時の扱い、
下位 safety との責務分担を確認してから、L3 / L2 / L1-L0 / Eval-only のどこへ置くかを
決める。詳細は [10-llm-assisted-rule-authoring.md](10-llm-assisted-rule-authoring.md)。
...
LLM が生成した rule 案も、positive / negative / boundary / stale / emergency injection /
multi-robot conflict の fixture を持つまでは `approved` にしない。
...
## Safety-Critical Profile の Review + Version + Hash Gate

site profile の `safety.yaml` / `calibration.json`（[§Site Profile](#site-profile) の 2 artifact）は誤 actuation を左右する **safety-critical content** であり、他 artifact と区別して扱う。すなわち (1) human review + version + content-hash で pin し、(2) 承認済み hash と一致しない drift は **preflight で FAIL**（safety-critical 以外の artifact は RECORD に留める）。これは **calibration self-certification hole** を閉じるための gate である: Visual Resolver の Gate 2（reprojection-error ceiling）は calibration が **自分について申告する** `reprojection_error` で駆動するため、その field が `None` のとき gate は **skip** される（`visual_resolver/resolver.py` Gate 2）。誤った homography が error を省くだけで gate を自己通過し、実 motion を誤地点へ向けうる。防御は **UPSTREAM** に置く（resolver は非改変）: 本番 build path は `GovernedCalibrationLoader` 経由でのみ calibration を得、`error 未申告 / 非有限 / site ceiling 超過 / ceiling 未設定` を **reject** する（明示 waiver〔who / why / when〕がある時だけ admit し、[effective-composition record](09-run-manifest-and-plugin-composition.md) に記録する）。governance policy は site profile の `safety.yaml` の `calibration:{max_reprojection_error, waivers}` に置く（bridge-local proposal・`max_reprojection_error` 未設定＝reject-all）。profile bundle には identity（customer / site / version）と content hash（merged-canonical と per-file の 2 法を併走）を与え、`draft -> ... -> approved` の approved 以上だけを run manifest で有効化し（[10-llm-assisted-rule-authoring.md](10-llm-assisted-rule-authoring.md) の Authoring Loop profile ライフサイクル）、rollback は前 profile version へ戻す。この gate の結果は [run_manifest.v1 の `profile_version` / `profile_content_hash`](09-run-manifest-and-plugin-composition.md) で run に残る。座標 goal を扱う下流は [03 §そのまま移せない箇所の coordinate-goal 注記](03-l3-planning-core-box.md#そのまま移せない--設計し直す箇所注意点)（plugin / site profile では吸収不可・Core 契約の再凍結が必要）と整合させる。決定の背景は [ADR-0003](../adr/0003-bridge-local-manifest-composition.md)。
