...
- **coordinate goal**: 製造は「名前付き location へ snap」では足りず、精密な part pose が常態。倉庫 MVP が defer している coordinate-goal 経路（L3-G4）が製造では first-class の必須項目になり、work envelope / reachability / collision check を持つ独自 gate を**先に凍結**する必要がある。
  - **注記（composition 層では吸収不可）**: この coordinate-goal 経路の凍結は **plugin / site profile では吸収できない**＝run manifest / plugin composition（[09](09-run-manifest-and-plugin-composition.md)）の対象外。plugin は box interface を変えない案件差分の差替点であり、新しい実行契約（座標 goal を MCP / Policy Gate 経由で流す product contract）は **Core 契約の再凍結**を要する別トラック（owner docs + contract PR 先行）。coordinate goal を MCP / Policy Gate 経由で流す正式 contract が未凍結である点は本 box 上流（§Visual Resolver Box `:113`）と、Mode X-ER 側で「known location 以外の coordinate goal は未凍結の間 compile しない」とする [mode-x-er/02:231](../mode-x-er/02-l3-planning-core.md)（+ `:37`,`:152`）と整合する。A案（manifest resolution 層）スコープ外。calibration も同じく plugin では自己認証を閉じられない「吸収不可」境界であり、そちらは上流 gate（[04 §Safety-Critical Profile の Review + Version + Hash Gate](04-box-storage-and-reuse-guidelines.md#safety-critical-profile-の-review--version--hash-gate)）が閉じる。両者の決定背景は [ADR-0003](../adr/0003-bridge-local-manifest-composition.md)。
...
## durable store を pipeline entry point へ注入する seam（Mode X-ER 側の対応）

上記 `durable store`（`:132`）＝ `TaskGraphStore` を file/Redis/DB へ差し替える（`:135`）ための注入口は、Mode X-ER の pipeline entry point `compile_raw_output` 側に生えている。詳細（keyword-only・相互排他な `executor=` / `store=`・三分割の所有権 STORE/executor/caller loop・R-26 の zero get/put・single-live-handle-per-plan-per-cycle の stale-handle ハザード・default-path 後方互換）は [mode-x-er/02 §Pipeline entry point の store/executor 注入 seam](../mode-x-er/02-l3-planning-core.md#pipeline-entry-point-の-storeexecutor-注入-seamxer5s1) が正本。ここ（`:127-133` の site 差替点）は「何を差し替えるか」、mode-x-er/02 側は「どこに注入するか」を持つ。
