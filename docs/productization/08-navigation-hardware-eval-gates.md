...
5. gate family（N-G / H-G / E-G）と `box=...` は所有 box に帰属する**集計軸**（種別の正本は `01` §Box 種別と分類規則）。`N-G1` の owner「L3 Handoff + Governance + Nav2 Bridge」は **multi-owner seam**（`L3 Handoff` は L3 Planning Core 配下の seam であって独立 box ではない）。
