...
| Eval / Observability | `eval_sdk`（doc21 proposal、API は未凍結）、Bridge-owned Langfuse fail-open、JSONL / decision event、warehouse-specific KPI producer。 | OpenTelemetry / OTLP、DuckDB、rosbag2、ros2_tracing。Hermes Langfuse plugin は HLF-G0〜G5 を満たす場合のみ trace owner 候補に戻す。 | Langfuse v4 live、Hermes plugin trace passthrough、OTLP attribute retention、DuckDB report export、trace join gap。 | Eval が Safety / Governance の enforcement path に入ること。高頻度 ROS signal を常時 Langfuse へ投げること。Bridge wrapper と Hermes plugin の二重 generation。 | E-G0 から E-G6。sink failure fail-open、same `run_id` / `gen_id` join、missing trace を説明できること。Hermes plugin 採用時は HLF-G0〜G5 を全て満たすこと。 |
