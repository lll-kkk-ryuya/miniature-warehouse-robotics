...
| Plugin hook | pluggy / `importlib.metadata.entry_points` | L3 Validator rule、Visual Resolver、Governance policy hook、Eval exporter を box interface を変えずに差し替える候補。詳細は [09](09-run-manifest-and-plugin-composition.md) |
| Offline aggregation | DuckDB / JSONL | `audit.jsonl`、`decision_events.jsonl`、result export を schema-on-read で join する report 候補。詳細は [09](09-run-manifest-and-plugin-composition.md) |

## Layer / Box ごとの判断

| Layer / Box | 採用 | 候補 | 要 spike | 不採用 | 採用条件 |
|---|---|---|---|---|---|
| L4 Input Context (sub-box) | Pydantic / `warehouse_interfaces` 系の構造化 validation。Langfuse trace metadata。STT は Hermes voice / STT transport を第一候補にする。 | JSON Schema export、OpenCV calibration artifact、offline Whisper / custom STT adapter、OpenTelemetry span。 | STT latency、Hermes STT failure fallback、camera calibration fixture、artifact retention。 | Nav2 URL、MCP endpoint、`/cmd_vel` を model input bundle に入れること。 | secret / endpoint 混入検査、stale state policy、offline fixture から同じ bundle を再生成できること。 |
| L4 Model Adapter (sub-box・proposal) | **Hermes Agent Gateway を第一候補 transport**として使う。Bridge-owned adapter seam、Hermes / direct / worker の切替、raw output recorder、Langfuse observation。 | OpenVLA / LeRobot offline fixture、provider-specific JSON Schema、direct adapter は Hermes 非対応時の fallback。 | OpenVLA runtime / GPU / license、Hermes 経由と direct 経由の同等性、provider timeout、Hermes fallback が比較公平性に与える影響。 | ER / VLA / LLM から Nav2、Open-RMF、micro-ROS、motion tool を直接実行すること。 | timeout / provider error が 0 dispatch、raw output が artifact 化、同じ L3 Handoff input を返せること。比較 run は provider 固定 leg と fallback-enabled leg を分けること。 |
...
| Eval / Observability | `eval_sdk`（doc21 proposal、API は未凍結）、Bridge-owned Langfuse fail-open、JSONL / decision event、warehouse-specific KPI producer。 | OpenTelemetry / OTLP、DuckDB、rosbag2、ros2_tracing、run manifest / plugin manifest。Hermes Langfuse plugin は HLF-G0〜G5 を満たす場合のみ trace owner 候補に戻す。 | Langfuse v4 live、Hermes plugin trace passthrough、OTLP attribute retention、DuckDB report export、trace join gap、expected emitter gap。 | Eval が Safety / Governance の enforcement path に入ること。高頻度 ROS signal を常時 Langfuse へ投げること。Bridge wrapper と Hermes plugin の二重 generation。 | E-G0 から E-G6。sink failure fail-open、same `run_id` / `gen_id` join、missing trace を説明できること。Hermes plugin 採用時は HLF-G0〜G5 を全て満たすこと。 |
