...
> 以下の **Input Context / Model Adapter / Fusion は L4 Robotics Bridge Super-Box の sub-box**（種別の正本は `01` §Box 種別と分類規則）。**Model Adapter / Fusion は ws/src に実体なし＝proposal**、Fusion は単一 model では不使用＝optional。`L3 Handoff`（下記）は box でなく L3 Planning Core 配下の seam。各節の `box=...` / gate ID は観測の集計軸として据え置く。
...
## L3 Handoff（seam → L3 Planning Core 配下）

L3 Handoff は **独立 box ではなく、L3 Planning Core Box の先頭 seam** である（種別の正本は `01` §Box 種別と分類規則）。L4 の raw / fused output を L3 Planning Core が読める deterministic input に正規化する常時随伴ステージで、**新しい schema を産まず**出力は L3 Validator の入力契約そのもの（`03` のパイプライン先頭 `RawModelOutput → RoboticsPlan draft → Validator`）。ER/VLA output があれば必ず通り、単独で入替/省略できないため box には昇格しない。**観測上は `box=l3_handoff` funnel literal と gate `L3H-G*` を据え置く**（decision_event の集計軸は再利用 box 階層と別レイヤ）。gate `L3H-G*` は L3 Planning Core Box の acceptance-gates へ帰属させる。
...
output を deterministic plan draft へ正規化する **seam（前段ステージ）**であり、L3 Validator はその
plan draft を実行候補として扱えるかを判定する **box の中核**である。両者は同じ L3 Planning Core Box 内にあり、責務分割を seam で表現する（独立 box には昇格しない）。
