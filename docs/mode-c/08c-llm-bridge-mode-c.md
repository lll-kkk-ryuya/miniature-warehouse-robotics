...
**生成側（producer / #4）の責務 — 直列化で省略する**: 上記 Optional 化は省略を**許可するだけ**で、~200トークン削減は自動では達成されない。pydantic v2 は既定で `None` フィールドも出力するため（[pydantic serialization 公式doc](https://docs.pydantic.dev/latest/concepts/serialization/)）、LLM Bridge は Mode C の `situation` を**戦略フィールドのみ set した状態で構築し `model_dump(exclude_unset=True)` で未設定フィールドを落とす**（実装は dict を返し、JSON 直列化は publish 側。`situation.py`。実測で各 robot は `position`/`status`/`current_task`/`battery` のみ残る）。**`exclude_none=True` では不十分**: `obstacle_ahead` の既定値 `False` は `None` でないため出力に残ってしまう。`exclude_unset` は「明示的に set されたフィールドのみ出力」（`model_fields_set` 連動）なので、未設定の省略対象（velocity/heading/predicted_position_3s/obstacle_ahead/obstacle_distance）が全て落ちる。
...
- Open-RMFの調整が3回失敗した場合のみClaudeにエスカレーション。**通知経路**: producer `11c RMFTrafficManager.get_traffic_state()` が解消不能な衝突を situation の `traffic.escalation`（非null）に載せる → 司令官は §システムプロンプト の `traffic.escalation` gate で介入する（階層は §エスカレーション階層 / `11c §6`＝`11c-traffic-mode-c.md:343-371`）
