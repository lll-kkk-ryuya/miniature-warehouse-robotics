...
## 3.6 実 Open-RMF の velocity 取り扱い（design note・GATE 前・未配線）

> §3.5 案A（自作 EasyFullControl Fleet Adapter）を**実 Open-RMF**で配線する段（**R-38 メモリゲート #187 OPEN・R-44 でブロック中**）の velocity 設計メモ。本リポの `warehouse_rmf_adapter` は RMF 非依存の offline core（location→Nav2Goal だけ・`fleet_adapter.py`=`NotImplementedError`）で、実 RMF スタックは未配線（§3.5 D / rmf_adapter CLAUDE.md）。本節は **design-only**＝fleet config の数値・angular 上限・schema を凍結しない。`:284` の残未決「**≤0.3 m/s で RMF デコンフリクトが有効か**」を一段具体化する。

### なぜ実 RMF では velocity（kinematic limits）が必要か

実 Open-RMF の Fleet Adapter（EasyFullControl）は、robot の **kinematic limits（linear / angular の velocity・acceleration）** と profile（footprint / vicinity radius）を fleet config として要求する。これは RMF の **Traffic Schedule が軌道を予測して事前デコンフリクトする**ための **planning 入力**であり、offline core（経路調整・予約なし）には存在しない。X-rmf（mode-x-er の optional profile）も同じ RMF 経路を使うため同条件。

### 単一ソースと上限（最重要・safety 不変）

- linear velocity 上限は **`warehouse_interfaces.safety.MAX_LINEAR_VELOCITY` = 0.3 m/s（`safety.py:18`・単一ソース・hard cap）** から供給する。運用値は config `safety.max_linear_velocity`（`config/warehouse.base.yaml`・`load_config` が `0 < cap ≤ 0.3` を検証）で**下げられるが 0.3 を超えて上げない**（safety.md / `safety.py` docstring）。RMF fleet config に **`0.3` をハードコードしない**＝`safety.py` を import する（「import them directly and do NOT hardcode 0.3 elsewhere」）。
- RMF に渡す velocity は **planning 入力**であって、下流の hard clamp を上書きする権限ではない。実際の速度強制は従来どおり多層で行う: **L0 MCU clamp / Nav2 `vx_max`（`nav2_params.yaml:122`・config 由来 ≤0.3）/ Emergency Guardian（reflex, `safety.py:25-33` `clamp_velocity`）**。RMF が planning 上 0.3 を仮定し、実走は下流クランプが保証する**二重構造**を維持する。

### `Command` 契約には velocity を足さない

- velocity は **凍結 `CommandItem`（`schemas.py:143-153` = bot/action/destination/via/duration/retreat_to/idempotency_key）に追加しない**。司令官 LLM / L3 は速度を compile・指定せず、velocity は下流（L0 / Guardian / Nav2）でのみ強制する。RMF velocity は **fleet adapter config の領域**であり、**LLM-facing Command 契約とは別レイヤ**に保つ。これにより、stale 応答・幻覚・誤指示が「速度の actuation 面」に化けることを防ぐ（`Command` への velocity 追加は破壊的 contract 変更かつ safety regression のため不採用）。

### 未凍結・残未決（GATE 後に確定）

- **angular velocity / acceleration 上限**: 現 単一ソース `safety.py` は **linear のみ**（`MAX_LINEAR_VELOCITY`）。angular 上限は未定義 → 実 RMF 配線時に決める（暫定値を本節で凍結しない。必要なら `safety` 契約へ **additive**＝contract PR）。
- **linear acceleration 上限 / robot profile（footprint・vicinity radius）の値**: 実測（#187 ゲート後の sim / 実機）で確定。本節では数値を発明しない。
- **≤0.3 m/s での RMF デコンフリクト有効性**（特に 200mm 真隘路 #124）: sim 検証待ち（`:284` / §3.5 残未決 6）。
- 配線自体は **#187（R-38 メモリゲート）+ R-44** でブロック。本節は設計提案で、RMF 配線・fleet config の凍結は GATE 後。

正本: `safety.py:18,25-33` / `.claude/rules/safety.md` / 本 §3.5（`:203-296`）・`:284` / `schemas.py:143-153` / `nav2_params.yaml:122`。関連 issue: #228（実 Open-RMF 転送 seam）/ #180（EasyFullControl spike）/ #187（メモリゲート）。
