...
- **composition preflight**（startup fail-closed） — `manifest.plugins ⊆ registered hookimpls` でなければ起動拒否（`CompositionError`）。pluggy の 0-impl=空 list=fail-open（plugin 不在が「全 approve」と観測的に区別できない）穴を閉じる**唯一の防御**。 — owner: [productization/09](productization/09-run-manifest-and-plugin-composition.md) §startup fail-closed composition preflight
