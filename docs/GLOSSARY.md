...
- **`policy_gate`（config sub-tree）** — Policy Gate のロボット鮮度窓を config から解決する `policy_gate`（`stale_after_s` / `unavailable_after_s`）。base 既定 0.5 / 2.0（現行値・env overlay で上書き可・additive）、fail-closed 検証（非数値・非有限・`<=0`・`stale>unavailable`・`>MAX_FRESHNESS_S=10.0` は起動拒否）。 — [config/warehouse.base.yaml:125-127](../config/warehouse.base.yaml) / [architecture/12-infrastructure-common.md:346](architecture/12-infrastructure-common.md)
