...
prod は **git タグ固定**（`v0.x`）を clone/checkout して実行（doc19:6,:118 / doc17 §4.0 別マシン=clone）。
...
deploy/jetson/bin/preflight.sh --arrival
```
...
deploy/jetson/bin/preflight.sh --arrival --gates G0,G1,G7
...
`preflight.sh --gates` は G0/G1/G7 のうち自動判定できる部分を PASS/FAIL し、Layer 0 の速度クランプ・
近接 e-stop、Bridge 認証サイクルなど実測が必要な項目は **MANUAL** として明示する。secrets 本体
（`config/prod/.env` / `~/.hermes/.env`）は読まない。
...
|---|---|---|:---:|
| `warehouse-microros-agent.service` | micro-ROS Agent（WiFi/UDP, doc02:81） | `network-online.target` | — |
...
- `deploy/jetson/bin/preflight.sh --offline`（下記の静的検査を束ねる。macOS では Linux 専用コマンドを SKIP）。
