...
現在は **Step 1＝各トラックの並列実装フェーズに突入**（#4 llm-bridge 着手済み）。
...
| 4 | llm-bridge | 🟡 **in progress**（核心・最重量。`mwr-llm-bridge`/`feat/llm-bridge` 着手済。LLMClient IF + action_map + gen_id B-3 を偽トピックで先行） |
...
## 次の山（#4 と独立並行できるもの）
- **#5 safety-state** — 完全独立・実機不要（偽入力でCI完結）。安全機構（R-26）。このセッション着手の最有力。
