...
**【現況 2026-06-03 更新】**上記のうち **#76 sim `/clock`+占有マップ(#94, critical-path) / #6 wo node-wiring(#92) / #75 bringup 合成(#93)** は **3 本ともマージ済**、**#2 jetson deploy scaffold(#96) もマージ済**。**#4 S2-PR2 HALF B（実 dispatch + MCP→nav2_bridge REST）=#104 / docs CTRV(08a)=#100 もマージ済**。派生 issue **#101(CTRV実装・#100 merge で解錠)/#102(history充填+current_task)/#103(Hermes Memory)** を起票。その後 **#105(STATUS鮮度)/#106(jetson safety doc)/#107(doc03 内部トピック表網羅, closes #61)/#110(CTRV 実装, closes #101)/#111(Phase4 比較公平性＝memory/skills 強制 OFF の運用契約を docs 化。#103 本体は未実装 OPEN)/#113(nav-traffic #67 E2E gate 修正)** もマージ済 → **main=`7eb9409`（#114 まで land）**。続く **#115（llm-bridge: #102 短期記憶 + #108/#109 cross-lane Langfuse join 修正・review 中）** と R-37/R-44 tracking 起票（#116/#117）。#91 で **docs-first 実装ゲート**（[parallel-workflow.md §1.1](../.claude/rules/parallel-workflow.md)：着手前 file:line 引用 → 実装中 CLAUDE.md 記録 → 完了前 `check_consistency.py` 0ERROR + `/consistency-audit` + 未決列挙）が**完了条件として必須化**された。

## main の状態（CI 常時 green, origin/main = `7eb9409`）
...
| 4 | llm-bridge | 🟢 **S1 + S2-PR1 + S2-PR2 HALF B main 入り（#70/#78/#86/#104）**: 司令官サイクル(Scheduler+HermesClient+Situation+action_map+排他 A/B-3/C) / **Bridge-owned Langfuse trace `tracing.py`(#78)** / **nav2_bridge part1(#86)** / **実 in-process tool dispatch（`WarehouseTools().dispatch` 注入・同一トラック #81）+ MCP→nav2_bridge REST 転送（`nav2_client`・受理時のみ・R-26）(#104)**。MCP 層(#35/#41) 済。**残**: Mode C None / R-35A(#54) / Open-RMF 転送経路 / history 充填+current_task = #102。**予測 CTRV 化 = #101 ✅完了（#110、`situation.py._predict` を CTRV 化）**。Langfuse Phase3 検証は #88。**latent（Phase3/4 前に要解決）: live trace join 二重故障 — #108（Bridge が `WAREHOUSE_RUN_ID` 未配線）/ #109（MCP executed audit に `gen_id` 欠落）** |
...
- **（決定・実装済）Langfuse trace 所有 + provider access**（#72 docs / #78 Bridge seam / #83 wo wiring。**#73 は close、Phase3 実トレース検証は #88**）: trace 所有は **Bridge**（`langfuse.openai` + `base_url`=Hermes、Hermes 内蔵 Langfuse は無効化）。score は v4 `create_score`（v2 `langfuse.score` 廃止）。`trace_id`=32hex no-dash、#4↔#6 は `create_trace_id(seed)` 導出で契約不要、audit 突合は `gen_id`+timestamp。**⚠️ live join は従来 no-op だった**（#108: Bridge が `WAREHOUSE_RUN_ID` を読まず run_id=session_id(strftime)＝#4↔#6 trace_id 不一致 / #109: MCP executed audit 行に `gen_id` 欠落＝WO score join 不成立）が、**両者を #115 で修正（review 中。`resolve_run_id`＝WAREHOUSE_RUN_ID 起点 + executed 行 `detail.gen_id`、producer↔consumer / cross-leg test 検証）**。#115 マージで Phase3/4 比較検証の前提が成立。**Vertex AI SDK は不採用・Hermes 単一経路維持**（doc13 §7.5/§7.6）。

## 次の山（kickoff brief: `~/Developer/mwr-handoff/round-s2-2026-06-03/`）
**直近マージ済（2026-06-03）**: #94 sim `/clock`+`sim_time`+占有マップ（critical-path 解錠） / #92 wo node-wiring / #93 bringup.launch.py 合成 / #96 jetson deploy scaffold / **#104 #4 S2-PR2 HALF B（実 dispatch+MCP→nav2_bridge REST 転送）** / **#100 docs CTRV（08a linear→CTRV）** / **#105 STATUS 鮮度** / **#106 jetson safety doc（Requires→BindsTo）** / **#107 doc03 内部トピック表網羅（closes #61）** / **#110 CTRV 実装（closes #101）**。
...
- 派生 issue: **#101**(CTRV実装 ✅**#110 で完了**＝`situation.py._predict` CTRV 化済) / **#102**(history充填+current_task) / **#103**(Hermes Memory(FTS5) 本体は **OPEN・未実装**。#111 は Phase4 比較での memory/skills 強制 OFF 運用契約のみ docs 化)。
- **latent 整合（Phase3/4 比較検証の前提）**: **#108**(Bridge が `WAREHOUSE_RUN_ID` 未配線→#4↔#6 trace_id 不一致) / **#109**(MCP executed audit 行に `gen_id` 欠落→WO live score join no-op)。両者で Langfuse 比較 join が二重に成立しない（CI 緑・fail-open で silent）。
...
- main = `origin/main`(`7eb9409`) 同期・クリーン。**open PR: #115(llm-bridge #102 短期記憶 + #108/#109 cross-lane Langfuse 修正) + 本 docs PR `docs/status-cleanup-0603`**。新規 issue: #116(R-37 micro-ROS 2台)/#117(R-44 free_fleet)。直近 land（新しい順）: #114(CTRV 横展開 + STATUS 鮮度) / #111(Phase4 比較 memory/skills 強制OFF 運用契約 docs。#103 本体は未実装) / #113(nav-traffic #67 E2E gate 修正) / #110(CTRV 実装, closes #101) / #107(doc03 内部トピック表網羅, closes #61) / #106(jetson safety doc Requires→BindsTo) / #105(STATUS 鮮度) / #100(08a CTRV 改訂) / #104(#4 S2-PR2 HALF B 実 dispatch+nav2_bridge REST 転送) / #98(doc12 battery `<=`) / #97(doc09 §7 sim/SLAM 二系統) / #94(sim `/clock`+占有マップ, critical-path) / #96(jetson deploy scaffold) / #92(wo node-wiring) / #93(bringup 合成) / #91(docs-first 実装ゲート) / #90(battery `<=10%`, closes #59)。
- **issue 整理（2026-06-03）**: #73(Langfuse trace 所有=完了 → Phase3 は **#88**) / audit umbrella **#37**(根本=#41確定) / #59(battery `<=10%`、#90 で close) を close。日次レポート #53/#62/#63/#84 を close。残 open の整合 issue は **#44/#54/#55/#108/#109**（#61 は #107 で close 済）（+ #88 Phase3 Langfuse 検証）。
