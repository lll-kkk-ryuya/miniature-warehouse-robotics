...
**【現況 2026-06-03 更新】**上記のうち **#76 sim `/clock`+占有マップ(#94, critical-path) / #6 wo node-wiring(#92) / #75 bringup 合成(#93)** は **3 本ともマージ済**、**#2 jetson deploy scaffold(#96) もマージ済**。**#4 S2-PR2 HALF B（実 dispatch + MCP→nav2_bridge REST）=#104 / docs CTRV(08a)=#100 もマージ済**。派生 issue **#101(CTRV実装・#100 merge で解錠)/#102(history充填+current_task)/#103(Hermes Memory)** を起票。その後 **#105(STATUS鮮度)/#106(jetson safety doc)/#107(doc03 内部トピック表網羅, closes #61)/#110(CTRV 実装, closes #101)/#111(Hermes Memory/Skills を Phase4 比較で強制 OFF, closes #103)/#113(nav-traffic #67 E2E gate 修正)** もマージ済 → **main=`2956869`・open PR 0**。#91 で **docs-first 実装ゲート**（[parallel-workflow.md §1.1](../.claude/rules/parallel-workflow.md)：着手前 file:line 引用 → 実装中 CLAUDE.md 記録 → 完了前 `check_consistency.py` 0ERROR + `/consistency-audit` + 未決列挙）が**完了条件として必須化**された。
...
- 派生 issue: **#101**(CTRV実装 ✅**#110 で完了**＝`situation.py._predict` CTRV 化済) / **#102**(history充填+current_task) / **#103**(Hermes Memory ✅**#111 で完了**＝Phase4 比較で memory/skills 強制 OFF)。
