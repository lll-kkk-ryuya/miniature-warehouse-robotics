...
**Step 1＝各トラックの並列実装フェーズが進行中**。#4 llm-bridge S1 / #8 nav-traffic / #6 wo / #5 safety-state / #7 sim / #25 gen_id 冪等化 / Mode C 契約(#66) を並列実装し、**Phase 0.5 スライスが main に land 済**（下表）。次ラウンドは #4 S2（Langfuse trace 所有）・#8 Gazebo E2E・#6 Langfuse 実配線（Issue #73）。
...
- **残り `main()` スタブ / 未実装**: `warehouse_nav2_bridge`（REST→BasicNavigator）、`warehouse_teleop`、`warehouse_bringup`（per-bot launch 合成）。
...
| 4 | llm-bridge | 🟢 **司令官サイクル S1 main 入り（#70）**: BridgeScheduler + HermesClient(/v1/chat/completions) + Situation 組立 + action_map dispatch + 排他 A/B-3/C。MCP 層(#35/#41) 済。**残(S2)**: Langfuse trace 所有(`langfuse.openai`, Issue #73) / `nav2_bridge` / 実 tool dispatch / Mode C None / R-35A(#54) |
...
| 6 | wo | 🟢 **KPI scaffold main 入り（#69）**: audit.jsonl reader + result/task_completion_time + Langfuse seam(NO-OP)。**残**: Langfuse 実配線(v4 `create_score`, Issue #73) / efficiency 他 score / Grok cost / Metrics+Experiments。trace_id は seed 導出で契約不要 |
...
| 8 | nav-traffic | 🟢 **TrafficManager/VirtualScan/nav2_params main 入り（#68）**: None/Simple + `/bot{n}/virtual_scan` + DWB→MPPI + twist_mux 移設(#40)。**残**: twist_mux source 削除 hand-off(#5) / 実 bot1/bot2 Gazebo E2E(#7 統合) / `bringup.launch.py` per-bot 合成(#1) |
...
- **（決定）Langfuse trace 所有 + provider access**（Issue #73 / #72）: trace 所有は **Bridge**（`langfuse.openai` + `base_url`=Hermes、Hermes 内蔵 Langfuse は無効化）。score は v4 `create_score`（v2 `langfuse.score` 廃止）。`trace_id`=32hex no-dash、#4↔#6 は `create_trace_id(seed)` 導出で契約不要、audit 突合は `gen_id`+timestamp。**Vertex AI SDK は不採用・Hermes 単一経路維持**（doc13 §7.5/§7.6）。

## 次の山
- **#4 S2（Langfuse trace 所有 + nav2_bridge + 実 dispatch）** — `langfuse.openai`(base_url=Hermes) で Bridge が trace 所有（Issue #73）。R-35(A) /stop は #54。
- **#8 nav-traffic E2E** — twist_mux 削除 hand-off(#5) + 実 bot1/bot2 Gazebo E2E(#7 統合) + per-bot launch 合成(#1)。
- **#6 wo Langfuse 実配線** — v4 `create_score` + efficiency/他 score + Metrics/Experiments（Issue #73）。
- **#2 jetson** — 実機不要で先行可。
- 既知の設計リスクは [07-research-notes](shared/07-research-notes.md) R-35〜R-52。

## git 衛生（現況, 2026-06-02）
- main = `origin/main`(`4fb0cd2`) 同期・クリーン。Phase 0.5 スライス **#65/#66（Mode C 契約）・#68（nav-traffic）・#69（wo）・#70（llm-bridge S1）・#71/#72（docs: dispatch 整合 / Langfuse v4 + Vertex 不採用）** を含め **すべて main マージ済**。
- **stale worktree 掃除待ち**: `mwr-contract-modec`/`mwr-docs-modec`/`mwr-docs-bridge-dispatch`（完了・削除）、`mwr-llm-bridge`/`mwr-nav-traffic`/`mwr-wo-metrics`（PR 済・各 session が次ラウンドで再作成）。
