...
| **OQ-OP2** | 操作者停止要求・明示解除の入力 topic の形（別 topic か同 topic payload か）→【2026-09-08 解消】単一 global topic `/operator/stop_request`（`std_msgs/String` JSON・同 topic payload 方式: `{"action": "engage"}`／`{"action": "clear"}`。不明・不正 payload は無視＝clear 扱いにしない。M1 単騎ゆえ per-bot 選択性は安全要件でない。PR #TBD） | doc03 additive 追記と同一 PR（済） | 高（順序 5 の前提） |
