...
| # | 未決事項 | 決め方 | 優先度 |
|---|---|---|---|
| **OQ-OP1** | **平常時も流れる Guardian 生存証明チャネル**（周期 publish の現在状態）を導入するか、#593 の level mirror を恒久受容するか（§3-2）。導入時の topic 名・型・周期・鮮度閾値は doc03 additive 追記と同一 PR。先行条件だった **doc03 への既存 `/bot{n}/cmd_vel/emergency` 追記は解消済**（doc03:113・#597＝[doc12:638](../architecture/12-infrastructure-common.md) 解消マーク）。**【2026-09-09 で範囲縮小】** §4-1 が **L0' 停止上乗せ用の常時 publish チャネル `/bot{n}/stop_state`**（topic 名・型・鮮度/期限規律・doc03 additive 行）を確定したため、残る問いは「**そのチャネルを L2 feed にも流用するか、level mirror を恒久受容するか**」＋**publish 周期**。「常時チャネルを作るか否か」自体はもはや論点ではない。**【2026-09-09 producer 実装で更新】** `/bot{n}/stop_state` の producer（Guardian 50ms tick 周期 publish）が land し、**publish 周期は Guardian 反射 tick に確定**（§10）＝残る論点は「L2 feed 流用か level mirror 恒久受容か」の一点のみ | fault injection（§8）で Guardian 死の実害を実測 → オペレーター裁定 | 中（CURRENT は暫定受容済み） |
...
- **#582（joy 鮮度）が満たせていなかった §6「再接続だけでは走行を再開しない」は、本スライス（`feat/teleop-estop`）で `teleop_joy` に latch＋再アーム（中立観測 → deadman rising edge）を実装して充足**（#582 のガードは mask であって latch ではなかった）。§5 の操作者非常停止も同じボタン操作で駆動し、standalone は teleop 自身のゼロ送出、統合構成は `/operator/stop_request` への engage/clear publish で成立する（既定 index・sentinel・joy_node 前提 = [mode-m1/03:52](03-joystick-teleop-bringup.md)）。あわせて mode-m1/03 への追記で割れる [jetson/02](../jetson/02-remote-access-and-dev-link.md) の `mode-m1/03` pin は **本スライスで :55 → :56 へ再 pin 済**（この形の pin は check_consistency の検査対象外＝CI では検出されない）。なお **§6 のうち運転モード購読（`:93` OQ-OP6）は本スライスでは未着手**（standalone 姿勢のまま＝deadman と本 latch が主ゲート）。
...
- **【2026-09-09 追記】停止上乗せの入力契約は §4-1 に確定**（topic `/bot{n}/stop_state`・doc03「Jetson 内部」表へ additive 1 行・consumer 側配線と R-26 のみ land）。**producer（Guardian 側 publisher）は本スライスで実装済**（`warehouse_safety` 所有・`emergency_guardian.py` が 50ms tick ごとに同一 tick の `decisions` から `stop_requested` を導出し全 bot へ**周期 publish**＝§4-1 準拠・既存 estop/level/event/operator latch 経路は不変更の additive）。これで `stop_overlay_enabled:=true` が実用可能になった（従来は feed 不在で常時停止側＝fail-closed）。**publish 周期 = Guardian の 50ms reflex tick**（§4-1「publish 周期は確定しない」に対する実装スライスでの確定。実値は Phase 1 実測の対象のまま）。**producer 再起動の裁定**: 同一 boot のプロセス再起動では `CLOCK_MONOTONIC` が継続するため `valid_until` watermark は逆行しない（§4-1「同一ホスト・同一 boot」前提で cross-boot は契約外＝別 boot では consumer が fail-closed 側に留まり、復帰手段は driver 再起動）。consumer 側 clip 検出（log/counter）は **monotonic producer では正常運用で非発火**（§4-1「静かに縮退」は壁時計 producer 限定の挙動）ゆえ **counter は追加しない**（§4-1 の「clip の log/counter は producer 実装スライスで足す」は、monotonic+整合窓の本実装では踏まないため実装せず＝理由による close）。**§4-1「本節が裁定しないこと」の forward-pointer（publish 周期・producer 実装・復帰手段）は §4-1 契約本文を凍結する本 PR では書き換えず、本 §10 エントリが supersede する**（契約は rate を固定しない・実装が本スライスで 50ms に確定、の意図的レイヤ分離は保つ）。**sim / `use_sim_time`（`/clock`）は本スライスの scope 外**（§4-1 が producer スライスへ委ねた点）: producer は `time.monotonic()` 無条件使用で、`/clock` 下では consumer と時計ドメインが食い違い overlay は恒久 fail-closed（安全側）。実機 room-scale（ADR-0009）では実害小・sim 対応は後続。**OQ-OP1 の本題（L2 feed を level mirror のまま恒久受容するか）は §4-1 / 本スライスでは裁定していない**（§9 の表は未決のまま）。
- **本スライス（`feat/teleop-estop`）自身の残件（隠さない）**: ①**node 配線層の AST pin unit が未整備**（param 名・topic・QoS・publish 条件が unit/CI のどちらにも乗っていない＝上の #593 残余 follow-up ①と同型。先例 = `tests/unit/test_speed_band_bringup_wiring.py`）。②**param epoch が非対称**——`_estop_cfg`（ボタン index・chord・deadzone）は**起動時スナップショット**、`joy_to_twist` に渡す軸・cap は**毎サンプル live 読み**。`read_only` 宣言で epoch を揃えるのは次スライス送り。③**Guardian 再起動時に teleop 側 latch と Guardian 側 latch が乖離する窓**——teleop は engage を rising edge でしか publish せず level 再送しないため、latch 中に Guardian が落ちて上がると Guardian 側だけ停止理由が抜ける（経路 A＝teleop 自身のゼロ送出は効き続ける）。
...
- **用語**: [GLOSSARY.md §11](../GLOSSARY.md) — 停止上乗せ / 操作者非常停止要求 / 運転モード / **停止状態フィード（`/bot{n}/stop_state`）・期限 watermark（deadline watermark）・許可窓上限（`stop_state_max_validity_s`）**（いずれも本書 §4-1 と双方向・**2026-09-09 producer 実装スライスで追補＝旧「張り残し」を解消**）
