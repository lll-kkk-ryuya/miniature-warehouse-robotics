...
| [0012](../adr/0012-speed-band-no-l2-best-effort.md) | 速度帯（[04](04-runtime-speed-limiter.md) の②）は **L2 Policy Gate 非経由**の **best-effort 制御面**（最小安全方針の docs 初出・04 §6 の OQ-R1〜R7 一括裁定）。クランプ `min(帯値, ①, MAX_LINEAR_VELOCITY)`（①=launch が MPPI へ注入する解決値を同一 launch から publisher param で受ける）・**20Hz 周期送出**・`percentage=false`・publisher は `gesture_detector` 同居（package 名は 09 OQ-13 従属）。hard 安全床（①・凍結契約・L0'）は不変 | accepted（2026-08-30・スライス 1 実装済＝publisher node + R-26 unit・配線未） |
...
- [04](04-runtime-speed-limiter.md) = OQ-T3（走行中の速度上限変更）の設計解 doc を同ラウンド（2026-08-28）で新設。経路選定確定 → **実装スライス 1（2026-08-30）で publisher node + R-26 unit まで実装済**（Nav2 側配線は未＝下記スライス 1 行）。
- 「## 関連 ADR」索引を新設（2026-08-28）。**ADR 本体は [docs/adr/](../adr/README.md) から移動・複製しない**方針を明文化し、本ツリーは分野別ビュー（索引）のみを持つ。
- [04](04-runtime-speed-limiter.md) §6 の OQ-R1〜R7 は **[ADR-0012](../adr/0012-speed-band-no-l2-best-effort.md) で一括裁定**（2026-08-30・L2 非経由・帯 = best-effort 制御面）。残未決 = OQ-T1（最速段実値）/ OQ-T2（帯遷移時間窓）/ `V_FLOOR` / 帯遷移過渡の実測回帰。実装スライス 1（2026-08-30）: `warehouse_perception` 新設（OQ-13 裁定）＋ publisher node ＋ R-26 unit まで実装済——残スライスは [04 追補②末尾の統一リスト](04-runtime-speed-limiter.md)（bringup 配線〔アサート・`reset_period` 含む〕・帯 config 実値・過渡実測回帰）。
