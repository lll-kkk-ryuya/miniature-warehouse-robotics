...
| [0012](../adr/0012-speed-band-no-l2-best-effort.md) | 速度帯（[04](04-runtime-speed-limiter.md) の②）は **L2 Policy Gate 非経由**の **best-effort 制御面**（最小安全方針の docs 初出・04 §6 の OQ-R1〜R7 一括裁定）。クランプ `min(帯値, ①, MAX_LINEAR_VELOCITY)`（①=launch が MPPI へ注入する解決値を同一 launch から publisher param で受ける）・**20Hz 周期送出**・`percentage=false`・publisher は `gesture_detector` 同居（package 名は 09 OQ-13 従属）。hard 安全床（①・凍結契約・L0'）は不変 | accepted（2026-08-30・実装未着手） |
...
- [04](04-runtime-speed-limiter.md) §6 の OQ-R1〜R7 は **[ADR-0012](../adr/0012-speed-band-no-l2-best-effort.md) で一括裁定**（2026-08-30・L2 非経由・帯 = best-effort 制御面）。残未決 = OQ-T1（最速段実値）/ OQ-T2（帯遷移時間窓）/ OQ-13（package 名）/ `V_FLOOR` / 帯遷移過渡の実測回帰。実装（publisher node・R-26 unit 6 本）は依然未着手。
