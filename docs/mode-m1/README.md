...
| [0013](../adr/0013-stm32-command-stream-watchdog.md) | **W-3（MCU communication watchdog）を vendor V3.6.5 ソースへの additive 追記で実装する**方向（[02:64](02-m1-driver-and-watchdog.md)「埋められない」↔ doc10 G-g「追加せよ」の衝突解消）。clamp・IK 不変・前提ゲート 4 点（U-5 / ライセンス / stock hex 書き戻し実証 / G-g 抜線試験）通過まで書き込まない。W-4 緩和は G-h で別裁定 | **proposed**（2026-09-08・オペレーター裁定待ち） |
