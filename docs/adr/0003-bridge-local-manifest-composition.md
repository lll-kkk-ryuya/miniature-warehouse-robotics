...
3. **startup fail-closed preflight**：`manifest.plugins ⊆ registered hookimpls` でなければ起動拒否（`CompositionError`）。silent pass 経路を持たない。
