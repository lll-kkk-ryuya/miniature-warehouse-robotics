...
1. **A案採用** = manifest resolution 層が `compile_raw_output` に構成を供給する（[`pipeline.py`](../../ws/src/warehouse_llm_bridge/warehouse_llm_bridge/robotics_planning_core/pipeline.py):90 は既に `resolver_policy` / `context` / `compiler` を param injection で受ける）。**B案（box-body DI container）は却下**。
...
- frozen validator seam（非改変・検証側）: [`report.py`](../../ws/src/warehouse_llm_bridge/warehouse_llm_bridge/robotics_planning_core/validator/report.py):69-88（9-code `ValidationCode`）・:184（`from_rules`）・:100-105（`_STATUS_PRIORITY` lattice）/ 供給 seam: [`pipeline.py`](../../ws/src/warehouse_llm_bridge/warehouse_llm_bridge/robotics_planning_core/pipeline.py):90（`compile_raw_output` の resolver / compiler / context 注入点）
- 実装ステータス（2026-07-07 更新）: composition 層は **`origin/main` に着地済み**（PR #403-#407 ＋ residual #410-#413・`robotics/composition/` 11 module＝`manifest`/`loader`/`preflight`/`record`/`profile`/`calibration_gate`/`plugins`/`plugin_results`/`plugin_manifest`/`fixtures`/`__init__`）。稼働 Bridge cycle への配線＝XER6 pending は不変（caller 契約＝[mode-x-er/08](../mode-x-er/08-x-er-bridge-node-spec.md)）。想定 vs 現状の追跡は [mode-x-er/07 §L3 composition 層](../mode-x-er/07-implementation-status.md)（matrix row）。
