...
| ADR | 決定 | 状態 |
|---|---|---|
| [0005](0005-isaac-sim-as-verification-gate.md) | Isaac Sim を映像用オプションから**投入前検証ゲート（Mode X-ER/VLA から必須）＋ fixture 工場**へ格上げし、**映像用途のみ optional** として残す。司令官入力が pixel になると sim レンダリング＝テスト入力。Mode A/C（situation JSON 入力）は Gazebo で足りる | accepted |
