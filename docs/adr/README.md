...
| ADR | 決定 | 状態 |
|---|---|---|
| [0012](0012-speed-band-no-l2-best-effort.md) | 速度帯（ジェスチャ速度セレクタの runtime 経路 = [mode-m1/04](../mode-m1/04-runtime-speed-limiter.md) の②）は **L2 Policy Gate 非経由**の **best-effort 制御面**とする（**最小安全方針の docs 初出**・04 §6 の OQ-R1〜R7 一括裁定）。帯は安全機構ではなく hard 床（①起動基準値・凍結契約・L0'）は不変。クランプ `min(帯値, ①, MAX_LINEAR_VELOCITY)`（①=launch 注入と同一解決値）・**20Hz 周期送出**・`percentage=false`・単一 publisher 規律。根拠 = 3 レーン敵対的検証（Nav2/エコシステムに緩和裁定機構が不在・L2 経由は rate limit / `active_tasks` 帳簿を破壊・Humble MPPI では帯値が厳密上界にならない） | accepted（2026-08-30・実装未着手） |
| 0011（予約） | **ros2_control 採否** — 未裁定・未起票。[mode-m1/02 §5](../mode-m1/02-m1-driver-and-watchdog.md) と [mode-m1/README §関連 ADR](../mode-m1/README.md) が本番号で先行参照しているため番号を予約（「最大番号 +1」規約の明示的例外。起票時に本行を実 ADR 行へ置換） | 未起票 |
