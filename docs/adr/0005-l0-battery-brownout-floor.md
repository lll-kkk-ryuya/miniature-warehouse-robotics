...
- 本 ADR は [adr/README](README.md) から back-link される。[GLOSSARY.md](../GLOSSARY.md) の safety 語群（`command-stream watchdog（comms-loss deadman）` エントリ直後）に正準語「**L0 voltage brownout floor（over-discharge floor）**」を**追補済**（percent `battery_is_critical(pct)` とは別名・非対称である旨・本 ADR へリンク）＝**ADR↔GLOSSARY 双方向**リンクで閉じた。当初は並行 firmware PR（#476・`docs/firmware-phase1-decisions`）との GLOSSARY 同時編集衝突回避で defer していた residual を、#476 land 後に本追補で解消した。
