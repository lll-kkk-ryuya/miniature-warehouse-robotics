...
- **完全な電源断（USB 抜き → メインスイッチ OFF 5 s → ON → USB 挿し）をまたぐ `car_type = 0x0A` の保持を 2026-09-10 17:37 JST に確認**（MCU リセット後の `m1_probe` 読み戻し **10**・FW 3.6・電池 12.3 V・低電圧ラッチ無し＝P-9d の起動順序どおり）。`# TODO(実機)` は close。
