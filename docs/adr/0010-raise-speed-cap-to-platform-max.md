...
## 【2026-08-30 追補】Decision 3 の前提条件「結線と R-26 pin」は #550 で充足（Status・Decision の行は不変）

Decision 3（:24）の「現状未結線」と References（:74）の同旨は**決定時点（2026-08-19）の記述**として保存する。その後の変化のみ記録する:

- **前提条件は PR #550（`93bfc93`・2026-08-26 merge）で充足した**: `m1_driver` serial driver slice が land し、dispatch 経路が `clamp_body_velocity` を必経（`M1DriverCore.on_cmd_vel`・R-26 pin = `tests/unit/test_m1_driver_core.py`）＝[mode-x-er/10 §11 G-l](../mode-x-er/10-room-scale-safety-review.md) land 済。
...
- **処置（2026-09-10 12:55 頃）**: `Rosmaster_Lib.set_car_type(10)`（`FUNC_SET_CAR_TYPE=0x15` + `SAVE_VERIFY=0x5F` → firmware 通常パーサ `Upper_Data_Parse` は `Motion_Set_Car_Type` + `Flash_Set_CarType` のみ＝**MCU リセット無し・即時反映**。`Bsp_Reset_MCU` を伴うのは低電圧ラッチ中の縮小パーサ `Upper_Data_Parse_Low_Battery` 経路だけ）で flash を **`0x0A`（`CAR_MECANUM_M1`）** へ書換えた。**書換え後の読み戻し = 10**・battery 12.6 V・version 3.6。
- **運用規則（ドライバ param）**: flash は永続するので、`m1_driver` の ROS param **`car_type` は既定 `-1`（＝送らない）のまま運用する**。非負値を渡すと `RosmasterBackend.__init__` が**起動のたびに** flash 書換えを繰り返す（通常パーサでは MCU リセット無し・動作は変わらないが、永続する値を毎回書き直す意味が無く flash 書込み回数だけ増える）。
- **`get_car_type_from_machine()` の `-1` は証拠にならない**: vendor lib 3.3.9 の実装は 20×1ms しか待たずタイムアウトで `-1` を返す＝**偽 `-1` は普通に出る**。判定はリトライして非負値を得てから行う。

**§Open 1 の残り（未決）と、本追補で閉じた項目**:

- `# TODO(実機・現物)` **520 モータラベルの目視確認**と**メカナムホイール径 80mm 相当のノギス実測**は未実施（`m1_probe` では読めない＝現物作業）。
- ✅（閉じた）**完全な電源断（USB 抜き → メインスイッチ OFF 5 s → ON → USB 挿し）をまたぐ `car_type = 0x0A` の保持を 2026-09-10 17:37 JST に確認**（MCU リセット後の `m1_probe` 読み戻し **10**・FW 3.6・電池 12.3 V・低電圧ラッチ無し＝P-9d の起動順序どおり）。`# TODO(実機)` は close。

**結論**: §Open 1 のうち **FW 版 / car_type の半分は閉じた**（3.6 / `0x0A` へ確定）。一方 Decision 1（:22）の contract PR は、残る現物確認（モータラベル・ホイール径。電源断保持は上記のとおり 17:37 に確認済）と **§Open 2（:64 S-SPEED 実測）**、および **Decision 5（:26-29 派生再導出）** の DoD 充足を待つ。**契約値は 0.3 のまま不変**（`warehouse_interfaces.safety.MAX_LINEAR_VELOCITY`）＝本追補にコード変更は無い。
