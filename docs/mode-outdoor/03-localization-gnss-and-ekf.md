...
| 判定 | `pose_age` が閾値超 **かつ** 変位ゲート開 → `pose_stale` estop | [guard_logic.evaluate 規則 (4)](../../ws/src/warehouse_safety/warehouse_safety/guard_logic.py)（`pose_age > pose_freshness_timeout` ∧ `pose_gate_open`・symbol 参照）[D] |
| 破綻の形 | `pose_age` は最初の pose まで `None`（[guard_logic.BotState.pose_age](../../ws/src/warehouse_safety/warehouse_safety/guard_logic.py)・symbol 参照）で `is not None` ガードがある → AMCL が居ないと **estop は一度も発火せず、ガードは恒久沈黙**（誤発火ではなく無音化） | [D]→[I] |
...
- [emergency_guardian.py `EmergencyGuardian.__init__`（`/{bot}/amcl_pose` 購読・`pose_freshness_timeout`）／`_bot_state`（`pose_age`）](../../ws/src/warehouse_safety/warehouse_safety/emergency_guardian.py) / [guard_logic.py `BotState.pose_age`／`evaluate` 規則 (4)](../../ws/src/warehouse_safety/warehouse_safety/guard_logic.py)（symbol 参照・行 pin しない） / [config/warehouse.base.yaml:21](../../config/warehouse.base.yaml:21)
