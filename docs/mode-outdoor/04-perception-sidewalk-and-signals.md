...
- **判定は tick で 1 回**。`SensorHealthMonitor.evaluate` は `health_epoch` を進める副作用を持つ（[sensor_health.py:345](../../ws/src/warehouse_safety/warehouse_safety/sensor_health.py:345)）ため、`_bot_state` で **bot ごとに 1 回だけ**呼ぶ。`_check_safety` 側には足さない（`gl.evaluate` の呼び出しは従来どおり tick に 1 回）。
...
- **置き場の理由 ①（gate の隣に置く）**: 本 health は producer（`terrain_publisher`）・consumer（`collision_monitor` の `cliff_scan` source）と**同じ `perception.terrain.enabled` で arming される**（[追補 ⑩ :1048](04-perception-sidewalk-and-signals.md:1048)「ON/OFF の真実は本キー 1 つ」）。しきい値をその gate の直下に置くと、「何を true にすると何が要るか」が 1 ブロックで読める。**意味的な所有は X2 / Guardian（L1）**であって `terrain_publisher` の param ではない——その区別は base config のコメントに明記した。
...
- `OQ-OD4Y-o4` **モード別の必須 source 集合** = `OQ-OD91`（[09:251](09-external-review-v3-response.md:251)）。AUTO / REMOTE / MANUAL で何が必須かは未裁定で、本 PR の必須集合は `{cliff_scan}` 固定。`scan` を同じ monitor に入れて `scan_stale` と統合するか（鮮度語彙の統一）も同時に決まる論点。
