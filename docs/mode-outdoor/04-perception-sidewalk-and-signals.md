...
- mutation（実測・**10/10 KILLED**・実ファイル差替＋`git diff --quiet` で復元検証）: `ABSENT` を無条件 estop（odom 証人を落とす・2 赤）／証人規則を全 verdict へ拡大（証人が無ければ無音・10 赤）／`INVALID` を健康扱い（非 OK の allow-list 化・15 赤）／未知の綴りを fail-OPEN（既知 4 名のみ estop・7 赤）／gate を無視して常に購読・常に monitor 生成（1 赤）／しきい値を `.get` 既定で埋める（1 赤）／callback 内で `evaluate`（1 赤）／`_bot_state` が時計を読み直す（2 赤）／base config に実値を入れる（1 赤）／source ラベルを `guard_logic` 内で発明（2 赤）
