...
`:261` が別 PR に送った運用値（k = 1.875・径 0.150・`lateral_enabled` false・`odom_enabled` true＝上表 `:252`-`:256` の「150 mm での値」列）の実体は **[`ws/src/warehouse_bringup/config/m1_wheel_plain150.yaml`](../../ws/src/warehouse_bringup/config/m1_wheel_plain150.yaml)**（bringup 所有の新規 1 ファイル）。**明示的に `--params-file` で渡したときだけ効く**（launch 自動注入なし＝driver 既定は stock 80 mm のまま bit 等価）。**適用条件は「150 mm を物理装着した」の 1 つだけで、装着したら直ちに渡す**——未適用のまま 150 mm で走るのが 1.875 倍の fail-open（`:252`）だからである。**本 doc §10 の G-W ゲートは「適用の条件」ではなく「出てきた odom の使用制限」**: G-W1 → `track_m`／G-W4 UMBmark → k・`yaw_scale`／`m1_probe` → `wheel_signs`（`:270`）が揃うまで odom は PROVISIONAL＝bring-up 観測に限り、航法に使わない。**G-W4 は k を校正する試験なので、profile を適用しないと実施できない**（適用はゲートの前提であって結果ではない）。`yaw_scale`（`:253` ＝ 値でなく「実測」）・`track_m` / `wheel_signs` / 共分散（`:257`-`:258` の **PROVISIONAL**）は転記せず driver 既定に残す（「書いてある＝測った」の誤読・二重更新箇所・実測値を既定へ入れた後の**静かな 1.0 上書き**を作らないため）。144 mm 退避（`:243` / `OQ-OD77`）は同ファイルの 2 値のみ変更（k 1.8 / 径 0.144）で、片側だけの編集は R-26 unit（`tests/unit/test_m1_wheel_plain150_profile.py`）が赤にする。詳細・コマンド・理由は [mode-m1/02 追補③](../mode-m1/02-m1-driver-and-watchdog.md)（起動手順は [mode-m1/03 追補](../mode-m1/03-joystick-teleop-bringup.md)）が正本。
