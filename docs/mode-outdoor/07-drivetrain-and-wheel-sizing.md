...
【2026-09-14 追記】`OQ-OD76` **裁定 = A'（シシク PU-150 ＋ Pololu #2674）**。実測（軸径 ≈ 6・側板外面→軸端 ≈ 19・D 面 15）で #2674 の取付は確定（[06 ④-7](06-hardware-delta-and-base-selection.md)）。車輪側の 4 点確認と確定購入リストは [06 ④-8](06-hardware-delta-and-base-selection.md)。退避 = Amazon 6 インチ 車いす前輪（608ZZ）→ #3281。
