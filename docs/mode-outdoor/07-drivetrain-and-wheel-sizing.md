...
作成日: 2026-09-12
Status: **記載済（設計値・一次情報つき・実測未）**。2026-09-12 のエージェントチーム 4 レーン（法規／駆動系物理／駆動方式・車体／M1 実機制約）の報告を統合し、数値は本 doc 執筆時に**再計算**、引用は**再 Read** した（[D]=一次情報を執筆者が実 Read・[L]=調査レーン報告で URL あり（執筆者未再読）・[I]=推論・計算）。実機の実測（§10）で確定するまで**設計値**として扱う。
...
- stock ファームの各輪 clamp `CAR_M1_MAX_SPEED = 700`（FW 換算 mm/s）は**車輪 167 rpm** に相当し、実速度は `π·D·167/60`。これが 6.0 km/h に達する径は **190 mm**（§3）。しかし ① 前後輪の軸間距離（推定 185 mm・§7）で入らない、② 9.6 V ではモータ無負荷が 164 rpm しか回らず 167 rpm の clamp に届かない、③ 牽引力は `1/D` で落ちる。→ **FW を触らない前提の実用最大は 150 mm（6 インチ）**: 最大設定 **4.7 km/h**・9.6 V 負荷時 **4.3 km/h**・満充電無負荷 **6.1 km/h**（＝解釈「満充電でも物理的に 6 km/h を超えない」の径 148 mm とほぼ一致）。**目標 ≥4 km/h は満たすが、6 km/h ギリギリにはならない。**
...
**推奨**: **Phase 1 = 150 mm 通常輪・1:56 据え置き・stock FW・M1 car_type ＋ ホスト側車輪スケール k**（4.3〜4.7 km/h・構造上 6 km/h 未満の根拠が MCU 側に残る）。**Phase 2（任意・ADR 要）= FW 定数修正 ＋ 1:40 L 型で 6 km/h**（[ADR-0013](../adr/0013-stm32-command-stream-watchdog.md) の additive fork と同じツールチェーン課題を共有＝§8-5）。
...
| **1:56** | **150** | **1.313 = 4.7** | **6.1** | 5.8 | 4.6 | **4.3** | **○** | ✗ |
...
**読み方**: 1:56 のまま径を上げる限り、**clamp 列が 6.0 を超えるのは 190 mm 超だけ**＝「構造上 6 km/h を超えられない」根拠は MCU 側に残る。150 mm では最大設定が 4.7 km/h、電池が警報電圧まで落ちても 4.3 km/h。
...
**結論**: 1:56 のまま (a)(c) を満たす径（190〜210 mm）は (d) で入らない。**(b)(d) を同時に満たす最大 = 150 mm** が「法律ギリギリ径」の実用解であり、その最大設定は 4.7 km/h。6 km/h を**維持**したいなら径ではなくモータ回転数（1:40 L 型）と FW 定数の側で解く（§9 案 C）。
...
| **150 mm** | 75 | **0.27** | 0.67 | **0.75 N·m** | **定格超・停動未満 → 助走 ＋ 4WD（後輪の押し）前提・実測ゲート** |
...
|---|---|---|---|---|---|---|
| **A. 1:56 + 150 mm 通常輪・stock FW・M1 type**（**Phase 1・推奨**） | ホスト L0' に車輪スケール k = 150/80 = 1.875（送信 `v / k`・odom は `0x0D` 生カウント × 真の周長）。契約 `MAX_LINEAR_VELOCITY` を実単位で再 pin | **4.7 / 4.3 km/h** | **FW clamp（MCU 側）＋ 物理（満充電無負荷 6.1）** | 定格超・停動未満（助走 + 4WD 前提） | 車輪・ハブ・外部軸受・バンパ処理・k の実測校正・R-26 unit・contract PR | ◎ |
| A'. 1:56 + 160 mm | 同上・k = 2.0 | 5.0 / 4.6 | FW clamp（6.0 まで余裕 1 km/h） | 同上（T 0.78 N·m） | 同上 + 前後隙間 25 mm・バンパ 30 mm 超え | ○（現物次第） |
...
**推奨**: **A を Phase 1** とする（4 km/h 目標を 9.6 V まで満たし、法定「構造上 6 km/h 未満」の根拠を MCU 側に残せる唯一の低コスト解）。**「法律ギリギリ（6 km/h）で走る」が要件になった時点で C を ADR で裁定**する（ADR-0013 の toolchain 課題と束ねる）。B は採らない。
...
- `OQ-OD71` 車輪スケール k を L0' のどこに持つか（`m1_driver` param か凍結契約の定数か）。契約 `MAX_LINEAR_VELOCITY` の再 pin 値（案 A: 実単位 1.31 m/s 候補）と ADR-0010「platform max」の意味の再定義（[00 §4 行 7](00-mission-and-scope.md)）。
