...
| producer / consumer | `teleop_joy` → Emergency Guardian（[emergency_guardian.py `_on_operator_stop`（`/operator/stop_request` 購読・symbol 参照）](../../ws/src/warehouse_safety/warehouse_safety/emergency_guardian.py)）[D] | Guardian（[`_bot_state`（latch を全 bot の `operator_stop_requested` へ）／`_publish_stop_state`（`/bot{n}/stop_state`）](../../ws/src/warehouse_safety/warehouse_safety/emergency_guardian.py)）→ `m1_driver`（[stop_state.py:30](../../ws/src/warehouse_m1_driver/warehouse_m1_driver/stop_state.py:30)）[D] |
