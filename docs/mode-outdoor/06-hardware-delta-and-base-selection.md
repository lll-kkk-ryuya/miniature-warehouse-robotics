...
| 車輪 | **決定（2026-09-12・ユーザー）: 換装前提・最低 4 km/h（歩行速度）** → §2 | 140 mm 級（保守基準で ≤147 mm）の通常タイヤ（減速比 1:56 のまま）。ホイールアーチ干渉＝加工要 |
