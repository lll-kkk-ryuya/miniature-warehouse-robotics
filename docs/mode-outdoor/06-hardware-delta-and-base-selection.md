...
| 車輪 | **決定（2026-09-12・ユーザー）: 換装前提・最低 4 km/h（歩行速度）** → §2 | 140〜150 mm の通常タイヤ（減速比 1:56 のまま）。ホイールアーチ干渉＝加工要 |
...
- **目標: タイヤ換装で最低 4 km/h（歩行速度）**。モータは MD520Z56（205 rpm / 12 V・6.5 kg·cm・6 mm D 軸）。**減速比 1:56 のまま 140〜150 mm の通常輪で成立**: 140 mm では満充電無負荷 5.7 km/h（構造上 6 km/h 未満）・FW clamp 相当 4.4 km/h・9.6 V 時 4.3 km/h。127 mm は満充電時のみ 4 km/h。**1:30 への減速比変更は採らない**（低電圧で 4 km/h を割る＋トルク半減）。
- 車体側は**ホイールアーチに干渉**するため加工が要る（説明書表紙画像で確認・実物採寸は `# TODO(実測)`）。
...
- **契約への帰結**: FW clamp が実単位の cap でなくなるため、**L0' ホスト clamp が唯一の実 cap**になる。[ADR-0010](../adr/0010-raise-speed-cap-to-platform-max.md)「platform max」の意味と [ADR-0012](../adr/0012-speed-band-no-l2-best-effort.md) の帯上限は再導出が要る（contract PR・[00 §4 行 7](00-mission-and-scope.md)）。
