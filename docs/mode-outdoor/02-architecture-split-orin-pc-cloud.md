...
- **実体**: `ws/src/warehouse_teleop/warehouse_teleop/operator_link_logic.py`（純 stdlib）。unit は `tests/unit/test_operator_link_logic.py`（182 ケース・うち 66 が safety マーク）＋ AST pin `tests/unit/test_operator_link_boundary.py`（9 ケース・**import allowlist 形**・先例 `tests/unit/test_web_bridge_noactuation.py`）。produce/consume と API の記録は [`ws/src/warehouse_teleop/CLAUDE.md`](../../ws/src/warehouse_teleop/CLAUDE.md)「2026-09-16 追記」節。
...
- **独立レビューで塞いだ 4 点（2026-09-16・初版から同ブランチで修正）**: ①**セッション終了が 1 か所では足りない**——`reconnect()` はキュー内の駆動指令しか捨てられず、1 tick 前に取り出し済みのサンプルが前セッションのまま走れた。`replay_teleop` に **必須の `current_session_epoch`** を足し、`reconnect()`・`replay_teleop`・`VideoFreshness.reset()` の**3 か所**で効かせる形にした（[09 §2-d](09-external-review-v3-response.md:104) の破棄はこの 3 点セットで初めて閉じる）。②**映像 stamp の単調順序だけでは防御にならない**——`1e308` を 1 枚受けると「永遠に最新」になり以降の実フレームが全部捨てられ channel が STALE で固着した。壁時計からの **skew 上限**（注入値）で未来日付を*受理せず拒否*し、送信側再起動には `reset()` で復帰する。③**AST pin を deny リストから allowlist へ**——`socket`・`threading`+`http.client`・`sensor_msgs.msg` が素通りしていた。純関数ライブラリだという主張は「これだけ・他は無い」としか強制できない。④**拒否された message が `sequence` を進めない**ことを unit で固定（進めてしまうと、拒否された 1 通が後続の正当な指令の番号を焼いて replay 扱いにする）。
- **残件**: (a) wire JSON（kind 名・キー名・payload 形）は **[提案・未凍結]**——doc03 契約カタログにも `warehouse_interfaces` にも未登録。凍結は additive-first の contract-PR で行う。(b) `reason_code = link_loss`（[05 §3-2](05-safety-envelope-and-intervention.md:51)）は **[doc03:112](../architecture/03-software-architecture.md:112) の凍結 payload に載る場所が無い**（`{"action": ...}` のみ）ため in-process の verdict に留め、wire へは engage だけを出す。reason を運ぶ合流 topic は [05 §3-2](05-safety-envelope-and-intervention.md:57) の `/safety/stop_request` 案＝`OQ-OD53` 裁定待ち。(c) 承認トークンの照合失敗時に token を burn するか（anti-probing）は未決＝現状は**成功時のみ burn**。burn 済み集合は期限切れエントリを掃いて**有界**にしたが、**壁時計が burn 済み token の期限より手前へ巻き戻る**と再消費を許しうる（margin を発明せず残件とする）。(d) **`sequence` の意味論が未凍結 [提案・未凍結]**——[09 §2-a](09-external-review-v3-response.md:62) は「単調増加」としか言わず、**kind ごとの別カウンタか単一共有カウンタか**も**跳躍幅の上限**も定めていない。現実装は**セッション単位の単一共有カウンタ**を前提とするため、peer が kind 別カウンタを使うと heartbeat が番号を進めるたび teleop が replay 扱いで飢え、巨大値を 1 通 accept するとそのセッションは reconnect まで実質ロックされる。contract 凍結時に決める。
