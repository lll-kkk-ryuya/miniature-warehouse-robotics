...
| **昇圧 DC-DC 150W**（入力 10–32V / 出力 12–35V 可変・自然空冷 100W） | 12.6V→**19V** | 🚚 **発送済 2026-08-20・着荷は現物確認待ち**（2026-08-19 発注・予定 8/21–22。8/23 に出品者評価依頼メール＝着荷済みの公算大だが配達完了通知は未確認 `# TODO(現物確認)` → 末尾追記 P-2。**未着扱いなのは本項だけ**＝下2行の「未購入」とは別） |
...
Amazon 注文確認メールの履歴: **8/19 注文 → 8/20 発送済み → お届け予定 8/20–22 → 8/23 に出品者評価依頼メール**。評価依頼は通常配達後に送られるため**着荷済みの公算が大きい**が、**配達完了通知そのものは確認できていない**＝`# TODO(現物確認)`。

現物が確認できた時点で `:442` の「未着扱いなのは本項だけ」が解消し、`:422-428` の配線設計が要求する部材（T型コネクタ・WFR-3・ヒューズホルダ＋10A ヒューズ・DC プラグケーブル・テスター。いずれも 8/20–21 着荷済＝[01](01-budget-and-procurement.md) §B）が**全て揃う**。すなわち `:451` の①〜④（Orin 非接続で 19.0V に追い込む）に着手できる状態になる。**着手可能になっても `:451` の順序（初通電前の導通・＋/−分離 → ① 出力測定 → ② 19.0V → ③ 再確認しポット固定 → ④ 極性確認 → Orin 接続）は飛ばさない。**
