...
**発注済**: Amazon.co.jp 注文 `249-3401070-6986233`（2026-08-05・¥67,527）。商品名「Yahboom Jetson Nano B01搭載 ROS2ロボット …**Superior without Nano**」＝ `Superior-without / NANO 4GB SUB`（公式 sku 3000200910）。**着荷済（2026-08-19 開梱確認: T-mini Plus LiDAR・Nuwa-HP60C 同梱を実物確認）**。
...
| **T型（ディーンズ）コネクタ オス＋メス ＋ WAGO WFR-3BP レバーコネクタ**（3本用・8個入） | バッテリー分岐の自作。**ワイヤー付き**T コネクタで裸線端が出るため、WFR-3（単線・より線 0.14–4.0mm² 対応＝14AWG 可）で結線＝**はんだ不要**。＋側／−側で各1個使う。`# TODO(到着後)` **定格電流**の一次情報を確認——分岐ノードは `:427-428` の図のとおり**ヒューズより上流＝無保護で総電流**（拡張ボード側ピーク 6A（`:318`）＋ Orin 系統最悪 7.5A（`:433`）≈ 13.5A）を通す | ✅ **調達済**（ワイヤー付き 14AWG 約30cm オス／メス 各1・予備に裸コネクタ10個・WFR-3BP 8個入。2026-08-21 着荷。挿入前に規定の被覆剥き長さを確認） |
...
| **テスター（マルチメータ）** | **昇圧出力を 19.0V に設定・確認**（下記警告） | ✅ **調達済**（HIOKI 3244-60 カードハイテスタ・日本製。2026-08-20 着荷） |
...
> ⚠️ **手順厳守**: 採用した昇圧モジュールは**多回転ポテンショメータで 12〜35V に可変**であり、**出荷時の設定値は不明（35V の可能性がある）**。Orin の入力上限は 20V。**① 12V を入れ Orin を繋がずにテスターで出力を測る → ② 19.0V に合わせる → ③ 再確認しポットを固定 → ④ その後で初めて Orin を接続**。この順を飛ばすと Orin を破壊する。テスターが必須なのはこのため。
