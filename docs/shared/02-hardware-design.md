...
| **テスター（マルチメータ）** | 3役: **昇圧出力を 19.0V に設定・確認**（下記警告）＋**通電前の DC プラグ極性（センタープラス `:307`）確認**＋**ハーネス導通確認** | ✅ **調達済**（HIOKI 3244-60 カードハイテスタ・日本製。2026-08-20 着荷） |
...
> ⚠️ **手順厳守**: 採用した昇圧モジュールは**多回転ポテンショメータで 12〜35V に可変**であり、**出荷時の設定値は不明（35V の可能性がある）**。Orin の入力上限は 20V。**① 12V を入れ Orin を繋がずにテスターで出力を測る → ② 19.0V に合わせる → ③ 再確認しポットを固定 → ④ その後で初めて Orin を接続**。加えて ④ の前に、**組み上げたハーネスの導通と DC プラグ極性（センタープラス）をテスターで確認**する——無はんだ構成では極性が結線作業に依存し、**逆接も Orin を破壊する**。この順を飛ばすと Orin を破壊する。テスターが必須なのはこのため。
