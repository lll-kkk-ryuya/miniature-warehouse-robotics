...
10. **【一部解決】小項目**: 公式パラメータ表の実見（2026-08-05）で **USB-serial = CH340**（→ udev は `1a86:7523`）・**IMU = ICM20948**・**通信 115200bps** が確定。公式 STM32 ソース V3.6.5 で **M1 用 `car_type=0x0A`** も確定した。実機確認済（2026-09-10・P-8-3 `:888`）: 搭載 FW は **3.6**（V3.6.5 系と major.minor 一致）、`get_car_type_from_machine()` は**工場値 `0x02`（X3 PLUS）を返した**ため同日 `set_car_type(10)` で **`0x0A`** へ書換え・電源断保持も確認（[ADR-0010](../adr/0010-raise-speed-cap-to-platform-max.md) 2026-09-10 追補）。残るのは **MCU auto-report が 40ms=25Hz 固定**である点の Nav2 チューニング（`controller_frequency` / AMCL 更新レート）への影響。
