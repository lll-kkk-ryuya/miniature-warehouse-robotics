...
| 5 | repo clone `/ssd/warehouse` → symlink **`/opt/warehouse`**（規約パス＝[setup/jetson-deploy.md:47](../setup/jetson-deploy.md)） | `v0.x` タグ未発行のため **main SHA 固定が暫定**（タグ発行後にタグ固定へ）。ws **17 pkg `colcon build` 成功**（`eval_sdk` 込み。2026-08-30 の手打ち build は 16 pkg／**`build.sh` 初回 2026-09-10 16:00 `build_id c79d34c-20260910T160050-dev`〔profile=dev＝`v0.1.0` タグ未発行の暫定〕・17 pkg・17.2 s・exit 0・`forced=false`・`install/.mwr-build-info.json` と `log/build-info/` 履歴を確認**）・`ros2 pkg list` で warehouse 15 pkg 可視（2026-08-30 時点の値・現 ws は `warehouse_*` 16＋`eval_sdk`）・`clamp_body_velocity`（L0'）import 確認。記録付き build（build-info v0・走行中 build 拒否）＝[jetson/03 §1](03-build-deploy-run-and-run-records.md) |
| 6 | `/etc/warehouse/warehouse.env`（`ROS_DISTRO=humble`） | 雛形 [env.example](../../deploy/jetson/env/warehouse.env.example) は **2026-08-30 時点 `jazzy` だった**ため手で `humble` を書いた（雛形自体は #650 で `humble` に是正済・2026-09-10）。`TRAFFIC_MODE` / `MAP` は**未決マークのまま**（prod traffic 変更は安全レビュー PR＝[mode-m1/01](../mode-m1/01-mode-boundary-and-traffic.md)） |
