...
> 実機を動かさない**（[`.claude/rules/safety.md`] / 安全ゲートは下記 §4・doc16:211-214）。
>
...
> - 安全4層・実時間目標: [`docs/architecture/12-infrastructure-common.md:47-48`](../architecture/12-infrastructure-common.md)（Hard/Soft RT）`:75-84`（Layer0/1）`:397`（50ms/100ms は非ハードRT）`:168`（battery scale #44）。
...
- **逆風（原理的に近似不可な側）**: 一方、以下は **Mac/Docker では原理的に検証できない**。実 Jetson でしか
  ...
  - **実時間性**（Docker Desktop は Mac 上で軽量 VM 経由＝50ms Guardian / 100ms State Cache の jitter が
    ...
    best-effort。doc12:397 が「50ms/100ms は設計目標値でハードRT保証ではない」と明記）。
...
|---|---|:---:|---|---|---|
| F1 | ROS ノードロジック・凍結契約・launch 合成 | ◯ | arm64 一致＋偽トピック/偽 `state.json` で完全独立検証可（doc16:211-214） | dev（pytest/CI） | unit/CI 緑。`scripts/check_consistency.py` 0 ERROR |
| F2 | config overlay（`WAREHOUSE_ENV`・base+prod） | ◯ | パス解決は純 Python（`paths.py`）。prod=`/run/warehouse` は env 解決で再現可 | dev（unit） | `WAREHOUSE_ENV=prod` で `/run/warehouse` 解決（doc19:18 / jetson-deploy.md:154-155） |
...
| F5 | **実時間性**（50ms Guardian / 100ms State Cache の jitter） | ✕ | Docker Desktop は Mac 上 VM 経由でスケジューラが別物。R-40（doc07:250）GC/GIL スパイク＝最悪応答有界でない。doc12:397 が非ハードRT明記 | **実機ゲート G3**（Jetson） | §4 G3 の jitter 合否（p99 / max・stale 検出）。最終防衛は Layer0(doc12:75-78) |
...
| F8 | **実センサ精度**（MS200 測距 R-41 / encoder / battery scale #44） | ✕ | 物理センサ固有。MS200 dToF 誤差 ±1-3cm が地図 1cm 解像度と同オーダー（doc07:251）。battery `percentage` スケールはドライバ依存（doc12:168） | **実機ゲート G5**（Jetson+実機） | §4 G5＝測距誤差実測→地図解像度確定 / `safety.battery_percentage_scale` 確定（doc12:168 #44） |
...
> battery しきい値・スケールは `warehouse_interfaces.safety`（#44・doc12:168）、inflation 半径は
...
構文/引数解決）・**systemd unit 定義**（`deploy/jetson/` の静的検査）・**pytest**（安全機構 unit＝doc16:213）・
**config overlay**（`WAREHOUSE_ENV` の base+prod 解決）・**2台 Gazebo 自律走行 E2E**（環境成立・spike GO
doc06:112、sim 範囲。実 bot E2E は sim track #8/#156）。→ F1-F3, F2。**ARM64 三者一致が効く領域**（doc06:91）。
...
**Layer 0 estop**(F11)。→ §4 のゲートで合否を取る。**Mac では「無い／別物／未接続」のため値が出ない**のが本質。
...
|---|---|---|---|---|---|
| **G0 安全**（必須・最優先） | Layer 0 速度クランプ ≤0.3 m/s・近接 e-stop / Layer 1 Emergency Guardian unit | Jetson+実機 1台接続後すぐ。motion 有効化の**前** | ROS 側で 0.3 超指令を出しても MCU が ≤0.3 にクランプ・近接停止。Guardian unit テスト緑 | **昇格不可**。原因解消まで motion 系 unit を enable しない | Layer0=doc12:75-78 / Guardian unit=doc16:213 / 昇格ゲート=doc19:21・safety.md / jetson-deploy.md:15-26 |
...
| **G3 実時間性 jitter** | Guardian 50ms / State Cache 100ms 周期の実測ヒストグラム | Jetson 実機（R-40＝Phase 0.5(Jetson)） | 周期の **p99 / max** がデッドライン内に収まる。`gc.disable()` 適用後に外れ値が許容内。pose stale 検出が機能 | hot path を Python から外し **C++ `nav2_collision_monitor` + ESP32(Layer0)** へ委譲（#126） | doc07:250(R-40),:249(R-39) / doc12:47,:397,:416-467 |
...
| **G5 実センサ精度** | MS200 測距誤差 / encoder / battery `percentage` 実スケール | Jetson+実機（Phase 1-2） | 測距誤差を実測→**地図解像度を誤差の2-3倍**へ。`safety.battery_percentage_scale` を実測で確定 | 余裕不足 → 通路幅/inflation 再設計（R-41/R-42）。scale 未確定は fail-fast（doc12:168） | doc07:251(R-41),:252(R-42) / doc12:168 |
...
| 安全ゲート | doc19:21（estop テスト通過後のみ）＋ doc16:213（安全機構 unit 必須） | `install.sh` は導入のみ＝**enable/start しない**。motion はゲート後手動 | ◯ |
...
| traffic_mode 単一ソース | doc19:54（config 単一ソース）/ 11a:317（Mode C） | env.example が prod=`open-rmf` を `config/prod/warehouse.yaml:13` と同期せよと明記＋ `# TODO(#75)` で重複解消予告 | ◯（#75 着地で launch が config 直読み） |
...
- **暫定しきい値（実機で確定予定）**: `safety.pose_freshness_timeout`（既定 1.0s 暫定・doc12:424）／
  `safety.battery_percentage_scale`（Phase 1 実測・doc12:168）／ `ROBOT_RADIUS`（Phase 1 実測・R-42）／
  残RAM 500MB 閾（doc06:98 の Go/No-Go 値）。いずれも到着後に G1/G3/G5 で更新。
- **#75 依存**: `warehouse-nav2.service` の launch 引数は `bringup.launch.py` 合成（#75）着地まで no-op
  （jetson-deploy.md:90-94）。G4 は #75 後に実効。
