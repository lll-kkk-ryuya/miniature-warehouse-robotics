...
> ⚠️ **未解決（Issue #55）**: 上表の `status == "blocked"` 条件は、現状の **State Cache（doc12）が `status` を `"moving" | "idle"`（linear 速度から導出）のみ産出**し `"blocked"` を出さないため、situation JSON 上では発火しない。確定までは下記**パターン2（`history` の `result:"blocked"` 持続）/ パターン3（予測位置収束）**が実効的な検出経路。方針（doc12 State Cache に `"blocked"` 導出を追加 ＝ 契約拡張 か、本条件を速度ゼロ+同一ゴール継続+近接で置換 か）は **#55** で確定する。
