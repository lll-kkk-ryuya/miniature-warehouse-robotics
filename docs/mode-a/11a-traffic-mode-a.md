...
- **Mode C（Open-RMF）使用時**: VirtualScanNode 自体を起動しない。`traffic_mode: "open-rmf"`（`config/warehouse.base.yaml:6`）のとき **launch-time の `IfCondition`** で起動を抑止する（`nav2_bringup.launch.py:175`）。prod では systemd（`deploy/jetson/systemd/warehouse-nav2.service:29`）が `traffic_mode:=` を launch に渡すだけで、gating 自体は launch が行う（systemd がノードを直接制御するのではない）。
