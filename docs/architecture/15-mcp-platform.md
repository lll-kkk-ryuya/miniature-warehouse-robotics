...
> 📌 **採用実装（S1, #4 / PR #70）= Bridge 仲介ディスパッチ**: 直前の「ツール呼出しはサーバーサイドで実行」は Hermes の機能説明であり、**本PJ採用の S1 トランスポートではない**。S1 は **LLM が Command JSON を返し、`warehouse_llm_bridge` が `action_map` で MCP 7ツール呼出に写像して自らディスパッチする**（凍結コード `action_map.py` + `tools.py:11-13` の verbatim 受理 + #41）。これにより Bridge が mint する冪等キー（C, §競合状態の防止§2）が成立する（サーバーサイド実行では Bridge が tool call を仲介できず C を注入できない）。Hermes ネイティブのサーバーサイド実行は将来の代替経路（採用形は Bridge 仲介ディスパッチ。明示 `/stop` は **Issue #54** で撤回済）。設計根拠は `08-llm-bridge-common.md §同時発火制御` の「採用実装」📌注記。
...
- Nav2 Controller は `/bot{n}/cmd_vel/nav2` に publish（remap。同 topic へは collision_monitor 出力（doc12:543）と behavior_server recovery の直行（doc12:559）も合流。この prio10 入力は自作ノードが両端に居ない plumbing のため **doc03 トピックカタログ対象外**＝スコープ規約 doc03:115）
