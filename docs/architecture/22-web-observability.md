...
- 環境 / テスト / デプロイ: [doc19:5,56](19-environments-and-config.md)（コード1本・env は config）, :76（.env）, :94（git tag deploy）, :129（G1 残RAM≥500MB）, [doc20](20-dev-quality-and-testing.md), [doc16:82](16-repository-and-conventions.md)（命名）, :199（topic catalog 所有）, :184-191（branch 表）, [jetson-deploy](../setup/jetson-deploy.md)（5-unit systemd）
