...
- `provider_tags(provider, mode) → [provider, mode]` ＝ trace tag リスト形（doc08:375）の taxonomy 単一ソース。**live emit は `eval_sdk/tracer.py:174`**（doc21 で抽出。Bridge が構築・所有し `tracing.py` で re-export）であり、cross-lane import 禁止（parallel-workflow §2.1）のため #4 はこの関数を import しない。wo 側では reference + test 用の **inert**（`langfuse_sink` の予約 `SCORE_*` 名と同型）。wo 自身は score（tag 無し）のみ送るため、消費するのは `TAG_KEYS` であり `provider_tags` ではない。**`env=<v>`（環境タグ）は live emit 側（`eval_sdk/tracer.py`、#4 レーン）に末尾追加するが、`provider_tags` への `env` 同期は wo follow-up（#6）として残す**（それまで `provider_tags` は env 要素について既知 stale・実発信が正）。
