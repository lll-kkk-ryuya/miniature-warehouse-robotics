...
2. **v4 score の `metadata` group-by 可否**が未確認（doc08:518）。不可なら §8.3 の score `name` 符号化を採用。実 Langfuse 4.7.x で確認＝Phase 3/4。
3. **実 Langfuse（4.7.x）でしか検証不可**（doc13:520 ①〜⑤）: Bridge-owned trace の 1 本集約（**二重 generation 無し**）／ `trace_id` 突合（#4↔#6 が同一 trace・audit `gen_id`+timestamp とも一致）／ Grok cost 計上（カスタムモデル定義）／ managed-prompt 連携／ SDK 4.7.1 スモーク。**#88 Phase 3 残**（本スライスでは列挙のみ）。
