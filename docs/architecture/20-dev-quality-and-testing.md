...
| `environment` | dev / stg / prod | **trace tag 末尾（目標形 `env=<v>`）**（現 live emit は `[provider, mode]`・score 側は未対応＝trace-only） | #4 trace（`paths.warehouse_env()`／env `WAREHOUSE_ENV`） | doc13:526,607（`env=` 慣用）／ doc19 ／ `warehouse_interfaces/paths.py:14-19` |
...
- **trace**（#4 Bridge 所有, live emit は `eval_sdk/tracer.py:167` の `propagate_attributes(tags=...)`〔doc21 で抽出。Bridge が構築・所有し `tracing.py` で re-export〕）: 現 live emit は `langfuse_tags=[provider, mode]`（doc08:375）。目標形は `langfuse_tags=[provider, mode, env=<v>]`（`env=<v>`＝deployment 環境タグ `env=dev`/`env=stg`/`env=prod`、doc13:526,607 の `env=` 慣用に準拠・**emit リスト末尾追加**で既存の emit 側 tag[0]/tag[1] 記述を壊さない）。**注**: Langfuse は保存時にタグを**アルファベット順へ正規化**するため fetch/filter は順序でなく**値**で行う＝tag[0]/tag[1] は emit 側のみ。`env=<v>` の実配線と live filterable 検証は #88 follow-up（#4 trace-only code PR）で行う。trace はこれに加えて `gen_id` metadata ＋ `session_id` を持つ。tag で filter / group 可。
...
- **deployment 環境タグ `env=<v>`（dev/stg/prod, #88 追加）は 12構成の軸ではなく直交フィルタ**：env ごとに**別鍵**（doc19 §Secrets＝env ごとに別 `.env`／別鍵を規定。別「**プロジェクト**」運用は鍵の向き先次第で doc19 は明記しない）で分離されるため比較本線（Datasets+Experiments）に影響しない設計。実配線後は UI での env 別フィルタ／自己記述／誤設定検知用のラベルとして使う。
