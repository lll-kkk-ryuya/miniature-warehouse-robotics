...
- **常時 job（全 push/PR）**: `python-quality`（`ruff check`→`ruff format --check`→`pytest`）／ `consistency`（`scripts/check_consistency.py`・docs↔code 整合・0 ERROR 必須）／ `firmware-safety`（host R-26 速度クランプ unit ＋ キネマティクス unit ＋ skeleton compile・ESP32/PlatformIO 不要）。1つでも失敗で赤。
- **条件付き job**: `governance`（PR 時のみ・`warehouse_interfaces` 変更に contract ラベル必須＋他トラック内部 import 禁止）／ `web-e2e`（Playwright・Web UI 未実装で現状 `if: false`・Phase 4 で有効化）。
...
| `environment` | dev / stg / prod | **trace tag 末尾（`env=<v>`）**（score 側は未対応＝trace-only） | #4 trace（`paths.warehouse_env()`／env `WAREHOUSE_ENV`） | doc13:526,607（`env=` 慣用）／ doc19 ／ `warehouse_interfaces/paths.py:14-19` |
...
- **trace**（#4 Bridge 所有, live emit は `eval_sdk/tracer.py:167` の `propagate_attributes(tags=...)`〔doc21 で抽出。Bridge が構築・所有し `tracing.py` で re-export〕）: `langfuse_tags=[provider, mode, env=<v>]`（doc08:375。`env=<v>`＝deployment 環境タグ `env=dev`/`env=stg`/`env=prod`、doc13:526,607 の `env=` 慣用に準拠・**emit リスト末尾追加**で既存の tag[0]/tag[1] 記述を壊さない。**注**: Langfuse は保存時にタグを**アルファベット順へ正規化**するため fetch/filter は順序でなく**値**で行う＝tag[0]/tag[1] は emit 側のみ。**実 Langfuse 4.9.0 live で `env=dev` が filterable tag として永続することを検証済**＝#88 trace-tag 部分クローズ）＋ `gen_id` metadata ＋ `session_id`。tag で filter / group 可。
...
- `provider_tags(provider, mode) → [provider, mode]` ＝ trace tag リスト形（doc08:375）の taxonomy 単一ソース。**live emit は `eval_sdk/tracer.py:167`**（doc21 で抽出。Bridge が構築・所有し `tracing.py` で re-export）であり、cross-lane import 禁止（parallel-workflow §2.1）のため #4 はこの関数を import しない。wo 側では reference + test 用の **inert**（`langfuse_sink` の予約 `SCORE_*` 名と同型）。wo 自身は score（tag 無し）のみ送るため、消費するのは `TAG_KEYS` であり `provider_tags` ではない。**`env=<v>`（環境タグ）は live emit 側（`eval_sdk/tracer.py`、#4 レーン）に末尾追加するが、`provider_tags` への `env` 同期は wo follow-up（#6）として残す**（それまで `provider_tags` は env 要素について既知 stale・実発信が正）。
...
- **deployment 環境タグ `env=<v>`（dev/stg/prod, #88 追加）は 12構成の軸ではなく直交フィルタ**：env ごとに**別鍵**（doc19 §Secrets＝env ごとに別 `.env`／別鍵を規定。別「**プロジェクト**」運用は鍵の向き先次第で doc19 は明記しない）で分離されるため比較本線（Datasets+Experiments）に影響せず、UI での env 別フィルタ／自己記述／誤設定検知用のラベル。
...
3. **実 Langfuse（4.9.x）でしか検証不可**（doc13:520 ①〜⑤）: Bridge-owned trace の 1 本集約（**二重 generation 無し**）／ `trace_id` 突合（#4↔#6 が同一 trace・audit `gen_id`+timestamp とも一致）／ Grok cost 計上（カスタムモデル定義）／ managed-prompt 連携／ SDK 4.9.0 スモーク。**#88 Phase 3 残**（本スライスでは列挙のみ）。
4. **`env=<v>` トレースタグ（#88・trace-only）の残差**: ① 別プロジェクト構成では `env=<v>` はプロジェクト内で**定数**＝同一プロジェクト内フィルタの弁別力は低く、価値は自己記述／誤設定検知（dev 鍵→prod project 等）／前方互換（隔離は doc19 の env 別鍵が担保）。② **`warehouse_orchestrator/tags.py` の `provider_tags` への `env` 同期は wo follow-up（#6）**（それまで live emit が正・`provider_tags` は env につき既知 stale）。③ **score 側 `environment` metadata は未対応**（trace-only。必要時に doc08:487／§8.1 を先に拡張してから `build_score_metadata` に optional 追加）。④ **`env=` 名前空間は複数の直交軸を共有**: 本タグ（deployment `env=dev/stg/prod`）に加え doc13:526 の backend（`env=prod_vertex`）・doc13:607 Phase 5 の digital-twin source（`env=isaac_sim`/`env=real`）が同一 `env=` 前置を使う。タグは単純文字列ゆえ技術衝突はない（1 trace が `env=prod`＋`env=real` を併持しうる＝直交ラベル）が、フィルタ時の意味分裂に注意＝Phase 5 owner と値域整理を要調整（env-tag スコープ外）。
