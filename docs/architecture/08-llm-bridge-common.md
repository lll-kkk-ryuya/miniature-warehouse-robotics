...
- 1 observation / ツール呼出し（navigate_to_pose等の引数と結果）※Option D では加えて非 tool observation `prompt-enrich`（`PluginTraceEnricher` の書込み・#612）が plugin-owned trace に載る
