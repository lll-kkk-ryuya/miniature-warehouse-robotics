...
- **B2 非一様 odom・欠落区間 → (c) 開示＋ゲート**。(b) 等間隔リサンプル/補間は**不採用**: :467 が名指しする実害は jitter ではなく**欠落区間**であり、補間は欠落区間に存在しない sample を作って spectral 指標や ② を「もっともらしい数値」にする＝§12.3 の fail-open（落ちたら黙る・捏造しない）に反する。確定: **窓**は `SmoothnessStats` に `median_dt_s`（隣接 Δt の中央値＝名目周期）と `max_gap_s`（隣接 Δt の最大）を開示し、`max_gap_s > gap_ratio_limit × median_dt_s` のとき **SPARC / LDLJ を `None`**（「算出不能」＝平均レート近似が成立しない窓・`filtered_samples` も `None`）にする。`gap_ratio_limit = 3.0` は `smooth_window`=5 と同じ**工学的チューニング値であって domain しきい値ではない**（`SmoothnessStats.gap_ratio_limit` で毎回開示）。fs は引き続き窓の平均レート（§16 ④）＝ゲートを通った窓では jitter 内の近似。N_MU・idle 率（生カウント）と ② の台形積分は**変更しない**（② は欠落区間を跨いで積分する挙動を維持し、`max_gap_s` の開示で読者が判断する）。**run** は O(1) 制約から中央値を持てないため `RunMotionStats.max_gap_s` のみ開示（積分は不変・`samples/duration` と併せて欠落の有無を読む）。「3 通り同居」（①=生カウント・②=実スタンプ積分・spectral=平均レート近似＋gap ゲート）は解消せず**意図した非対称として文書化**する。
