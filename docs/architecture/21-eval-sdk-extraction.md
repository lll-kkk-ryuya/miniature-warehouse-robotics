...
|---|---|---|---|---|
| 介入率 intervention rate | `eval_sdk.stats`（rate） | 既存 audit の `escalation_response` 行（MCP ツール6 = [doc15:173](15-mcp-platform.md)・producer = `warehouse_mcp_server/tools.py` の tool6 `escalation_response`（executed 記帳は同関数末尾の `record("escalation_response","executed",…,robot=new_robot)`）・executed 集合 = `warehouse_orchestrator/kpi.py` の `COMMAND_TOOLS`） | nav 標準 `intervention_rate` | sink |
| throughput / makespan | `eval_sdk.stats` | 既存 audit の完了 task（Tier0 で completion 源が来たら即） | 倉庫 manifest | sink |
| fairness / 負荷均等 | `eval_sdk.stats`（gini/ratio helper・汎用） | 既存 audit `by_robot`（`warehouse_orchestrator/kpi.py` の `compute_kpis` が集計し `robot_load_fairness` が Jain 化） | 倉庫 manifest | sink |
| 軌道平滑性（jerk・方向反転） | `eval_sdk.stats`（速度差分・汎用） | 既存 `/bot{n}/odom`（`warehouse_orchestrator/motion.py` の `MotionAccumulator`/`smoothness_stats`＝`kpi_collector` の既存 odom callback 内で列保持。§16） | nav 系 `smoothness` | sink |

→ **実装場所 = ほぼドメイン（倉庫 `kpi.py` / `motion.py` 拡張）＋ eval_sdk.stats の汎用 helper**。**新ノード不要**（audit 系は保持ゼロ／odom 系は既存 subscriber 内の bounded 列保持が要る＝§16 ①）・**Phase 3 を待たず着手可**・additive。
...
## 16. 実装追補（odom 由来 Tier-1 で確定した点・#613 → #616 post-merge review）

§6 Tier-1 表 / §13.1 :306 / §13.2 :310 に対して、**実装が確定させた事実だけ**を記録する（新しい指標定義をここで作らない）。実装 = `ws/src/warehouse_orchestrator/warehouse_orchestrator/motion.py`（合成 `kpi.py` / 配線 `kpi_collector.py`）。**観測面のみ＝新トピック・新契約・新 score 送信ゼロ**。

① **:189 の改稿記録（「新 producer ゼロ」→「新ノード不要」）** — #613 land 時点の :189 は無条件に**「新 producer ゼロ」**だった。しかし audit 系 Tier-1 が保持ゼロで成立するのに対し、odom 系（軌道平滑性）は速度**列**が要り、`eval_sdk.stats.DistanceAccumulator` は総和と直前1点しか持たないので保持ゼロでは算出できない。実装は**新ノード・新トピック・新契約を足さず**、`kpi_collector` が既に張っている `/bot{n}/odom` subscription の内側に **bounded ring buffer**（`MotionAccumulator`・既定 4096 sample ＝ 約30 Hz で約2分・要約した窓は毎回 report に開示）を置いて解決した。この実態に合わせ、#616 で :189 の当該句を**「新ノード不要（audit 系は保持ゼロ／odom 系は既存 subscriber 内の bounded 列保持が要る）」へ改稿**した（誤りの訂正であって、読み替えの追加ではない）。Tier 2 (:195-197) の★は本来どおり「新ノードが要る」意味であり、この改稿は及ばない。

② **velocity の源 = odom メッセージの signed twist（pose 差分ではない）** — :301 (a) は「odom pose 列」を挙げるが、:306 の N_MU（速度符号反転数）は**符号**を要求し、pose 差分から作った速度は非負なので符号反転を原理的に検出できない。実装は `/bot{n}/odom` 自身の `twist.twist.linear.x` を採用する（per-robot 線速度の正準フィールド＝[doc12:262](12-infrastructure-common.md)（State Cache が `/{bot}/odom` を購読）/ [doc12:340](12-infrastructure-common.md)（`velocity{linear,angular}`）。他トラックのモジュールは import せず documented field を独立に読む）。SPARC / LDLJ には同じ系列の |v| を与える。

③ **:306「3階微分前に low-pass 必須」は SPARC / LDLJ の入力にも適用する（#616 で実装）** — odom が速度を直接くれるため残る微分は指標の内側にあり（LDLJ = 速度の2階差分／SPARC = その正規化スペクトル）、規範句はその**入力**に掛かる。実装は `eval_sdk.stats.low_pass`（`jerk()` が同じ :306 のために内部で使う centered moving average と同一物）で |v| を平滑してから両指標へ渡す。**幅（既定 5 sample）は工学的チューニング値であって domain しきい値ではない**——:306 は「low-pass すること」だけを規定し幅を規定しない——ので、`SmoothnessStats.smooth_window` に**毎回**、`filtered_samples`（= spectral 指標へ実際に渡った平滑後サンプル数）に**到達したときのみ**開示する（未到達は `None`＝幅未満／床未満／peak 0／レート不定のいずれか。理由の区別はしない）。**N_MU には掛けない**（微分を含まず、平滑は数えるべき反転そのものを消すため）。実測効果: 9 sample のジッタ窓で LDLJ −8.34 → −3.06（生値は sampling noise を jerk として読んでいた）。

> **⚠ 未決（doc owner 判断待ち・#616 で開示）— SPARC にも掛けるべきか**。:306 の規範句は文字どおりには**微分**に掛かる。LDLJ は速度の2階差分＝位置の3階微分そのものなので適用は疑いない。一方 **SPARC は微分せず**（正規化スペクトルの弧長）、**自前の帯域制限 `fc`=10 Hz を既に持ち**、:306 が照合元とする siva82kb 実装は前置フィルタを掛けない。さらに幅5・fs=30 の移動平均は最初のヌルが **6 Hz** ＝ SPARC が積分する 0–10 Hz 帯**の内側**に落ちるため、二重の帯域制限になる（実測: `_JITTERY9` で SPARC −2.05（生）→ −1.70（平滑））。**#616 は Issue DoD どおり SPARC/LDLJ の双方に適用して land した**が、これは :306 が決着させていない判断であり、外す場合は `smoothness_stats(..., smooth_window=1)` 相当の1行（SPARC 側だけ生 |v| を渡す）で可逆。**N_MU の noise 感度も同様に :306 未規定**（平滑すれば偽反転は減るが真の反転も消える）。
...
- **速度 ε 源（採用）**: odom 由来 |v| 系列（`motion.MotionAccumulator` が既に窓保持・§16 ①）に対し `|v| ≤ ε` を idle とする。ε は既存の `warehouse_state.aggregator.derive_status`（`|linear| ≤ ε → "idle"` を報告）と**同一値で第2定数を作らない**。**凍結先 = `warehouse_interfaces.safety.IDLE_SPEED_EPS`（0.01 m/s・#642・両消費者が import）**＝当初は `derive_status` が「別トラック（`warehouse_state`）所有・凍結契約ではない」ため import せず documented value を借用し cross-check test で等値を pin していた（§16 ② の twist 源採用と同流儀）が、#642 が凍結契約へ additive 追加して本レーンと State Cache の単一ソースにした（値・境界とも不変。[doc12【2026-09-10 追補】](12-infrastructure-common.md)）。
...
② **`速度予算消化率`（speed-budget utilisation）の定義を確定** — 上限（cap＝v_max）は **config `safety.max_linear_velocity`**（環境 tunable。`warehouse_interfaces.config.load_config` が `0 < cap ≤ MAX_LINEAR_VELOCITY` を検証＝config は hard cap **0.3 m/s** を**下げることしかできない**。base 値 = 0.3・`config/warehouse.base.yaml`）。hard cap 0.3 自体（`warehouse_interfaces.safety.MAX_LINEAR_VELOCITY`・:254「コードで強制」）は**上限（ceiling）であり分母そのものではない**（config 不読時の fallback・再タイプ禁止＝import する）。overlay で cap を下げた run と 0.3 の run は同じ軌道で異なる値を報告するため、**解決した cap を report に毎回開示する**（`SmoothnessStats.speed_cap`・`smooth_window` / `window_*` と同じ開示原則）。3 候補:

- **(i)** `mean|v| ÷ cap` — 最安。既出 `SmoothnessStats.mean_speed` から即算（`mean_speed / cap`）。問い＝「平均でどれだけ上限を使ったか」。
...
- **確定 = (iii)**。「予算に対する消化」の字義に最も忠実。(i) は (iii) の**近似**推定量にすぎない: 台形則は両端点を半重みにするため、一様サンプリング下でも両者が一致するのは **endpoint-balanced 窓**（`mean|v| = (|v_first|+|v_last|)/2`）のときだけで、jitter はそれとは独立な第 2 の乖離源。したがって **(iii) の算出には per-sample 系列とスタンプが要り、要約 `mean_speed` だけでは足りない**（§17 ① が §16 ⑤ に施した訂正と同じ非対称。`mean_speed/cap` は近似値として併記可）。(ii) は別指標（飽和率）として分離し、消化率には採らない。`∫|v|dt` は窓内＝`MotionInputs.samples` の台形則、run 全体＝**#632 で改定: `MotionAccumulator.run_totals()` の実スタンプ台形 ∫|v|dt**（当初案の `DistanceAccumulator.totals()`（pᵢ）は採らない＝pᵢ は pose 差分・∫|v|dt は twist 積分で、`DistanceAccumulator` は `MotionAccumulator.add` が落とすサンプル（非前進スタンプ・非有限）も積むため両者は一致しない。pᵢ は `KpiReport.distance_traveled` として別途開示・窓版と同一の式を優先。odom 欠落区間を跨ぐ積分の扱いは §17 ③ / #632 B2）。観測窓（§17 ③）に合わせるなら**窓内積分**、`T = window_end − window_start`。
...
- → **同一 report 内に「run 全体」と「直近窓」の2つの時間スコープが同居**する。窓境界は `SmoothnessStats.{window_start,window_end,samples}` で毎回開示（§16 ①）。**idle 率・速度予算消化率（§17 ①②）は smoothness と同じ窓スコープを既定**とする（材料が同じ ring buffer 由来のため）。run 全体版が要るなら `DistanceAccumulator` 同様の軽量な idle-time / ∫|v|dt 累算器を足す（本節では**窓スコープを確定**。**run 全体版は #632 で実装済**＝`MotionAccumulator` が同じ `add()` 検証経路の中で O(1) の run 累算（`eval_sdk.stats.TimeSeriesAccumulator`）を保持し、`run_motion_stats` が `KpiReport.run_motion` へ合成する。**式・ε・cap は窓版と同一でスコープのみ異なる**ため ring buffer の eviction は run 値に影響せず、1 つの report に両スコープが同居して**各々が自分の境界を開示する**＝窓は `window_start`/`window_end`/`samples`、run は `t_first`/`t_last`/`duration`/`samples`）。
- **非一様サンプリング**: SPARC / LDLJ は**一様サンプリングを仮定**するが odom は jitter する。実装は窓の**平均レート** `fs = (n−1) ÷ (t_last − t_first)`（=`sample_rate_hz`・`eval_sdk.stats.throughput` を再利用）を両指標へ渡し、ジッタを平均レートで吸収する（§16 ④）。一方 §17 ② は実スタンプで台形積分し、§17 ① は生サンプル数の割合（jitter 非補正＝parked 中の odom burst は idle *時間*を過大に見せ、odom 欠落区間は ② が跨いで積分する一方 ① には見えない）＝**同一窓に jitter の扱いが 3 通り同居**する。**残件＝厳密な扱い**（等間隔リサンプル/補間 vs 平均レート近似・3 通りの整理）は未決＝follow-up。fs の効き方の非対称（SPARC は fs 依存・LDLJ は fs 不変）は §16 ④ に既述。

> **本節が確定していないこと（follow-up）**: (1) ε=0.01 m/s の所有・凍結 **→ ✅ #642 で確定**（凍結契約 `warehouse_interfaces.safety.IDLE_SPEED_EPS` が単一ソースで、`warehouse_state.derive_status`（[doc12【2026-09-10 追補】](12-infrastructure-common.md)）と本節の idle 率が**両方 import** する。旧: 別トラック私有 `_MOVING_EPS` + wo ミラー定数を cross-check test で等値 pin。値・境界不変＝additive・観測用で actuation 非関与）、(2) 非一様 odom の厳密なリサンプル方針 **→ ✅ ④ で裁定**（**(c) 開示＋ゲート・補間は不採用**。run スコープも ① は生サンプル数・② は実スタンプ台形積分のまま＝意図した非対称として文書化）、(3) idle 率 / 速度予算消化率を run 全体窓へ広げるか **→ ✅ #632 で実装済**（`MotionAccumulator.run_totals` → `run_motion_stats` → `KpiReport.run_motion`＝additive・report フィールドのみ・**新 producer / topic / 契約 / score 送信なし**。窓スコープは既定のまま**併存**＝置換ではない）。**④（B2 ゲート/開示・B3 cap 出所・B4 `rejected`）の実装は** report フィールド水準の**非破壊**＝additive・別 PR（(1) は #642・(3) は #632 で実装済＝窓版と併存）。
>
...
④ **#632 B2–B4 の裁定（非一様 odom・欠落区間／cap の出所／clock-reset の silent freeze）** — ③ の残件 (2) と #629/#637 の review 残件を docs で確定する。いずれも **report フィールド水準・additive**（新 producer / topic / 契約 / score 送信なし・受理規則不変）で、実装は別 PR。

- **B2 非一様 odom・欠落区間 → (c) 開示＋ゲート**。(b) 等間隔リサンプル/補間は**不採用**: :467 が名指しする実害は jitter ではなく**欠落区間**であり、補間は欠落区間に存在しない sample を作って spectral 指標や ② を「もっともらしい数値」にする＝§12.3 の fail-open（落ちたら黙る・捏造しない）に反する。確定: **窓**は `SmoothnessStats` に `nominal_dt_s`（隣接 Δt の**下位四分位**＝名目周期。**中央値は不採用**: frame と outage の二峰分布では outage が区間の過半を占めた時点で中央値が outage 側へ移り、ゲートが最悪の stream でこそ解除される〔review で再現: 10 s 欠落 55% で SPARC が fs≈0.2 Hz で公開〕。下位四分位は outage 比 75% 未満で真の周期に留まる）と `max_gap_s`（隣接 Δt の最大）を開示し、`max_gap_s > gap_ratio_limit × nominal_dt_s` のとき **SPARC / LDLJ を `None`**（「算出不能」＝平均レート近似が成立しない窓・`filtered_samples` も `None`）にし、判定そのものを **`spectral_gated: bool`** で開示する（`None` は「sample 不足／fs 不定／不動／numpy 不在／gate」で多義のため）。`gap_ratio_limit = 3.0` は `smooth_window`=5 と同じ**工学的チューニング値であって domain しきい値ではない**（`SmoothnessStats.gap_ratio_limit` で毎回開示・非有限/非正の値は既定値へ sanitize してから gate と開示の両方に使う＝`allow_nan=False` の JSON を壊さない。`nominal_dt_s`/`max_gap_s` の非有限は run と同様 `None` へ degrade・gate は生の Δt で判定）。fs は引き続き窓の平均レート（§16 ④）＝ゲートを通った窓では jitter 内の近似。**既知の崖（正直に）**: (a) outage が区間の **75% 以上**を占めると名目周期が outage 側へ崩れてゲートが解除される（`sample_rate_hz` が唯一の手がかり）、(b) 健全だが二峰の stream（例 100 Hz/30 Hz の対到着）は mode 比が `gap_ratio_limit` を超えると**保守的に gate される**＝捏造しない側に倒す、(c) ゲートは**保持窓**で評価するため一過性の欠落 1 回で spectral 対は当該 Δt が退避されるまで（既定 4096 sample ≈136 s ≈ report 4–5 回）`None` のまま＝`max_gap_s` がその理由を示す。N_MU・idle 率（生カウント）と ② の台形積分は**変更しない**（② は欠落区間を跨いで積分する挙動を維持し、`max_gap_s` の開示で読者が判断する）。**run** は O(1) 制約から中央値を持てないため `RunMotionStats.max_gap_s` のみ開示（積分は不変・`samples/duration` と併せて欠落の有無を読む）。「3 通り同居」（①=生カウント・②=実スタンプ積分・spectral=平均レート近似＋gap ゲート）は解消せず**意図した非対称として文書化**する。
- **B3 cap の出所 → 開示する**。`MotionInputs.speed_cap_source ∈ {"config","fallback"}` を `kpi_collector` が `resolve_speed_cap` の結果（warning あり＝fallback）から与え、`SmoothnessStats.speed_cap_source` / `RunMotionStats.speed_cap_source`（`speed_cap` の直後・cap が `None` なら `None`）に開示する。`fallback` は「hard cap `warehouse_interfaces.safety.MAX_LINEAR_VELOCITY` を分母に使った」の意味で、config 未設定（合法）と config 不読/不正の両方を含む（区別は node の warning ログ）。② の「解決した cap を毎回開示」原則の延長＝overlay で cap を下げた run の過小報告（config 不読時）を report 単体で識別可能にする。
- **B4 clock/sim reset の silent freeze → `rejected` counter を開示する**（受理規則は不変）。`eval_sdk.stats.SeriesTotals.rejected`（accumulator が拒否した `add()` の累積＝非有限・非前進スタンプ・汎用）を持ち、`MotionAccumulator` は自身の事前 guard（x/y 非有限）の拒否も合算して `RunMotionStats.rejected`（末尾 key・run 起点からの単調カウント）に出す。**判定規則**: 連続する report で `samples` が横ばいのまま `rejected` が増え続ける ＝ **この robot の sample が拒否され続けている**（counter は**症状であって原因ではない**: スタンプ逆行〔sim reset / bag 再開 / clock 切替〕と非有限 pose〔推定器の発散〕は両拒否クラスを合算するため同一の signature＝review で byte-identical を再現。区別は node の warning ログ、または counter 分割＝follow-up）。自動 re-seed（逆行を新 epoch として受理）は**採らない**（受理規則の変更＝窓版にも波及する別判断・fail-closed 維持）。窓側には出さない（同一 stream・run counter で十分）。`samples = 0` でも `rejected > 0` の robot は report に残す（「全 reject」を ghost として落とさない）。
