...
> **本節が確定していないこと（follow-up）**: (1) ε=0.01 m/s の所有・凍結（現状 `warehouse_state.aggregator.derive_status` 私有・別トラック）**＝未決**、(2) 非一様 odom の厳密なリサンプル方針**＝未決**（run スコープも ① は生サンプル数・② は実スタンプ台形積分のまま＝窓と同じ非対称を引き継ぐ）、(3) idle 率 / 速度予算消化率を run 全体窓へ広げるか **→ ✅ #632 で実装済**（`MotionAccumulator.run_totals` → `run_motion_stats` → `KpiReport.run_motion`＝additive・report フィールドのみ・**新 producer / topic / 契約 / score 送信なし**。窓スコープは既定のまま**併存**＝置換ではない）。**(1)(2) は** report フィールド水準の**非破壊**であり、確定後に additive 実装する（(3) は #632 で実装済＝窓版と併存）。
>
