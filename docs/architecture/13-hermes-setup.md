...
WAREHOUSE_CONFIG_DIR=config   # base+overlay（warehouse.base.yaml + $WAREHOUSE_ENV/warehouse.yaml）を paths.config_paths が解決。旧 WAREHOUSE_CONFIG_PATH 単一ファイル方式は doc16:112 / doc19 で置換
...
#     （二重計上回避。§7.5 / :483。本例の enabled:true はメッセージング bot 用の既定）
# ───────────────────────────────────────────────
observability:
  langfuse:
    enabled: true
