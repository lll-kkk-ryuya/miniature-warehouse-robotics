...
# ───────────────────────────────────────────────
# Memory / Skills / session_search（Hermes 内蔵の自己学習機能）
#   ↓ 実デプロイ済みスキーマ（deploy/hermes/gcp/config.yaml:474-560 と一致）。
#   旧記載 memory.enabled / cross_session_recall / skills.enabled / auto_create は
#   実在しないキーだった（#103 で実キーへ修正）。比較 OFF 機構の正本は直下の
#   「Memory / Skills / session_search — 比較公平性のための OFF 機構」節を見よ。
# ───────────────────────────────────────────────
memory:
  memory_enabled: true        # MEMORY.md（エージェントのメモ）を毎セッション system prompt に注入
  user_profile_enabled: true  # USER.md（ユーザープロファイル）を注入
  memory_char_limit: 2200     # ~800 tokens
  user_char_limit: 1375       # ~500 tokens
  nudge_interval: 10          # N ユーザターンごとに「記憶を保存」を促す（0=無効）
  flush_min_turns: 6          # 文脈消失前に記憶保存の1ターンを与える最小ターン数（0=無効）

skills:
  creation_nudge_interval: 15 # N ツール反復ごとに skill 作成を促す（0=無効）。enabled キーは無い

# session_search（FTS5 過去会話想起）は memory: ブロックではなく **toolset**（過去会話を
# FTS5 + Gemini Flash で要約・想起）。有効/無効は platform_toolsets に含めるかで切替（下節）。

# ───────────────────────────────────────────────
# Langfuse (内蔵プラグイン・opt-in: pip install langfuse + enabled:true。§7.1。env=LANGFUSE_BASE_URL)
#   ⚠ Bridge-owned trace を採る比較セットアップでは Hermes 側プラグインを OFF 上書き
#     （二重計上回避。§7.5 / :517。本例の enabled:true はメッセージング bot 用の既定）
# ───────────────────────────────────────────────
observability:
  langfuse:
    enabled: true
...
### Memory / Skills / session_search — 比較公平性のための OFF 機構（#103 / R-36）

Phase 4 の4社比較では Hermes の自己学習を**強制 OFF** にする（公平性運用契約の正本は [`08-llm-bridge-common.md`](08-llm-bridge-common.md) §比較の公平性）。「長期記憶」は実 Hermes では**3つの独立したノブ**に分かれており、3つすべてを落として初めて OFF になる:

| ノブ | 実体 | 既定（messaging bot 用） | 比較 run で OFF にする方法 |
|------|------|------------------------|--------------------------|
| **memory** | `MEMORY.md`（メモ）/ `USER.md`（プロファイル）を毎セッション system prompt に注入 | `memory.memory_enabled: true` / `user_profile_enabled: true` | **両方 `false`** |
| **skills** | 成功手順の保存・読込（`skills_list` / `skill_view` toolset） | `skills.creation_nudge_interval: 15`（+ `skills` toolset が露出） | `creation_nudge_interval: 0` **かつ** `platform_toolsets` から `skills` を除外 |
| **session_search** | FTS5 + Gemini Flash による過去会話想起（toolset） | `cli: [hermes-cli]` プリセットに含まれる | `platform_toolsets` から `session_search` を除外 |

- **skills には `enabled` キーが無い**（`creation_nudge_interval` は「作成を促すか」だけで、既存 skill の**読込**は止めない）。`platform_toolsets.cli: [hermes-cli]` プリセットは「send_message 以外すべて」（`deploy/hermes/gcp/config.yaml:731-732`）＝ `memory` / `skills` / `session_search` を含む。よって比較 run は **プリセットを明示リストに差し替えて**これら3 toolset を除外する（例 `cli: [terminal, file, web, todo]`）。
- **権威ある OFF の置き場所 = Hermes 側 `~/.hermes/config.yaml`**（`deploy/hermes/gcp/config.yaml`）。LLM Bridge ↔ Hermes は**ステートレス `/v1/chat/completions`**（[`08`](08-llm-bridge-common.md) / `hermes_client.py`）であり、Bridge から Hermes の memory 状態は**読めも設定もできない**。さらに Hermes Gateway は Bridge より**先に**起動し、config を起動時に1度だけ読む（hot-reload 無し。§8 起動順 / :544）。したがって OFF を実際に効かせるのは Hermes config 側であって Bridge ではない。
- **Bridge 側の役割 = fail-closed な intent/consistency ガード**（#103 実装）。倉庫 config（§3.3 `hermes.memory_enabled` / `skills_enabled` / `comparison_run`）に宣言された**運用意図**を起動時に検証し、`comparison_run: true` なのに memory/skills が ON 宣言なら**起動を abort**（`warehouse_interfaces/config.py` の `_validate_safety` と同型の独立チェック）。これは「比較 run の Bridge 側設定が自己矛盾していたら参加を拒否する」ものであり、**Hermes が実際に OFF である保証ではない**。
- **倉庫 intent → 実 Hermes OFF の配線**（env interpolation / 生成 config / toolset 差し替え）は **Phase 4 の起動ハーネス責務**として未配線（#103 では docs 化のみ・実配線 defer）。比較ハーネス構築時に、上表の3ノブを OFF にした Hermes config をデプロイし、同一の `comparison_run` を倉庫 config 側にも設定すること。
...
hermes:
  ...
  # ── Memory/Skills 公平性 intent フラグ（#103、上節「OFF 機構」）──
  #   これらは Hermes を制御するキーではなく、LLM Bridge が起動時に検証する「運用意図」。
  #   既定は OFF（False）で Bridge コード側に持つ（この base.yaml には値を書かない＝環境差分は overlay）。
  #   Mode A エンタメ回は overlay/env で memory_enabled: true を立てて学習 ON 可（doc08 §比較の公平性）。
  #   memory_enabled: false   # 倉庫意図。実 OFF は Hermes config の memory_enabled + user_profile_enabled + session_search 除外
  #   skills_enabled:  false   # 倉庫意図。実 OFF は Hermes config の creation_nudge_interval:0 + skills toolset 除外
  #   comparison_run:  false   # true=Phase4 比較 run。Bridge が memory/skills の ON 宣言を起動時に abort（公平性ガード）
...
内蔵 Langfuse プラグインは **opt-in**（キーが `.env` にあるだけでは自動有効化しない）。プラグインが読む host env は **`LANGFUSE_BASE_URL`（または `HERMES_LANGFUSE_BASE_URL`）** で `LANGFUSE_HOST` ではない（:128。Bridge-owned trace を採る比較セットアップでは Hermes 側を OFF にする・§7.5）。

```bash
pip install langfuse                            # 内蔵プラグインの依存
hermes plugins enable observability/langfuse    # opt-in 有効化（or config.yaml の enabled:true）
hermes config check                             # → "langfuse: enabled" が出ればOK
```
...
- **Phase 3 実トレース検証項目**: ① Hermes が inbound `metadata.trace_id` を尊重するか（Bridge-owned 方式なら非依存）／② `usage_details`/cost が4社で ≠0 か（**xAI Grok はカスタムモデル定義が必要な可能性**＝無いと cost 空欄で比較破綻）／③ 二重 generation の有無／④ **managed-prompt `prompt=` 連携の live 紐付け確認**（**offline は実装済**: `get_prompt(fallback)` 取得＋code fallback＋`langfuse_prompt=` 配線＝[doc08 §Langfuse Prompt Management 方針](08-llm-bridge-common.md) / `warehouse_llm_bridge/prompts.py`。本項は実 Langfuse への seed・取得・generation 紐付けの live 確認）／⑤ SDK 4.9.0 スモーク。
...
| Langfuse にトレースが出ない | プラグイン未有効化（opt-in §7.1）/ `LANGFUSE_BASE_URL`（Hermes が読む env・`LANGFUSE_HOST` ではない）到達性 / keys 正当性 / Hermes は失敗時サイレント | `hermes plugins enable observability/langfuse` を確認し `curl $LANGFUSE_BASE_URL/api/public/health` で疎通確認、`HERMES_LOG_LEVEL=debug hermes gateway` で再起動確認 |
