...
└────────────────────────────────────────────────────────────┘
       ↓ /negotiation/start
┌────────────────────────────────────────────────────────────┐
│ キャラLLM Bot1（新規、Haiku）   キャラLLM Bot2（新規、Haiku）│
...
| 提案フォーマット強制 | 自由文NG。JSON Schema で構造化を強制（Haiku のシステムプロンプトで指示 + Bridge側でvalidate） |
...
| ロール | モデル | 理由 |
|---|---|---|
| 司令官LLM | Sonnet 4 (Phase 4 で4社比較) | 戦略判断の質が重要 |
| キャラLLM | **claude-haiku-4-5** | 応答0.3-0.5s、コスト1/10、max_tokens=60 で1〜2文 |

キャラLLM はテンポ重視。Phase 4 の Sonnet 比較対象には含めない（演出専用扱い）。
...
| `/negotiation/start` | std_msgs/String (JSON) | 司令官LLM | キャラLLM Bot1/Bot2 |
...
| `/state_cache/snapshot` | std_msgs/String (JSON) | State Cache Node | キャラLLM, 司令官 |
