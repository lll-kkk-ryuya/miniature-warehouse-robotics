...
- いずれにせよ **LLM API 課金は不変**なので、本当のコスト最適化はホスティングではなく **プロンプト/トークン削減・モデル選定**（[feedback: MCP token cost] の方針）側にある。

> 将来 [Physical AI ローカルファースト構想](../shared/) に沿って Jetson ローカルへ寄せる場合、クラウド常駐自体が不要になる。その意味でも現状の「無料 VM を Phase 0 のつなぎ」とする位置づけが整合的。
