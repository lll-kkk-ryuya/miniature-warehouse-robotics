...
│  ├── LLM Bridge Node（3秒タイマー → Hermes Gateway） │
...
| 3 | Claude / Hermes | 3秒周期 | ◎ | 事後の説明・タスク再割当・復旧方針の提案 |
...
| `/map` | `nav_msgs/OccupancyGrid` | SLAM生成の2D地図 |
...
| `/wo/mission` | カスタム | WOからのミッション指示 |
| `/llm/situation` | カスタム（JSON） | LLMに送る状況データ |
| `/llm/command` | カスタム（JSON） | LLMからの指示データ |
...
**注意**: ROS 2 Jazzy が公式LTS（Long Term Support）リリースかどうかは一次確認が必要（※未検証）。Humbleは確定LTS。
...
詳細設計は `08-llm-bridge-common.md` を参照。Hermes Agent + Warehouse MCP Server の統合設計は `12-infrastructure-common.md` を参照。
...
- [Nav2 Simple Commander API](https://docs.nav2.org/commander_api/index.html) — 参照日: 2026-05-21（※実装では Warehouse MCP Server に置き換え、`12-infrastructure-common.md` 参照）
