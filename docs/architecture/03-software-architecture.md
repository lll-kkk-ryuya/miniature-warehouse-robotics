...
| `/bot{n}/battery` | `sensor_msgs/BatteryState` | バッテリー残量（実機 micro-ROS firmware が供給, Phase 1+） |

> ※ sim（#43 `ros_gz_bridge`）は `/bot{n}/scan` `/bot{n}/odom` `/bot{n}/cmd_vel` のみを ROS へ橋渡しする。`/bot{n}/imu` は sim では ROS に橋渡ししない（gz トピックは URDF が出すが ros_gz_bridge 対象外＝doc03 契約外、必要時に bridge へ追加）。`/bot{n}/battery` は sim に sensor が無く生成しない。両者とも実機 micro-ROS（firmware, Phase 1+）が供給する。
...
| `/negotiation/abort` | `std_msgs/String`（JSON） | 交渉中断信号（Emergency Guardian 発火時に `{reason, bot, event_id}` を `/emergency/event` と同時 publish）（doc12 Emergency Guardian / doc14 §交渉） |
