...
| `/bot{n}/stop_state` | `std_msgs/String`（JSON） | 停止上乗せ（L0' m1_driver）への停止状態 feed。`{"stop_requested": bool, "valid_until": float}` を**周期 publish**（停止要求が無い間も流す level チャネル）。`valid_until` は同一ホスト単調時計の絶対期限で**単調非減少**（逆行・非有限・非正・パース不能はすべて即時失権＝fail-closed）。producer = Emergency Guardian（L1・後続スライス）／ consumer = m1_driver（`stop_overlay_enabled: true` のときのみ購読）。QoS・上限クリップ・期限規律の正本 = mode-m1/05 §4-1 |
