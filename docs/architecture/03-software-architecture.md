...
| `/bot{n}/scan` | `sensor_msgs/LaserScan` | ORBBEC MS200 dToF LiDAR（360°スキャン、AMCL自己位置推定 + 障害物検知）。**分解能/QoS は producer=firmware が設定し、consumer=nav-traffic（AMCL/costmap/`collision_monitor`＝R-41/R-42）と produce/consume で advisory 合意**（name+型は凍結・本注記は分解能/QoS の運用合意のみ＝contract 不変。既定/fallback/値の正本は R-43＝[doc07:253](../shared/07-research-notes.md) / firmware [PHASE1_CHECKLIST D2](../../firmware/PHASE1_CHECKLIST.md)） |
...
| `/negotiation/start` | `std_msgs/String`（JSON） | キャラLLM 交渉の開始（司令官の `start_negotiation` 経由で Warehouse MCP Server が発行、gen_id + starter 同梱。**Mode A/B で発動**＝キャラLLM は `traffic_mode in {none, simple}` でのみ起動。Mode C 交渉は Phase 4 で実装 doc14 実装フェーズ表）（doc14 §交渉プロトコル） |
...
| `/operator/stop_request` | `std_msgs/String`（JSON） | 操作者非常停止要求（**latch**・単一 global topic＝fleet 全体）。`{"action": "engage"}` で latch ON／`{"action": "clear"}` の**明示解除のみ**で OFF。不明・不正 payload は**無視**（clear 扱いにしない＝fail-safe）。consumer = Emergency Guardian（L1・estop 理由 `operator_stop_request`）。producer = `warehouse_teleop.teleop_joy`（空きボタン rising edge で engage／解除 chord で clear・既定 index は mode-m1/03 §3・手動 `ros2 topic pub` も可）。契約正本: mode-m1/05 §5（§9 OQ-OP2 裁定） |
| `/bot{n}/cmd_vel/emergency` | `geometry_msgs/Twist` | Emergency Guardian（L1）の緊急停止指令（**estop 級のみ**・recovery 級では publish しない）。twist_mux **prio100** 入力（timeout 0.5s）として `/bot{n}/cmd_vel` を最優先で奪取し、estop 条件持続中は**毎 50ms tick zero Twist を再アサート（level）**（doc12:185 / doc15:389-404）。QoS・consumer（L2/L3 ミラー系）詳細の正本 = doc12 §【2026-09-07 追補】Emergency後の状態同期②（doc12:616・QoS 値は doc12:622） |
| `/bot{n}/stop_state` | `std_msgs/String`（JSON） | 停止上乗せ（L0' m1_driver）への停止状態 feed。`{"stop_requested": bool, "valid_until": float}` を**周期 publish**（停止要求が無い間も流す level チャネル）。`valid_until` は同一ホスト単調時計の絶対期限で**単調非減少**（逆行・非有限・非正・パース不能はすべて即時失権＝fail-closed）。producer = Emergency Guardian（L1・`emergency_guardian.py` が 50ms tick で周期 publish＝実装済）／ consumer = m1_driver（`stop_overlay_enabled: true` のときのみ購読）。QoS・上限クリップ・期限規律の正本 = mode-m1/05 §4-1 |

> ※ 上表は Jetson 内部の**全アプリ契約トピックを網羅**する（doc03 = トピック契約の単一参照カタログ）。各トピックの publisher/subscriber・プロトコル・ペイロード schema の**正本は 内容列がリンクする設計 doc**（doc12 State Cache / doc14 キャラLLM・交渉 / 11a VirtualScan / 12a Nav2 Bridge）であり、doc03 は topic 名・型・一行責務のみを保持する（詳細の二重管理＝ドリフトを避ける）。生 Nav2/tf・`/clock`・costmap 等の plumbing トピックは契約対象外（doc03 スコープ外）。**Nav2→twist_mux 速度チェーンの内部トピック（twist_mux prio10 入力 `/bot{n}/cmd_vel/nav2`・中間 `/bot{n}/cmd_vel/nav2_raw`）もこの plumbing に含めて除外する**: 両端とも既存 upstream コンポーネント（Nav2 controller_server / behavior_server・collision_monitor・twist_mux）のみで自作ノードの producer/consumer を持たず、一行責務セルが挿入トポロジ（doc12:543 / doc12:559）の二重管理になるため。正本 = doc15 §twist_mux（doc15:389-404）＋挿入トポロジ doc12:522 節＋凍結 `ws/src/warehouse_bringup/config/twist_mux.yaml`。これに対し `/bot{n}/cmd_vel/emergency` は自作ノード（L1 Emergency Guardian）が producer のため plumbing ではなく**収録**（上表の `/bot{n}/cmd_vel/emergency` 行＝#597）。 **2026-09-18 追記（04 Perception 出力 2 本）**: `/bot{n}/cliff_scan`・`/bot{n}/terrain/coverage` も本カタログの対象だが、**上表への中段挿入を行わず[本 doc 末尾の【2026-09-18 追補】](03-software-architecture.md:316)に同形式の表**で置く。理由は行ズレ回避——本行以降を指す `doc03:NNN` pin が **16 site / 18 occurrence・7 ファイル**（12 / 15 / 19 / 22 / jetson/01 / mode-m1/05 / mode-x-er/05）あり、2 行の挿入がそのすべてを同時に割るため（[#165 教訓](../dev/03-retrospectives.md) / [status-maintenance.md :26 末尾追記原則](../../.claude/rules/status-maintenance.md:26)、直前の同型事例 = [04 追補 ⑤-1](../mode-outdoor/04-perception-sidewalk-and-signals.md:907)）。**カタログとしての効力は上表と同一**（topic 名・型・一行責務のみを保持し、詳細の正本は内容列のリンク先）。
...
**2026-08-05: Jazzy から Humble へ変更した（[ADR-0008](../adr/0008-ros2-distro-humble-for-rosmaster-m1.md) accepted）。** 理由:
...
> **旧記述（2026-05-29 時点の判断・記録として保持）**: 当時は「Isaac ROS 最新版が Jazzy をターゲット」「Humble（EOL 2027年5月）より長いサポート期間」を理由に Jazzy を選定していた。前者は **Orin が Isaac ROS 4.x のサポート表から外れたことで無効化**され、後者（EOL 2029年5月 vs 2027年5月）はトレードオフとして受け入れた（ADR-0008 §トレードオフ）。
...
| 対応ROS 2 | **Humble 対応済み**（Jazzy 対応も 2026-05-22 に確認済だが、ADR-0008 により Humble を採用） |

注意:
- WiFi経由のUDP転送では遅延が生じる。タイムクリティカルな制御ループが必要になった場合はUSB有線接続を検討。
- micro-ROS は Humble・Jazzy とも公式対応。**ADR-0008 により ROS 2 Humble で統一する**（旧記述の「Humble フォールバック計画」が本線になった形）。
...
| `tiryoh/ros2-desktop-vnc:humble` | ROS 2 Humble + Gazebo（版は未決＝[ADR-0008](../adr/0008-ros2-distro-humble-for-rosmaster-m1.md) §Open。公式ペアは Fortress、Harmonic は非公式）。旧 `:jazzy` の GO 実績（gz-sim 8.11 Harmonic, ARM64-native, 2026-05-30 / #43）は **Jazzy+Harmonic のもので、Humble では再スパイクが要る** | Phase 0〜 |
...
| Web Console（`web_bridge` + `web/console`） | Mode A 会話・稟議・司令官判断のリアルタイム観測（observe-only・既存トピックの consumer。[doc22](22-web-observability.md)） | Phase 3〜 |
...
## 【2026-09-18 追補】Jetson 内部トピック（04 Perception 出力 2 本・`OQ-OD4Y-i` 裁定）

正本 = [mode-outdoor/04 追補 ⑨](../mode-outdoor/04-perception-sidewalk-and-signals.md)（topic / 型 / frame / stamp / QoS / 周期 / パラメータ / fail 方向 / hand-off）+ 型は [04 追補 ④](../mode-outdoor/04-perception-sidewalk-and-signals.md:390)（凍結契約 `warehouse_interfaces.perception`）。**本節の 2 行は「Jetson 内部」表（[:92](03-software-architecture.md:92)〜[:114](03-software-architecture.md:114)）と同じ効力・同じ形式**で、doc03 は topic 名・型・一行責務のみを保持する（[:116](03-software-architecture.md:116) のスコープ規約をそのまま適用。上表に挿入せず末尾に置いた理由も [:116](03-software-architecture.md:116) 同一行）。

| トピック | 型 | 内容 |
|---------|-----|------|
| `/bot{n}/cliff_scan` | `sensor_msgs/LaserScan` | 負障害物（縁石落ち・段差）の仮想スキャン。`DROP_DETECTED` セル**だけ**を方位・距離へ投影し、非 DROP は `inf` のまま（`UNKNOWN` は LaserScan に落とさない＝未観測を壁に変えない）。frame は `bot{n}/base_link`、`header.stamp` は**入力 depth frame の元計測時刻**（付け直さない）、**depth frame ごとに 1 回**（timer 無し・再 publish しない）。producer = `warehouse_perception.terrain_node`（`terrain_publisher`・既定 `enabled:=false` では subscription も publisher も作らない）／ consumer = 崖専用 ObstacleLayer instance（`cliff_layer`・local / global 両 costmap）と `collision_monitor`（source `cliff_scan`・**既定 `enabled: false`**）＝**配線済**（[04 追補 ⑩](../mode-outdoor/04-perception-sidewalk-and-signals.md:1009)。launch の arming は config `perception.terrain.enabled`＝同 §3、producer 側 launch は 04 追補 ⑫）。`source_timeout` の扱いは [`OQ-OD44`](../mode-outdoor/04-perception-sidewalk-and-signals.md:142)。QoS・パラメータ・fail 方向の正本 = 04 追補 ⑨ |
| `/bot{n}/terrain/coverage` | `std_msgs/String`（JSON） | 地形観測範囲・観測品質の自己申告（凍結契約 `warehouse_interfaces.perception.TerrainCoverage` の `model_dump_json()`。Phase 4 で `.msg` 化, doc16 §3）。LaserScan が表現できない `UNKNOWN`・観測済み範囲・元計測時刻・品質を運ぶ。producer = 同上（同じ depth frame から `cliff_scan` と**対で 1 回**・`header` を持たない型ゆえ時刻は payload の `source_stamp_s`）／ consumer = X2 `warehouse_safety.sensor_health`（**未配線**・field 名差は [`OQ-OD4Y-a`](../mode-outdoor/04-perception-sidewalk-and-signals.md:486)＝04 追補 ⑨ hand-off ②）。鮮度（`age = now − source_stamp_s`）の検査は**消費側の義務**で producer は `max_age` を配らない（[04 追補 ④ §2](../mode-outdoor/04-perception-sidewalk-and-signals.md:475)） **【2026-09-18 配線済】** consumer = X2 `warehouse_safety.sensor_health`（`terrain_health` アダプタ経由）を **Emergency Guardian が購読**し、品質不成立を estop 理由 `terrain_health` にする（`OQ-OD95` = A・arming は config `perception.terrain.enabled`）＝[04 追補 ⑬](../mode-outdoor/04-perception-sidewalk-and-signals.md:1270)。 |

> **QoS**（[:78](03-software-architecture.md:78) の produce/consume 規約に従う）: producer 側は両 topic とも **depth 10（既定 RELIABLE / KEEP_LAST）**。これは新しい値ではなく、既存 producer の踏襲である——`/bot{n}/virtual_scan` の `warehouse_traffic/virtual_scan.py` `create_publisher(LaserScan, ..., 10)`（[virtual_scan.py:42](../../ws/src/warehouse_traffic/warehouse_traffic/virtual_scan.py:42)）と `speed_limit` の [speed_band_node.py:73](../../ws/src/warehouse_perception/warehouse_perception/speed_band_node.py:73) が同じ深さを使う。consumer（costmap / collision_monitor / X2）との合意は配線スライスで行う（現時点で consumer は存在しない）。
