...
> §「Emergency後の状態同期」（:389-409）は Guardian → State Cache → `get_fleet_status` → LLM の**事後通知**フローのみを定義しており、L2 Policy Gate の `robot_in_emergency` 検査（doc15:307-309・実装 `ws/src/warehouse_mcp_server/warehouse_mcp_server/policy_gate.py` の `check_emergency`）へ emergency 状態を**供給する経路が未定義**だった＝`PolicyGate.set_emergency` は production で呼び出し元ゼロの never-fire（`policy_gate.py:288-297` / 構築点 `tools.py:118` は emergency を渡さない）。本追補がその feed を確定する。**凍結契約 `warehouse_interfaces`・`/emergency/event` コア形（:141-150）・twist_mux prio100 契約（doc15:389-395）はすべて不変**。additive tighten-only（reject が増える方向のみ＝[ADR-0004](../adr/0004-l2-restrict-only-policy-profile.md) と同じ向き）。

### 採用: Guardian estop level 信号のミラー（`/bot{n}/cmd_vel/emergency` 購読）

- **set**: L4 commander node（`llm_bridge`）が `/bot{n}/cmd_vel/emergency`（`geometry_msgs/Twist`・RELIABLE・doc15:392-402）を購読し、受信のたび `PolicyGate.set_emergency(bot, True)` へ写像する。Guardian の物理停止は **level**（estop 条件が続く限り毎 50ms tick 再アサート、:185 / `ws/src/warehouse_safety/warehouse_safety/emergency_guardian.py:224-229`）なので、この topic の継続受信自体が「Guardian がこの bot を停止させ続けている」の ground truth。
...
- **配置（layer 注記・productization/01:192 の layer ≠ process）**: 購読 marshal = **L4** node（`warehouse_llm_bridge/llm_bridge.py`）／ミラー純ロジック（stamp 管理・sweep）と config 検証 = **L2 Governance**（`warehouse_mcp_server/emergency_sync.py`・rclpy 非依存）／状態 = `PolicyGate._emergency`（**L2**）。検査自体は従来から `_validate_dispatch_inner`（`policy_gate.py:386`）と charging 経路で毎 dispatch 実行されている。
...
2. **State Cache `emergency.active` 参照**（#592 kickoff の当初候補）: active は **append-only の 50 件 ring で clear/resolution protocol が無い**（`ws/src/warehouse_state/warehouse_state/aggregator.py:165-182`・本 doc :401 も「active に追加」のみ）。これを feed にすると一度でも emergency を出した bot が**永続 reject**になる——既存 `self_action_gate._validate_live_state`（`ws/src/warehouse_llm_bridge/warehouse_llm_bridge/self_action_gate.py:192-197`）が実際にこの形で sticky（別件・本追補のスコープ外）。active の clear protocol は引き続き **Phase-2 TODO（safety-state 所有）**で、本追補はそれを代替しない（landed 後も L2 feed は level ミラーのまま＝独立系統）。

### 残（隠さない）

- **x_er_bridge（Mode X-ER commander）は未配線**: 自前 `WarehouseTools` 構築（`x_er_bridge.py`）に同じ never-fire が残る（既定 `mode_x_er.dispatch.forward_to_nav2: false`＝0 actuation で実害なし）。`emergency_sync` は純 module なので同型の購読 + sweep で配線可（XER レーン所有）。
- **stdio `server.py` 経路（Hermes 外部接続用の別プロセス MCP）は ROS 文脈が無くミラー不能**＝当該プロセスの emergency 検査は従来どおり never-fire。production の tool dispatch は in-process（doc16 §9 / #81）でありこの経路は現用外。
