...
- **不採用＝`/operator/stop_request` 互換 JSON を出す新 producer**。[doc03:112](03-software-architecture.md)／[mode-m1/05 §5](../mode-m1/05-operation-state-and-stop-authority.md) の通りこの topic は **fleet 全体の単一 latch**（`OperatorStopLatch` は bool 1 個＝`guard_logic.py:473-504`）で、producer は `teleop_joy`（人の意思表示）。機械 producer が `clear` を出すと**人が engage した停止まで解除**しうる（§5「停止理由は理由ごとに保持」に反する）。per-bot にもできず、payload に `source`／理由を足すのは契約変更（doc03:112・§5・`parse_operator_stop_action`）で、採用案なら不要。よって不採用（`/operator/stop_request` は人の意思表示専用のまま）。
- **impl slice 仕様**（`track:safety-state`・編集は `warehouse_safety`＋`config/warehouse.base.yaml` `safety:` 節・凍結契約不変・**本追補時点では未実装**）。各行 = [何をするか] — 根拠 — 検証:
  1. `BotState.scan_age: float | None = None`（初回受信前 `None`）と `BotState.odom_seen: bool = False`（`/{bot}/odom` を 1 度でも受信したか）を additive 追加 — `guard_logic.py:21-39` の既存 idiom（`pose_age` :29） — 既定値で既存 unit 全緑（非回帰）。
  2. `evaluate(..., scan_freshness_timeout=...)` に (6) ブロック: `scan_age > scan_freshness_timeout`（厳密 `>`・`pose_stale` :233 と同じ）**または**（`scan_age is None` **かつ** `odom_seen`）→ `Decision(bot, "estop", "scan_stale", {"scan_age", "freshness_timeout"})` — 「bot が生きている証拠（odom）があるのに scan が一度も来ない」を fail-closed にし、bot 不在（単騎 [ADR-0006](../adr/0006-single-bot-first.md)・`_BOTS` 2 固定＝`emergency_guardian.py:46`）では無音のまま — R-26 unit（独立オラクル＝手計算の age。`>`→`>=`／`None` 規則／odom 証人の各 mutation で赤になること）。
  3. node: `/{bot}/scan` を `sensor_qos`（`emergency_guardian.py:117-119`）で購読し**到着時刻（`time.monotonic()`）のみ**を記録する（header stamp は使わない＝sim／実機の時計差を持ち込まない。`_on_pose` :196-202 と同型）。`_on_odom` :204-213 で `odom_seen` を立てる。callback に logic を置かない（:195 の規律）。
  4. しきい値 `safety.scan_freshness_timeout`（`config/warehouse.base.yaml` `safety:` 節・既定 **1.0 s**） — 出所は [`collision_monitor.yaml:58`](../../ws/src/warehouse_bringup/config/collision_monitor.yaml:58) の node-level `source_timeout 1.0`（:552 ③ の live-tune 対象と**同一値を継承**＝発明ではない。T-mini Plus の走査 6-12 Hz＝83-167 ms（[doc23:494](23-perception-and-localization.md)）に対し ~6-12×） — `load_config` の hard-index（:82-91 と同じ「欠落は起動失敗」）＋既定値 pin の unit。
  5. 動作＝**level・自動解除**（scan 再開で `EdgeLatch` が落ちる＝`pose_stale` :511 と同じ）。event `type="scan_stale"` は additive（コア形 :141-150 不変）。`/{bot}/stop_state` へは既存 `stop_requested_for`（`emergency_guardian.py:290-310`）経由で自動伝播＝**L0' 停止上乗せ有効時は teleop 経路も止まる**（twist_mux を経ない `teleop_joy`→`/bot1/cmd_vel` 直 publish は prio100 では止まらない＝[mode-outdoor/09:16](../mode-outdoor/09-external-review-v3-response.md) の事実。overlay 無効の構成では Nav2 経路のみ）。
