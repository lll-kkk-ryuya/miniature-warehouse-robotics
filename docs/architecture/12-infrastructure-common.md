...
- **【2026-09-16 追記②】age の下限（負 age・NaN ＝ 時計不一致 → 「age 不明」）の裁定**: `scan_age` / `pose_age` は node が同一 tick の `time.monotonic()` から差分で算出し（上記 3.・:509）、単調時計は逆行しないため負にならない。純ロジック `guard_logic.evaluate` を別時計で呼ぶ caller（sim harness 等）が**負または NaN の age** を渡した場合、それは鮮度の測定値ではなく**age 不明（`None` と同格）**として扱う（`guard_logic.age_is_unknown`）——`scan_age` は上記 2. の `None` 規則（`odom_seen` なら fault → `scan_stale` estop・bot 不在なら無音）＝fail-closed。event `detail.scan_age` は**有限値のみ**を載せ、不明／非有限は `null`（NaN / Infinity を JSON に出さない＝:293 と同じ規律）。`pose_age` は :509 の `None` 規則（初回 pose 前は stale としない）＝estop しない＝**現行挙動と同一で code 不変**（unknown pose age の安全側は「未 localize の停止ロボットを estop しない」であり、変位ゲートの unknown odom（「抑止しない」＝[23 A-5③](23-perception-and-localization.md)）とは向きが異なる。各規則は各自の既定に従う）。`+inf` は不明ではなく「どの窓より古い」＝厳密 `>` で従来通り stale。有限・非負の age は bit-identical（additive・estop が増える方向のみ・凍結契約 / config / topic / event コア形 :141-150 は不変）。同型の裁定＝変位ゲート `PoseGateTracker.snapshot` の閉区間 `0 <= age <= stale_after`（PR #684・`warehouse_safety/CLAUDE.md`【2026-09-16 追記】）。R-26: `tests/unit/test_guardian_scan_stale.py` 末尾（不明 age × odom 証人の有無・`None` 同値・`+inf`・有限域の不変・`pose_age` 同型 pin・mutation で赤）。
