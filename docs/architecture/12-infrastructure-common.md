...
- **impl slice 仕様**（`track:safety-state`・編集は `warehouse_safety`＋`config/warehouse.base.yaml` `safety:` 節・凍結契約不変・**実装済 = `feat/guardian-scan-stale`（R-26 unit `tests/unit/test_guardian_scan_stale.py`・5 変異体 KILLED）**）。各行 = [何をするか] — 根拠 — 検証:
