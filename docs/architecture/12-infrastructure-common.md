...
|---|---|
| **戦略**（Non-RT / Layer 3） | LLM Bridge Node（Python・`warehouse_llm_bridge`）／ Hermes Gateway（Python・GCP VM・NousResearch Hermes）／ Warehouse MCP Server（Python・`warehouse_mcp_server`）／ nav2_bridge REST（Python・`warehouse_nav2_bridge`）／ LLM API（Claude/GPT/Gemini/Grok）＋ Langfuse ／ **CTRV 予測** `predicted_position_3s`（`situation.py:171-189`・旋回=円弧/直進=CV・**Mode A/B のみ**） |
...
│             （situation.py:171-189・旋回=円弧/直進=CV・Mode A/B のみ）      │
