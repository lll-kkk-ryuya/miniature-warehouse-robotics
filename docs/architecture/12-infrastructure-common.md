...
- **判定（純ロジック・R-26 unit）**: `pose_stale` を **`pose_age > pose_freshness_timeout` ∧ 変位ゲート開** に限定する。ゲートは独立ソース（wheel odom `/{bot}/odom`・`nav_msgs/Odometry`・doc03:77）から、**前回 pose 到着以降の累積変位 > `safety.pose_freshness_motion_epsilon`**／**累積 `|Δyaw|` > `safety.pose_freshness_angular_epsilon`**／**odom 速度 > `safety.pose_freshness_speed_epsilon`** のいずれかで開く。実装 = `guard_logic.pose_gate_open` / `guard_logic.PoseGateTracker`（rclpy 非依存）、node 側は marshal のみ。
- **しきい値（すべて config・`config/warehouse.base.yaml`）**: `pose_freshness_motion_epsilon: 0.10`（= 2 × `update_min_d`）／`pose_freshness_angular_epsilon: 0.4`（= 2 × `update_min_a`）／`pose_freshness_speed_epsilon: 0.02`（`BlockTracker` の epsilon と同オーダ）／`odom_freshness_timeout: 0.5`。**2× は「健全な AMCL なら必ず publish しているはず」の余裕**（正常 jitter でゲートが空振りしない）。**TODO Phase-2: 実機で確定**（既存の暫定値群と同格）。
...
- **非緩和性（R-26 で機械証明）**: 走行中の発火 tick が CURRENT と一致することを unit で固定する（`tests/unit/test_guardian_displacement_gate.py`）。**唯一の残留** = 速度がちょうど `speed_epsilon` 以下の匍匐前進は発火が遅れうるが、上限は `motion_epsilon / speed_epsilon`（既定 5.0s）に有界で、これも unit で pin 済。
