...
Warehouse MCP Server（Policy Gate）側で、**top-level `timestamp` の経過時間**に応じて robot 鮮度を判定する。鮮度はスナップショット単位（全 robot 共通）で、`availability` は **契約フィールドではなく MCP が局所導出**する（#5 が明示的な `availability` を出すまでの暫定。実装: `warehouse_mcp_server.policy_gate`）。**しきい値は config キー** `policy_gate.stale_after_s` / `policy_gate.unavailable_after_s`（`config/warehouse.base.yaml` の base 既定 **0.5 / 2.0**・env overlay で上書き可・additive）で、下の code は**既定値**を示す（`FreshnessThresholds` が seed）。**fail-closed 検証**: 非数値・非有限・`<=0`・`stale_after_s > unavailable_after_s`・**上限 `> MAX_FRESHNESS_S=10.0s`**（valid-but-huge な overlay が gate を defang するのを防ぐ hard ceiling。`warehouse_interfaces/config.py:101` の `MAX_LINEAR_VELOCITY` cap 前例に倣う）・**非 mapping な `policy_gate` block** は起動拒否し、既定への黙示 fallback をしない（overlay は `≤MAX_FRESHNESS_S` に有界）。既定は現行値と同一で gating を緩めず、全環境で prod と同一 gating（sim も mirror・operator 決定）。用語集: `GLOSSARY.md` の `policy_gate`（config sub-tree）:
