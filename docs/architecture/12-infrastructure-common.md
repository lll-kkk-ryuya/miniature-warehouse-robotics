...
| **物理安全**（即時 / Layer 0） | ESP32 firmware（**C++ 自作**・FreeRTOS・PlatformIO・`firmware/`）／ micro-ROS（C・XRCE-DDS）／ micro-ROS Agent（C++・Jetson 上）／ on-robot センサ MS200（`/scan`）・エンコーダ・バッテリ（※**RPLiDAR A1 は Jetson-USB 固定の外部トラッキング用・optional**＝on-robot ではない。doc02:179-180 / doc03:170） |
...
- **`/emergency/event` 後方互換**: `type` に新値 `"pose_stale"` を足すのみ ＝ **additive**。コア形（`event_id/robot/type/severity/action_taken/timestamp/requires_llm_review` [+任意 `detail`]、:141-150）は不変で、State Cache は `active`/`history` ring に積むだけ（既存購読者は無視可）。**凍結契約 `warehouse_interfaces` は無変更**。（`"scan_stale"` も同様に additive＝末尾【2026-09-16 追補】(3)・実装 `feat/guardian-scan-stale`）
- **scan 鮮度は対象外（Phase-2 defer）**: `/scan` の stale 停止（`source_timeout`）は本来 `nav2_collision_monitor` のパラメータであり（R-39, doc07:249）、Guardian は `/scan` を購読しない（§:97 の collision_monitor 委譲。cmd_vel 挿入トポロジは下記 §「collision_monitor 委譲」で確定済、配線 impl は nav-traffic 調整で defer）。本節の freshness は **amcl_pose のみ**を対象とする、collision_monitor 採用までの interim な Guardian policy 層ガードである。**【2026-09-16 訂正】この委譲は Humble 1.1.20 では不成立（`source_timeout` は点を落とすだけ＝fail-open・末尾【2026-09-14 追補】）。scan 鮮度停止は Guardian の `scan_stale` へ再配置＝末尾【2026-09-16 追補】(3)。**

### blocked → Nav2 `progress_checker` 委譲（責務の明文化）

- **責務の所在**: nav 実行中の進捗喪失（経路追従が進まない）は **Nav2 `progress_checker`（`nav2_controller::SimpleProgressChecker`、`ws/src/warehouse_bringup/config/nav2_params.yaml:94-97`：`required_movement_radius 0.05` / `movement_time_allowance 10.0`）が `FAILED_TO_MAKE_PROGRESS` を上げ controller recovery を起動**する責務である（`nav2_params.yaml` 所有 = nav-traffic, doc16:193）。Guardian はこれを置換しない（既存パラメータで足り、本スライスは `nav2_params.yaml` を編集しない）。
...
> :97 / :427 が「cmd_vel 挿入トポロジ未定義」のため defer していた collision_monitor 委譲の**トポロジを確定**する（docs-first 先行）。本節は **docs のみ**で、実配線（launch remap・collision_monitor config）は **nav-traffic 所有ファイル**（`nav2_bringup.launch.py` / `nav2_params.yaml`、doc16:193）に触れるため **nav-traffic 調整の impl slice に defer**。Guardian は **`/scan` を購読しない**（policy 層のまま）。

**配置 = Nav2 速度経路上・twist_mux の nav2(prio10) 入力の上流**（emergency prio100 は不変）:

```
Nav2 controller_server (FollowPath)
  └(remap cmd_vel)→ /bot{n}/cmd_vel/nav2_raw          ← 新・中間 plumbing topic（doc03 スコープ外, doc03:116）
       nav2_collision_monitor  (per-bot, /bot{n} ns)
         ...
         polygons = stop（＋任意 slowdown）／ source_timeout（/scan 途絶→停止）  ← Humble 1.1.20: 途絶は点が消えるだけ（fail-open・末尾 2026-09-14/16 追補）
...
- **source_timeout と #152 freshness は別系統の補完**: collision_monitor `source_timeout` = **/scan 鮮度**→物理停止（nav 経路）。Guardian `pose_freshness_timeout`（#152）= **/amcl_pose 鮮度**→policy estop（localization 整合）。別センサ・別層で重複しない（:427 が defer としていた scan 鮮度は本トポロジで collision_monitor 責務と確定）。**【2026-09-16 訂正】Humble 1.1.20 では collision_monitor 側の「/scan 鮮度→物理停止」は成立しない（末尾【2026-09-14 追補】）。scan 鮮度停止の責務は Guardian `scan_stale`（末尾【2026-09-16 追補】(3)）。**
...
- **defer / 触らない**: 配線は nav-traffic 所有（`nav2_bringup.launch.py` remap・collision_monitor config・lifecycle_manager 登録・`nav2_params.yaml`、doc16:193）＝ nav-traffic Issue へ予告し調整。nav_status gating（BlockTracker idle 誤検出解消）は Phase-2。**凍結契約 `warehouse_interfaces` 不変・twist_mux emergency prio100 不変**。
...
## 【2026-09-07 追補】Emergency後の状態同期②: L2 Policy Gate への同期（`robot_in_emergency` の feed 確定・#592）

> §「Emergency後の状態同期」（:389-409）は Guardian → State Cache → `get_fleet_status` → LLM の**事後通知**フローのみを定義しており、L2 Policy Gate の `robot_in_emergency` 検査（doc15:307-309・実装 `ws/src/warehouse_mcp_server/warehouse_mcp_server/policy_gate.py` の `check_emergency`）へ emergency 状態を**供給する経路が未定義**だった＝`PolicyGate.set_emergency` は production で呼び出し元ゼロの never-fire（`set_emergency`/`is_in_emergency`＝`policy_gate.py:290-299` / 構築点 `tools.py:118` は emergency を渡さない）。本追補がその feed を確定する。**凍結契約 `warehouse_interfaces`・`/emergency/event` コア形（:141-150）・twist_mux prio100 契約（doc15:389-395）はすべて不変**。additive tighten-only（reject が増える方向のみ＝[ADR-0004](../adr/0004-l2-restrict-only-policy-profile.md) と同じ向き）。

### 採用: Guardian estop level 信号のミラー（`/bot{n}/cmd_vel/emergency` 購読）

- **set**: L4 commander node（`llm_bridge`）が `/bot{n}/cmd_vel/emergency`（topic 名/timeout=doc15:392-402・型 = `geometry_msgs/Twist`・QoS = **RELIABLE / KEEP_LAST / depth 10**＝この値が docs 正本（doc03:113 行・doc15:569 から本行へ委譲・Guardian publisher＝`EmergencyGuardian.__init__` の `reliable_qos`（RELIABLE / KEEP_LAST / depth 10）と `self._cmd_pub[bot]`（`create_publisher(Twist, f"/{bot}/cmd_vel/emergency", reliable_qos)`）・symbol 参照・以後 verify のみ）。購読側も同じ RELIABLE を明示＝QoS 非互換の silent no-match で fail-open にしない）を購読し、受信のたび `PolicyGate.set_emergency(bot, True)` へ写像する。Guardian の物理停止は **level**（estop 条件が続く限り毎 50ms tick 再アサート、:185 / `ws/src/warehouse_safety/warehouse_safety/emergency_guardian.py` の `_check_safety` → `_emergency_stop`＝毎 tick `Twist()` publish ＋ goal cancel・symbol 参照）なので、この topic の継続受信自体が「Guardian がこの bot を停止させ続けている」の ground truth。
...
- **意味論**: estop 級（near_collision / battery_critical / pose_stale）のみ dispatch を塞ぐ。**recovery（blocked_timeout・low-harm）は `/cmd_vel/emergency` を publish しない**（`emergency_guardian._trigger_recovery`＝event のみ・`Twist` を publish しない・symbol 参照）ため塞がない＝意図どおり（bot は正当に idle かもしれない）。battery_critical 中は `send_to_charging` も reject される（:250 の表「≤10% = 全コマンド拒否」と一致。物理的にも Guardian が毎 tick zero Twist + Nav2 cancel を維持するため充電走行は不能＝人手回収）。

### 却下した 2 案（falsification・再検討時の再訪先）

1. **`/emergency/event` 駆動 set**: event は rising edge のみ（:185 / `EdgeLatch`）で clear イベントが無い上、bridge プロセス再起動で edge を取りこぼすと**復元手段の無い fail-open**（条件持続中は再発行されない）。level ミラーは次 tick（≤50ms）で自己回復する。
2. **State Cache `emergency.active` 参照**（#592 kickoff の当初候補）: active は **append-only の 50 件 ring で clear/resolution protocol が無い**（`ws/src/warehouse_state/warehouse_state/aggregator.py:165-182`・本 doc :401 も「active に追加」のみ）。これを feed にすると一度でも emergency を出した bot が**永続 reject**になる（厳密には 50 件 ring の押し出しで偶発的に外れうる＝決定論的 clear が無い）——既存 `self_action_gate._validate_live_state`（`ws/src/warehouse_llm_bridge/warehouse_llm_bridge/self_action_gate.py:192-197`）が実際にこの形で sticky（別件・本追補のスコープ外）。active の clear protocol は引き続き **Phase-2 TODO（safety-state 所有）**で、本追補はそれを代替しない（landed 後も L2 feed は level ミラーのまま＝独立系統）。
...
## 【2026-09-10 追補】State Cache の idle 判定しきい値 = `warehouse_interfaces.safety.IDLE_SPEED_EPS`（#642）

> :258「State Cache Node」と :297「State Cache JSON フォーマット」は `status` が線速度から `derive_status` で導出されること（:340）と、値域が `moving`/`idle` のみであること（:520）を規定していたが、**「停止（idle）」を分ける数値そのもの＝ε は本 doc のどこにも無かった**（実査: 本追補以前の doc12 に `0.01` / `_MOVING_EPS` の記述ゼロ）。実体は `warehouse_state` レーン私有の定数で、同じ値を観測 KPI（doc21:452 の idle 率）が**借用**していた。本追補はその ε を**凍結契約 `warehouse_interfaces.safety` の単一定数として確定**する。**値は 0.01 m/s のまま・境界も不変＝挙動変更ゼロの additive 契約変更**（[parallel-workflow.md §7.2](../../.claude/rules/parallel-workflow.md)）。

### 確定

- **定義**: `derive_status(linear)` は `abs(linear) > IDLE_SPEED_EPS` のとき `"moving"`、それ以外（`abs(linear) ≤ ε`）は `"idle"` を返す。**境界 ε ちょうどは idle**（厳密 `>`）。実装 = [`ws/src/warehouse_state/warehouse_state/aggregator.py`](../../ws/src/warehouse_state/warehouse_state/aggregator.py) の `derive_status`（:340 が指す導出関数）。
- **正本（単一ソース）**: `IDLE_SPEED_EPS: float = 0.01  # m/s` ＝ [`ws/src/warehouse_interfaces/warehouse_interfaces/safety.py`](../../ws/src/warehouse_interfaces/warehouse_interfaces/safety.py)。`MAX_LINEAR_VELOCITY` と**同じ import 規律**（[safety.py:8-12](../../ws/src/warehouse_interfaces/warehouse_interfaces/safety.py)「import them directly and do NOT hardcode」）＝**消費者は import する・`0.01` をレーン側で再タイプしない**。ただし**同じ規律なのは import だけで ε は cap ではない**（同 docstring:「shares the import-never-retype rule but is NOT a cap — it is an observation/status threshold; no actuation path may clamp to it」）。
- **消費者は 2 つ**: ① State Cache の `derive_status`（本 doc・状態導出）／② `warehouse_orchestrator.motion` の idle 率（doc21:452 の窓スコープと doc21:466 の run スコープの**両方**が同一 ε）。両者が同一定数を import するため、「停止」の意味が 2 レーンで分岐しない。
- **actuation には関与しない（重要）**: ε は**観測・状態導出専用**のしきい値であって速度上限ではない。`clamp_velocity` は ε を参照せず、L0（ESP32 firmware）/ L0'（`warehouse_m1_driver` のベクトルクランプ）/ L1（Emergency Guardian の 50ms reflex）/ L2（Policy Gate）の速度強制・estop 判定も ε を読まない。速度の hard cap は従来どおり `MAX_LINEAR_VELOCITY`（:254 / [ADR-0010](../adr/0010-raise-speed-cap-to-platform-max.md)）のみである。
- **`status` 値域は不変**: `moving` / `idle` の 2 値のみで `"blocked"` / `"stale"` は導出しない（:520 のガードレールは本追補で変わらない）。ε の凍結は「どの速度を idle と呼ぶか」の所有者を決めるだけで、値域にも `RobotSnapshot` 契約にも触れない。

### 経緯（provenance・#642 以前）

- 旧実装は `aggregator` 私有の `_MOVING_EPS = 0.01`（**別トラック所有・凍結契約ではない**）と、`warehouse_orchestrator.motion.IDLE_SPEED_EPS` の**ミラー定数**の 2 本立てだった。wo レーンは cross-lane import 禁止（[parallel-workflow.md §2.1](../../.claude/rules/parallel-workflow.md)）のため import できず、`tests/unit/test_wo_motion_kpi.py` の cross-check test で両者の**等値**を pin していた。
- 問題は、等値 pin が「値が一致していること」しか担保せず**どちらが正本かを決めない**点（片方を変えれば test が赤くなるだけで、正しい直し方が決まらない）。doc21:469 の follow-up (1) と #632 B1 がこれを未決として残していた。
...
## 【2026-09-14 追補】`nav2_collision_monitor` の `source_timeout` は Humble では「点が消えるだけ」（Mode Outdoor レビュー由来・fail-open の明記）

Mode Outdoor の外部レビュー v3 照合（[mode-outdoor/09 §1 #1](../mode-outdoor/09-external-review-v3-response.md)）で、本 doc の **[:513](12-infrastructure-common.md:513)「`/scan` の stale 停止（`source_timeout`）」と [:546](12-infrastructure-common.md:546)「`source_timeout` = `/scan` 鮮度 → 物理停止」は Jazzy / main の意味論**であり、**Humble 1.1.20（[ADR-0008](../adr/0008-ros2-distro-humble-for-rosmaster-m1.md)）では成立しない**ことを一次情報で確認した（参照日 2026-09-14・[D]）:

- humble `nav2_collision_monitor/src/scan.cpp` `Scan::getData()`: `if (data_ == nullptr) { return; }` / `if (!sourceValid(...)) { return; }`＝未受信・期限切れの source は**点を足さずに戻る**。`CollisionMonitor::process()` は `DO_NOTHING` から始まり点が無ければ入力 cmd_vel をそのまま通す → **センサ途絶 = fail-open**。
- humble `src/source.cpp` `Source::getCommonParameters()` が読む per-source 項目は `topic` と `enabled` のみ。**per-source `source_timeout` は main 系のみ**（`declare_or_get_parameter(source_name_ + ".source_timeout", ...)`）。したがって [`collision_monitor.yaml:90`](../../ws/src/warehouse_bringup/config/collision_monitor.yaml:90) の `virtual_scan.source_timeout: 0.0` は Humble では**読まれない**（node-level 1.0 s が全 source に適用）。同 yaml [`:58`](../../ws/src/warehouse_bringup/config/collision_monitor.yaml:58) のコメント「REAL /scan dropout → STOP」も Humble では成立しない。**【config PR `feat/collision-monitor-humble-config`】同 key・`state_topic`・`min_points` は撤去し `max_points: 3` を明示（:90/:56 は撤去注記に置換・行数不変）。**
- `stop_pub_timeout`（yaml `:61` = 2.0）は**停止開始からの計時**で、経過後は障害物が残っていてもゼロ出力の publish を止める（`collision_monitor_node.cpp` `stop_stamp_`）。現行配線では mux の下流に他入力が無く `m1_driver` W-1（0.5 s）で止まる [I] が、mux に手動入力を足す設計（Mode Outdoor v2.1）では「高優先入力の消失 = 復帰許可」にしてはならない。
- 帰結: Humble 構成で「センサ途絶で止まる」性質が要るなら CM の外で担保する（Mode Outdoor では X2 鮮度監視 → 期限付き走行許可の失効＝[mode-outdoor/09 §2-b](../mode-outdoor/09-external-review-v3-response.md)・`OQ-OD95/96`）。本節は主張の訂正であり、code / yaml の変更は別 PR（`track:safety-state`）。
- 一次情報: <https://raw.githubusercontent.com/ros-navigation/navigation2/humble/nav2_collision_monitor/src/scan.cpp> / <https://raw.githubusercontent.com/ros-navigation/navigation2/humble/nav2_collision_monitor/src/source.cpp> / <https://raw.githubusercontent.com/ros-navigation/navigation2/humble/nav2_collision_monitor/src/collision_monitor_node.cpp>（humble）・main の `source.cpp` <https://raw.githubusercontent.com/ros-navigation/navigation2/main/nav2_collision_monitor/src/source.cpp>（参照日 2026-09-14）。

## 【2026-09-16 追補】Humble でのセンサ途絶停止（R-39「lidar dropout → stop」）の所在 ＝ Emergency Guardian の `scan_stale` 拡張（`/operator/stop_request` 互換 producer 案は不採用）

> 直前の【2026-09-14 追補】（:668-676）が「Humble 1.1.20 では `nav2_collision_monitor` の `source_timeout` は点を落とすだけ＝fail-open」と訂正したのを受け、本追補は **(1) 一次情報の再照合と追加事実、(2) `collision_monitor.yaml` 注記の是正（`OQ-OD96` の code/yaml 応答・値は不変）、(3) センサ途絶停止の責務の所在の裁定と impl slice 仕様**を確定する。layer: 対象は **L1**（collision_monitor / Emergency Guardian＝[productization/01:185](../productization/01-commercial-box-map.md)）。**凍結契約 `warehouse_interfaces`・twist_mux prio100・`/emergency/event` コア形（:141-150）はすべて不変（additive のみ）**。

### (1) 一次情報（参照日 2026-09-16・[D]＝原文を取得して照合）

- **版**: rosdistro `humble/distribution.yaml` の `navigation2` release は `version: 1.1.20-1`（source = `humble` ブランチ）<https://raw.githubusercontent.com/ros/rosdistro/master/humble/distribution.yaml>。humble ブランチ先端 `3c3db59d6969d8ecee8e68468693d006397f4a0c` と tag `1.1.20`（`a097086719c88f781aa59788eca29ac6ca5e56db`）で `nav2_collision_monitor` の `src/source.cpp` / `src/scan.cpp` / `src/collision_monitor_node.cpp` / `src/polygon.cpp` / `src/circle.cpp` / `package.xml`（`<version>1.1.20</version>`）は**バイト同一**（`cmp`）＝以下の行番号は両者共通。**ボード（Jetson）の apt 導入版は本セッションでは未確認**（Tailscale・LAN とも到達不能）。別途 [jetson/02:276](../jetson/02-remote-access-and-dev-link.md) の `jetson status` 経路で `dpkg-query -W ros-humble-nav2-collision-monitor` を取り、`1.1.20-1` でなければ本節を再照合する。**【2026-09-16 実査済】** ボード実査（`jetson status`: UP・ROS 2 humble・`/opt/warehouse @ c79d34c`・clock ok。読み取り専用 `dpkg-query -W` / `apt-cache policy` / 導入済 `package.xml`）: `ros-humble-nav2-collision-monitor` **`1.1.20-1jammy.20260804.215756`**（installed ＝ candidate・`package.xml` `<version>1.1.20</version>`）／`ros-humble-navigation2` `1.1.20-1jammy.20260805.013510`／`ros-humble-nav2-mppi-controller` `1.1.20-1jammy.20260804.230127`＝rosdistro `1.1.20-1` と一致し、本節の行番号照合はボード導入版にそのまま適用できる。なお同時点のボード checkout は `c79d34c`（#678〜#685 より前＝`scan_stale` 実装・yaml 是正は**未デプロイ**。Build≠Deploy＝[jetson/03](../jetson/03-build-deploy-run-and-run-records.md) の別工程）。
- `src/source.cpp:60-75` `Source::getCommonParameters()` が declare するのは `<src>.topic`（:67-70）と `<src>.enabled`（:72-74）のみ。`:77-94` `sourceValid()` は `dt > source_timeout_` で `"Ignoring the source."` を WARN して `false`。per-source `.source_timeout` は **main の `src/source.cpp:173-176`**（node 既定を継承・`:186` の `!= 0.0` で無効化可）にしか無い。
- `src/scan.cpp:56`「Laser scanner has no own parameters」。`:65-76` `Scan::getData()` は `void` で、`data_ == nullptr`（:71）／`!sourceValid`（:74）のとき**点を足さずに return**（`:82-89` の TF 失敗も同様に無言 return）。
- `src/collision_monitor_node.cpp:343-403` `process()`: `:357-361` で各 source の点を集め、`:364` `Action robot_action{DO_NOTHING, cmd_vel_in}` から始めて polygon 判定（`:411` `getPointsInside > getMaxPoints()` で STOP）以外に停止契機は無い。main の `:454-458`「invalid source」STOP（`getSourceTimeout() != 0.0` 条件）は **Humble に存在しない**。
- `:160-171` `publishVelocity()`: 出力ゼロの**最初の tick**で `stop_stamp_` を取り（:163-165）、`stop_pub_timeout_` 経過後は **publish 自体を止める**（:166-170）。既定値は `stop_pub_timeout` 1.0（:219-222）／`source_timeout` 2.0（:210-213）。
- **追加事実（本追補で新規・yaml 注記に反映）**: ① `state_topic` は Humble の node に declare されず `nav2_msgs/CollisionMonitorState` publisher も無い（main の `:57-76` のみ）＝[`collision_monitor.yaml:56`](../../ws/src/warehouse_bringup/config/collision_monitor.yaml:56) は inert で、**:558 の「`collision_monitor_state` 購読で surfacing」turnkey は Humble では成立しない**。② polygon の点数しきい値は `src/polygon.cpp:261-263` の `max_points`（既定 3）で、`min_points` は Iron 以降の改名＝[`collision_monitor.yaml:70`](../../ws/src/warehouse_bringup/config/collision_monitor.yaml:70) の `min_points: 4` は inert だが既定 `>3` ＝ **≥4 点で同じしきい値**（[mode-x-er/10:268](../mode-x-er/10-room-scale-safety-review.md) の点数論証は数値的に維持）。③ `polygon.cpp:257-259` `<polygon>.enabled`・`:282-289` `visualize`／`polygon_pub_topic`・`circle.cpp:90-91` `radius`・node `:193-198` `cmd_vel_in/out_topic`・`:206-217` `transform_tolerance`／`source_timeout`／`base_shift_correction`・`:219-222` `stop_pub_timeout`・`:294-301` `observation_sources`／`<src>.type` は Humble で有効＝yaml の残りのキーは生きている。④ [tests/e2e/README.md:272](../../tests/e2e/README.md) の実測「`invalid source` STOP at dropout + ~1.0s」は **Jazzy nav2** での値（[deploy/dev/install-nav2-e2e.sh:21](../../deploy/dev/install-nav2-e2e.sh) `ros-jazzy-navigation2`＝[ADR-0008 §適用状況](../adr/0008-ros2-distro-humble-for-rosmaster-m1.md) 表の既知ドリフト「jazzy のまま」）で、Humble へ転写できない。
- URL（参照日 2026-09-16）: humble <https://raw.githubusercontent.com/ros-navigation/navigation2/humble/nav2_collision_monitor/src/source.cpp> ／ <https://raw.githubusercontent.com/ros-navigation/navigation2/humble/nav2_collision_monitor/src/scan.cpp> ／ <https://raw.githubusercontent.com/ros-navigation/navigation2/humble/nav2_collision_monitor/src/collision_monitor_node.cpp> ／ <https://raw.githubusercontent.com/ros-navigation/navigation2/humble/nav2_collision_monitor/src/polygon.cpp> ／ <https://raw.githubusercontent.com/ros-navigation/navigation2/humble/nav2_collision_monitor/src/circle.cpp> ／ <https://raw.githubusercontent.com/ros-navigation/navigation2/humble/nav2_collision_monitor/package.xml>、tag 1.1.20 は同 path の `humble` を `1.1.20` に置換（例 <https://raw.githubusercontent.com/ros-navigation/navigation2/1.1.20/nav2_collision_monitor/src/source.cpp>）、main <https://raw.githubusercontent.com/ros-navigation/navigation2/main/nav2_collision_monitor/src/source.cpp> ／ <https://raw.githubusercontent.com/ros-navigation/navigation2/main/nav2_collision_monitor/src/collision_monitor_node.cpp>。

### (2) `collision_monitor.yaml` 注記の是正（`OQ-OD96` への応答・値不変）

- [`collision_monitor.yaml:38-46`](../../ws/src/warehouse_bringup/config/collision_monitor.yaml:38)（header）・`:56`・`:58-59`・`:61`・`:70`・`:85-89` のコメントを Humble 意味論（センサ途絶＝fail-open／per-source `source_timeout` 不読／`stop_pub_timeout` は出力停止の計時／`state_topic`・`min_points` inert）へ書き換えた。**行数 90・pinned 行（:22-25／:26-29／:31-37／:55／:56／:58／:61／:63-68／:70／:72／:76／:81-90／:90）の位置・全パラメータ値は不変**（`yaml.safe_load` の同値を確認）。
- **値の変更**（`max_points: 3` の明示・`state_topic` 撤去・`virtual_scan.source_timeout` 撤去）は本追補を正本とする**別 config PR**（docs-first。[tests/unit/test_collision_monitor_config.py](../../tests/unit/test_collision_monitor_config.py) の値 pin と [:552](12-infrastructure-common.md:552) ③ の live-tune を同時に更新）。→ **実施 = `feat/collision-monitor-humble-config`**（`state_topic`／`virtual_scan.source_timeout`／`min_points` を撤去し `max_points: 3` を明示。Jazzy 1.3.13 は `max_points` を互換 shim（`min_points = max_points + 1`・WARN）で読むため両 distro で ≥4 点。yaml は 90 行・pinned 行不変＝撤去行は注記コメントに置換。**注意: dev の 2 台 Gazebo コンテナは依然 Jazzy（`deploy/dev/install-nav2-e2e.sh`・ADR-0008 §適用状況の既知ドリフト）で、そこでは `virtual_scan.source_timeout: 0.0` をローカルで戻さないと >1.0 m 走行で invalid-source STOP が再発**＝[tests/e2e/README.md:242](../../tests/e2e/README.md) 注記。宣言キー集合は `tests/unit/test_collision_monitor_config.py` が pin） → **【2026-09-16 追記】Jazzy 以降での戻しの置き場 = launch 注入（yaml には戻さない）**: `nav2_bringup.launch.py` が環境変数 `ROS_DISTRO` を見て、per-source key を**宣言しない** Humble／Iron（humble source.cpp:67-74・iron source.cpp:64-69・参照日 2026-09-16）**以外**では collision_monitor Node の `parameters` に `virtual_scan.source_timeout: 0.0` を追加する。根拠 = jazzy source.cpp:74-78（per-source key を宣言・既定は node-level 値）・:88（`source_timeout_.seconds() != 0.0 && dt > source_timeout_`＝**0.0 はその source の timeout 無効**）・jazzy collision_monitor_node.cpp:438-443（invalid source → STOP）。値は PR#229 が出荷した 0.0 と同じ。yaml は Humble の唯一の真実のまま（宣言キー集合 pin 不変・実 `scan` には注入しない＝Jazzy の途絶 STOP 意味論は温存）。実装 = `warehouse_bringup.collision_monitor_distro.virtual_scan_timeout_overrides(ros_distro)`（純関数・R-26 unit: humble／iron → 空、jazzy／kilted／rolling／未設定 → 注入、値 0.0、`scan` 不変）＋ launch 配線 pin（bringup PR `feat/collision-monitor-jazzy-inject`）。**未設定・未知の distro は注入側に倒す**（宣言しない distro では undeclared override として無害に無視され、宣言する distro で省くと >1.0 m 走行が偽 STOP になる＝注入が常に安全側）。
- 同旨の same-line 訂正（行位置不変）: 本 doc :183／:513／:535／:546／:558、[doc07:249](../shared/07-research-notes.md)、[doc23:380](23-perception-and-localization.md)、[mode-x-er/10:131](../mode-x-er/10-room-scale-safety-review.md)／[:268](../mode-x-er/10-room-scale-safety-review.md)／[:270](../mode-x-er/10-room-scale-safety-review.md)、[tests/e2e/README.md:240](../../tests/e2e/README.md)／[:272](../../tests/e2e/README.md)、`ws/src/warehouse_safety/CLAUDE.md`（TODO 行）・`ws/src/warehouse_bringup/CLAUDE.md`（collision_monitor 行）。

### (3) 裁定: センサ途絶停止は **Emergency Guardian の新理由 `scan_stale`**（L1・`warehouse_safety`）に置く

- **採用＝Guardian 拡張**。既存 freshness guard の idiom（`pose_stale`＝:506-513。[mode-m1/05 §5](../mode-m1/05-operation-state-and-stop-authority.md) が先例として指定する「`BotState` への既定値付きフィールド追加＋`evaluate` の追加ブロック」）を `/{bot}/scan`（[doc03:78](03-software-architecture.md) `sensor_msgs/LaserScan`・BEST_EFFORT sensor QoS）の**到着鮮度**に適用する。理由: ① 停止の実行機構（prio100 zero `Twist` の level 再アサート＋Nav2 goal cancel＋edge-trigger event＋`/{bot}/stop_state` feed＝L0' 停止上乗せへの伝播）が既に R-26 で固定されており、新理由は「入力 1 本＋純ロジック 1 ブロック」の additive で済む。② Guardian は既に pose・odom の**到着鮮度**を同じ単調時計で扱っており（`_last_pose_t`／`odom_freshness_timeout`）、別ノードにすると鮮度判定の時計とゲート規律が二重化する。③ Mode M1 では collision_monitor が唯一の対人 L1 反射（[mode-m1/01:12-22](../mode-m1/01-mode-boundary-and-traffic.md)）ゆえ、その**入力の生存**は同じ L1 で監視するのが層の整合。④ 責務分離表（[mode-m1/05:11-24](../mode-m1/05-operation-state-and-stop-authority.md)）で「停止条件が成立しているか」は Guardian の担当。
- **不採用＝`/operator/stop_request` 互換 JSON を出す新 producer**。[doc03:112](03-software-architecture.md)／[mode-m1/05 §5](../mode-m1/05-operation-state-and-stop-authority.md) の通りこの topic は **fleet 全体の単一 latch**（`OperatorStopLatch` は bool 1 個＝`guard_logic.OperatorStopLatch`）で、producer は `teleop_joy`（人の意思表示）。機械 producer が `clear` を出すと**人が engage した停止まで解除**しうる（§5「停止理由は理由ごとに保持」に反する）。per-bot にもできず、payload に `source`／理由を足すのは契約変更（doc03:112・§5・`parse_operator_stop_action`）で、採用案なら不要。よって不採用（`/operator/stop_request` は人の意思表示専用のまま）。
- **impl slice 仕様**（`track:safety-state`・編集は `warehouse_safety`＋`config/warehouse.base.yaml` `safety:` 節・凍結契約不変・**実装済 = `feat/guardian-scan-stale`（R-26 unit `tests/unit/test_guardian_scan_stale.py`・5 変異体 KILLED）**）。各行 = [何をするか] — 根拠 — 検証: **live 検証済（2026-09-16）**: Humble 1.1.20 rclpy（`ros:humble-ros-base`・Gazebo なし・偽 `/bot1/odom`／`/bot1/scan`）で P0 未受信＋odom → `scan_stale`（`scan_age: null`）／P3 途絶 → `scan_age` 1.035 s で estop・prio100 zero 20 Hz・`stop_state` true／P4 再開 → 解除・bot2 無音＝[deploy/dev/humble-guardian-livetest/README.md](../../deploy/dev/humble-guardian-livetest/README.md)。Gazebo（Humble）は ADR-0008 §Open のため未実施。
  1. `BotState.scan_age: float | None = None`（初回受信前 `None`）と `BotState.odom_seen: bool = False`（`/{bot}/odom` を 1 度でも受信したか）を additive 追加 — `guard_logic.BotState` の既存 idiom（`pose_age` フィールド） — 既定値で既存 unit 全緑（非回帰）。
  2. `evaluate(..., scan_freshness_timeout=...)` に (6) ブロック: `scan_age > scan_freshness_timeout`（厳密 `>`・`pose_stale` の (4) ブロックと同じ）**または**（`scan_age is None` **かつ** `odom_seen`）→ `Decision(bot, "estop", "scan_stale", {"scan_age", "freshness_timeout"})` — 「bot が生きている証拠（odom）があるのに scan が一度も来ない」を fail-closed にし、bot 不在（単騎 [ADR-0006](../adr/0006-single-bot-first.md)・`emergency_guardian._BOTS` 2 固定）では無音のまま — R-26 unit（独立オラクル＝手計算の age。`>`→`>=`／`None` 規則／odom 証人の各 mutation で赤になること）。
  3. node: `/{bot}/scan` を `sensor_qos`（`emergency_guardian.EmergencyGuardian.__init__` の BEST_EFFORT sensor profile）で購読し**到着時刻（`time.monotonic()`）のみ**を記録する（header stamp は使わない＝sim／実機の時計差を持ち込まない。`_on_pose` と同型）。`_on_odom` で `odom_seen` を立てる。callback に logic を置かない（同 node の「callbacks: store latest only, no logic」規律）。
  4. しきい値 `safety.scan_freshness_timeout`（`config/warehouse.base.yaml` `safety:` 節・既定 **1.0 s**） — 出所は [`collision_monitor.yaml:58`](../../ws/src/warehouse_bringup/config/collision_monitor.yaml:58) の node-level `source_timeout 1.0`（:552 ③ の live-tune 対象と**同一値を継承**＝発明ではない。T-mini Plus の走査 6-12 Hz＝83-167 ms（[doc23:494](23-perception-and-localization.md)）に対し ~6-12×） — `load_config` の hard-index（gate param と同じ「欠落は起動失敗」）＋ `guard_logic.validate_scan_freshness_timeout`（非有限・≤0・非数値は起動拒否＝NaN で比較が常に False になる fail-open を封じる）＋既定値 pin の unit。
  5. 動作＝**level・自動解除**（scan 再開で `EdgeLatch` が落ちる＝`pose_stale` :511 と同じ）。event `type="scan_stale"` は additive（コア形 :141-150 不変）。`/{bot}/stop_state` へは既存 `stop_requested_for`（`emergency_guardian._publish_stop_state`）経由で自動伝播＝**L0' 停止上乗せ有効時は teleop 経路も止まる**（twist_mux を経ない `teleop_joy`→`/bot1/cmd_vel` 直 publish は prio100 では止まらない＝[mode-outdoor/09:16](../mode-outdoor/09-external-review-v3-response.md) の事実。overlay 無効の構成では Nav2 経路のみ）。
  6. 記録: `warehouse_safety/CLAUDE.md` の consume に `/{bot}/scan`、produce の event type に `scan_stale`、[doc07:249](../shared/07-research-notes.md) R-39 の緩和列を更新（[implementation-and-dependencies.md §2](../../.claude/rules/implementation-and-dependencies.md)）。
- **非対象・残留（隠さない）**: ① Guardian 単独死の fail-active 窓（[mode-x-er/10:470](../mode-x-er/10-room-scale-safety-review.md)）は本裁定で縮まない＝`stop_overlay_enabled: true` の L0' 上乗せ（[mode-m1/05 §4-1](../mode-m1/05-operation-state-and-stop-authority.md)・無信号＝失権）が担う。② collision_monitor 自身の死（プロセス落ち）は twist_mux nav2 入力の沈黙→W-1 で止まるが**復帰許可は Nav2 側に残る**（[mode-outdoor/09 §2-b](../mode-outdoor/09-external-review-v3-response.md)「高優先入力の消失を復帰許可にしない」は Mode Outdoor の permit 契約で担保・共通構成では未対応）。③ scan の**内容**異常（全 NaN・凍結フレーム・有効観測率）は本裁定の対象外＝Mode Outdoor X2 `sensor_health`（`OQ-OD95`）に残す。本裁定は OQ-OD95 の「Guardian 拡張 or 新 node」に対して共通構成側の先例（Guardian 拡張）を与えるが、X2 の決定は mode-outdoor 側。**【2026-09-18 決定済】= [mode-outdoor/04 追補 ⑬](../mode-outdoor/04-perception-sidewalk-and-signals.md:1270)（`OQ-OD95` = A ＝本追補と同じ Guardian 拡張を採用し、estop 理由 `terrain_health` を additive 追加。既定 OFF ＝ config `perception.terrain.enabled`）。** ④ `collision_monitor.yaml` の inert キー 3 点は値不変のまま（(2)）。⑤ ボード導入版の実査は **2026-09-16 に実施済**（(1)・#692）。⑥ M1 実機の**配線順序**（T-mini Plus driver を契約 topic `/bot1/scan` へ上げてから `m1_driver` の `odom_enabled: true`。逆順は odom 到着の瞬間から `scan_stale` を恒久保持＝設計どおりの fail-closed）と「per-reason disable は**未裁定**（`validate_scan_freshness_timeout` が ≤0 を拒否＝無効化経路は意図的に閉）」は [mode-m1/03 末尾【2026-09-16 追補②】](../mode-m1/03-joystick-teleop-bringup.md) に置く。
- **【2026-09-16 追記②】age の下限（負 age・NaN ＝ 時計不一致 → 「age 不明」）の裁定**: `scan_age` / `pose_age` は node が同一 tick の `time.monotonic()` の読みから差分で算出し（上記 3.・:509）、単調時計は逆行しないため負にならない。純ロジック `guard_logic.evaluate` を別時計で呼ぶ caller（sim harness 等）が**負または NaN の age** を渡した場合、それは鮮度の測定値ではなく **age 不明**として扱う（`guard_logic.age_is_unknown`）。規則ごとの帰結: ① `scan_age` 不明は上記 2. の `None`（未受信）規則と同格＝`odom_seen` なら fault → `scan_stale` estop・bot 不在なら無音（fail-closed）。② `pose_age` 不明は「受信はあるが読みが信用できない」＝**stale 扱い**とし、通常の stale pose と同様に変位ゲート（[23 A-5③](23-perception-and-localization.md)）が裁く（駐機で odom 健全＝ゲート閉なら estop せず・走行中＝ゲート開なら `pose_stale` estop）＝fail-closed。**:509 の `None`（初回 pose 未受信）免除は不変**（未 localize の停止ロボットを estop しない理由は「受信が無い」ことにあり、時計不一致の読みには適用しない）。③ event `detail.scan_age` / `detail.pose_age` は**有限値のみ**を載せ、不明／非有限は `null`（NaN / Infinity を JSON に出さない＝:293 と同じ規律。`+inf` の age は従来通り厳密 `>` で stale だが detail は `inf` でなく `null`＝既存決定の detail 表現のみの変更）。有限・非負の age の判定は bit-identical（additive・estop が増える方向のみ・凍結契約 / config / topic / event コア形 :141-150 は不変）。同型の裁定＝変位ゲート `PoseGateTracker.snapshot` の閉区間 `0 <= age <= stale_after`（PR #684・`warehouse_safety/CLAUDE.md`【2026-09-16 追記】）。R-26: `tests/unit/test_guardian_scan_stale.py` 末尾（不明 age × odom 証人の有無・`None` 同値・`+inf`・有限域の不変・`pose_age`＝stale→ゲート裁定・`None` 免除・truth table・mutation で赤）。
