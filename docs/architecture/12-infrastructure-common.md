...
- **値の変更**（`max_points: 3` の明示・`state_topic` 撤去・`virtual_scan.source_timeout` 撤去）は本追補を正本とする**別 config PR**（docs-first。[tests/unit/test_collision_monitor_config.py](../../tests/unit/test_collision_monitor_config.py) の値 pin と [:552](12-infrastructure-common.md:552) ③ の live-tune を同時に更新）。
