...
2. **成果物は参照で残す・コピーしない**: `file:line` / `PR#` / commit sha / URL で指す（[docs-first.md §引用](../../rules/docs-first.md)）。全文貼り付けは腐敗と肥大の元。
3. **suggested next**: 次に読む doc / 起こす skill / 次の 1 手を 1–3 行で。
4. **秘密を redact**: 鍵・トークン・`.env` 値を書かない（[safety.md](../../rules/safety.md) / [environments.md](../../rules/environments.md)）。

## anti-sprawl（重要）

`local-memory.md` は**短命な再開コア**。古いラウンドは**prune する**（積み増しは肥大＝`MEMORY.md` が 24.4KB 上限を超えた実例と同じ失敗）。永続的な設計事実は docs / ADR へ、教訓は [docs/dev/03-retrospectives.md](../../../docs/dev/03-retrospectives.md) へ逃がす。runbook 運用の詳細は [session-orchestration.md](../../rules/session-orchestration.md)「local-memory.md 運用」。
