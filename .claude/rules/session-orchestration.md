...
- 記憶での file:line 引用（必ず実 Read 裏取り。docs-first.md:42（§引用））。

## 8. brief の引用は 2 レジーム（docs は file:line / impl-target は契約で）

brief（kickoff / next-action）で対象を指すとき、**2 種類を混ぜない**:
- **docs（正本）** = **`file:line`** で引く。docs は source-of-truth で不変性が高く、たどれる形が要る（docs-first.md:42（§引用）・本書 §4/§5 の「file:line」）。
- **implementation target（変更するコード）** = **型・IF・契約の形**で名指し、**path/line で固定しない**。実装は churn する＝`path:NN` は即ドリフト（#165 / status-maintenance.md:26）。「`warehouse_interfaces.X` の seam」「`/ns/topic`(型)」のように**契約で**指す。
> 目的: brief が腐らない。docs 参照は再検証可能に保ち、impl 参照は行ズレで無効化されないようにする。

## 9. local-memory.md 運用（anti-sprawl・reference-not-copy）

`.claude/local-memory.md` は**短命な再開コア**（設計正本ではない）。skill [session-handoff](../skills/session-handoff/SKILL.md) で書く:
- **参照で残す・コピーしない**（`file:line`/`PR#`/sha/URL）。全文貼付は腐敗・肥大の元。
- **古いラウンドを prune** する（積み増しは肥大＝`MEMORY.md` が 24.4KB 上限を超えた実例）。永続事実は docs/ADR、教訓は docs/dev/03 へ逃がす。
- **秘密を redact**（鍵/`.env` 値を書かない。safety.md / environments.md）。
