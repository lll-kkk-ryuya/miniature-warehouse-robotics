...
| 種別 | ラベル | 意味 |
|---|---|---|
| トラック | `track:skeleton` `track:llm-bridge` `track:safety-state` `track:sim` `track:nav-traffic` `track:wo` `track:jetson` `track:firmware` `track:web` `track:docs` | どの担当領域か |
...
| `web/**`（`web/console` フロント / `web/e2e`） | web（`track:web`） | **別ファイルなら自由**。`web_bridge` は observe-only consumer（doc22）。`.github/workflows/ci.yml` 内の web 系 job は governance 所有 |
